...
Функция HTTPConnection(ИмяСервиса) Экспорт
	...
	Если ИмяСервиса = "MS" Тогда
		
		КонечнаяТочка = РаботаВМоделиСервисаБТС.КонечнаяТочкаМенеджераСервиса();
		//СтруктураНастроек = РегистрыСведений.НастройкиТранспортаОбменаДанными.НастройкиТранспортаWS(КонечнаяТочка);
		СтруктураНастроек = РегистрыСведений.НастройкиТранспортаОбменаСообщениями.НастройкиТранспортаWS(КонечнаяТочка);
		
		WSURLВебСервиса = СтрЗаменить(СтруктураНастроек.WSURLВебСервиса, "https://", "");
		WSURLВебСервиса = СтрЗаменить(WSURLВебСервиса, "http://", "");
		СтрукрутаАдреса = СтрРазделить(WSURLВебСервиса, "/");
		Сервер = СтрукрутаАдреса[0]; 

		Порт = 443;
		Пользователь = СтруктураНастроек.WSИмяПользователя;
		Пароль = СтруктураНастроек.WSПароль;
		Прокси = Неопределено;
		Таймаут = 0;
