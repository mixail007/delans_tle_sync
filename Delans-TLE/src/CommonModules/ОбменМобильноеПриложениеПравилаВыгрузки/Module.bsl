...
Процедура СериализоватьИДобавитьОстаткиЗапасовВЕдиныйПакетВыгрузки(ВозвращаемыйСписок, Данные) Экспорт
	...
	ПередаваемыйОбъект = ОбменМобильноеПриложениеПравилаВыгрузки.СоздатьОбъектXDTO("Remains");
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.Номенклатура) Тогда
			КоличествоОбъектов = 0;
			ДобавляемаяСтрока.Nomenclature = ОбменМобильноеПриложениеПравилаВыгрузки.ПолучитьОбъектXDTO(Выборка.Номенклатура.ПолучитьОбъект(), КоличествоОбъектов);
		Конецесли;
