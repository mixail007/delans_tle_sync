...
Процедура ЗаполнитьПараметрыСканированияАкцизныхМарок(Параметры, Форма) Экспорт
	...
	Попытка
		Если Форма.Объект.Свойство("Ссылка") Тогда
			...
			ЭтоРозничнаяПродажа = ТипЗнч(СсылкаНаДокумент) = Тип("ДокументСсылка.ЧекККМ")
				ИЛИ ТипЗнч(СсылкаНаДокумент) = Тип("ДокументСсылка.ЧекККМВозврат");
			
			Если ЭтоРозничнаяПродажа Тогда
				
				ОрганизацияЕГАИС = ИнтеграцияЕГАИСУНФВызовСервера.ОрганизацияЕГАИСПоОрганизацииИТорговомуОбъекту(Форма.Объект.Организация, Форма.Объект.СтруктурнаяЕдиница);
				
				Если ТипЗнч(СсылкаНаДокумент) = Тип("ДокументСсылка.ЧекККМВозврат") Тогда
