...
Процедура ПриЗаписиЗаказа(ЗаказОбъект) Экспорт
	...
	Если ЗаказОбъект.ДополнительныеСвойства.Свойство("ДанныеИсторииСостояния") Тогда
		...
		Если ЗаказОбъект.ДополнительныеСвойства.ДанныеИсторииСостояния.ЗаписыватьСостояниеВИсторию Тогда
			...
			СостоянияЗаказов.СохранитьСостояниеЗаказаВИстории(ЗаказОбъект.Ссылка, ЗаказОбъект.СостояниеЗаказа, ЗаказОбъект.ДополнительныеСвойства.ДанныеИсторииСостояния);
...
//
// Параметры:
//  Состояние1,Состояние2  - СправочникСсылка.СостоянияЗаказовПокупателей, СправочникСсылка.СостоянияЗаказНарядов, СправочникСсылка.СостоянияЗаказовПоставщикам, СправочникСсылка.СостоянияЗаказовНаПроизводство - состояние для которого получается порядок.
//  ВидЗаказа - СправочникСсылка.ВидыЗаказовПокупателей, СправочникСсылка.ВидыЗаказНарядов	 - параметр является обязательным для состояний заказов покупателей и заказ-нарядов.
//
// Возвращаемое значение:
...
Процедура АссистентУправленияПриСрабатыванииСобытия(Заказ, ДанныеИсторииСостояния = Неопределено)
	...
	Если ТипЗнч(Заказ) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		АссистентУправления.ПриСрабатыванииСобытия(Заказ, "ИзменениеСостоянияЗаказаПокупателя");
	КонецЕсли;
	
	Если ТипЗнч(Заказ) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
		АссистентУправления.ПриСрабатыванииСобытия(Заказ, "ИзменениеСостоянияЗаказаПоставщику");
	КонецЕсли;
	
	Если ТипЗнч(Заказ) = Тип("ДокументСсылка.ЗаказНаПроизводство") Тогда
		АссистентУправления.ПриСрабатыванииСобытия(Заказ, "ИзменениеСостоянияЗаказаНаПроизводство");
	КонецЕсли;
...
Функция ЗаказПрошелСинхронизируемоеСостояние(Заказ, СинхронизируемоеСостояние, СинхронизируемыйВариантЗавершения = Неопределено, ТекущееСостояние = Неопределено)
	...
	ЗаказПрошелСинхронизируемоеСостояние = ?(СостоянияЗаказов.СравнитьСостояния(СинхронизируемоеСостояние, ДанныеЗаказа.СостояниеЗаказа, ДанныеЗаказа.ВидЗаказа) <= 0, Истина, Ложь);
