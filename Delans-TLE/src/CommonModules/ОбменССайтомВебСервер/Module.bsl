...
Функция ПолучитьКлассификаторИКаталог(ДатаИзменения, КодГруппы, КодУзлаОбмена = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(КодУзлаОбмена) Тогда
		...
		Если ЗначениеЗаполнено(УзелОбмена) Тогда
			...
			Возврат ОбменССайтом.ПолучитьКлассификаторИКаталогВебСервис(УзелОбмена, НСтр("ru = 'Обмен веб-сервис в формате 2.10'"));
...
Процедура ПодготовитьДанныеДляВыгрузкиОстатовИЦен(Параметры)
	...
	Параметры.Вставить("ВыборкаВидовЦен", МассивРезультатовЗапроса[18].Выбрать());
	
	Параметры.Вставить("ДеревоСвойствХарактеристик", 
		МассивРезультатовЗапроса[19].Выгрузить(ОбходРезультатаЗапроса.ПоГруппировкам));
	
	РезультатЗапросаДанныхОрганизацииВладельцаКаталога = МассивРезультатовЗапроса[21];
	Если РезультатЗапросаДанныхОрганизацииВладельцаКаталога.Пустой() Тогда
		ВыборкаДанныхОрганизацииВладельцаКаталога = Неопределено;
	Иначе
		ВыборкаДанныхОрганизацииВладельцаКаталога = РезультатЗапросаДанныхОрганизацииВладельцаКаталога.Выбрать();
...
Функция ПолучитьОстаткиИЦены(ДатаИзменения, КодГруппы, КодСклада, КодОрганизации, КодУзлаОбмена = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(КодУзлаОбмена) Тогда
		...
		Если ЗначениеЗаполнено(УзелОбмена) Тогда
			...
			Возврат ОбменССайтом.ПолучитьОстаткиИЦеныВебСервис(УзелОбмена, НСтр("ru = 'Обмен веб-сервис в формате 2.10'"));
...
Функция ПолучитьЗаказы(ДатаИзменения, КодУзлаОбмена = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(КодУзлаОбмена) Тогда
		...
		Если ЗначениеЗаполнено(УзелОбмена) Тогда
			...
			Возврат ОбменССайтом.ПолучитьЗаказыВебСервис(УзелОбмена, НСтр("ru = 'Обмен веб-сервис в формате 2.10'"));
