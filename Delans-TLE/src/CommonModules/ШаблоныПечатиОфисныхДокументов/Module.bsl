...
//
Процедура ДобавитьКомандыПечати(НазначениеШаблона, КомандыПечати, СписокФорм, МестоРазвещения, Порядок) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		КомандаПечати.Идентификатор = "ПечатьПоШаблонуОфисногоДокумента_ДоговорКонтрагента_" + Итератор;
		...
		КомандаПечати.МестоРазмещения = МестоРазвещения;
...
Функция СтруктураДокумента(Файл, УникальныйИдентификатор = Неопределено) Экспорт
...
Функция ДокументИзСтруктуры(СтруктураДокумента) Экспорт
...
Функция СФормироватьДокумент(Знач ОбъектПечати, Шаблон, УникальныйИдентификатор) Экспорт
	...
	Если СтруктураДокумента.Расширение = "docx" Тогда
		ЗаполнитьПараметрыДокументаDOCX(ДополнительныеПараметры, ОписаниеПараметров, СтруктураДокумента);
		ЗаписатьФайлСвязейДокументаDOCX(СтруктураДокумента);
		ЗаписатьФайлТипыКонтентаДокументаDOCX(СтруктураДокумента);
	ИначеЕсли СтруктураДокумента.Расширение = "odt" Тогда
		ЗаполнитьПараметрыДокументаODT(ДополнительныеПараметры, ОписаниеПараметров, СтруктураДокумента);
	КонецЕсли;
...
Функция ДобавитьКартинкуВСтруктуруДокумента(СтруктураДокумента, ДвоичныеДанные, Расширение) Экспорт
...
Функция ТабличныеЧастиДокумента(СтруктураДокумента, ОписаниеПараметров) Экспорт
...
Процедура ДобавитьПараметрыКонтактнойИнформацииОбъекта(Элементы, ВладелецКИСсылка, ПрефиксПараметр, ТаблицаВладелец = Неопределено) Экспорт
	...
	ВидыКонтактнойИнформации = УправлениеКонтактнойИнформацией.ВидыКонтактнойИнформацияОбъекта(ВладелецКИСсылка);
	
	Для каждого ВидКИ Из ВидыКонтактнойИнформации Цикл
		Если ТаблицаВладелец = Неопределено Тогда
			ДобавитьПараметрШаблона(Элементы, ПрефиксПараметр + ШаблоныПечатиОфисныхДокументов.ИмяПараметраВидКонтактнойИнформации(ВидКИ.Наименование), ВидКИ.Наименование);
		Иначе
			ДобавитьПараметрТаблицыШаблона(Элементы, ПрефиксПараметр + ШаблоныПечатиОфисныхДокументов.ИмяПараметраВидКонтактнойИнформации(ВидКИ.Наименование), ВидКИ.Наименование, ТаблицаВладелец);
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьОписаниеПараметров(ВыбранныеПараметры, НазначениеШаблона, Шаблон = Неопределено)
...
#Область ЗаполнениеПараметровШаблона
...
Процедура ЗаполнитьЗначенияКонтактнойИнформации(ОписаниеПараметров, КлючВладелец, ЗначениеВладелец) Экспорт
	...
	Для каждого ОписаниеПараметра Из ОписаниеПараметров Цикл
		...
		Для каждого ЗначениеКИ Из КонтактнаяИнформацияОбъекта Цикл
			ИмяПараметраВидКИ = ШаблоныПечатиОфисныхДокументов.ИмяПараметраВидКонтактнойИнформации(ЗначениеКИ.Вид.Наименование);
			Если СтрСравнить(ИмяПараметраВидКИ, ИмяПараметра) <> 0 Тогда
				Продолжить;
			КонецЕсли;
			Если ОписаниеПараметра.Значение = Неопределено Тогда
				ОписаниеПараметра.Значение = ЗначениеКИ.Представление;
			Иначе
				ОписаниеПараметра.Значение = ОписаниеПараметра.Значение + ЗначениеКИ.Представление;
			КонецЕсли;
		КонецЦикла;
...
Процедура ЗаполнитьЗначенияИзСтрокиТаблицыЗначения(ОписаниеПараметров, КолонкиТабличнойЧасти, СтрокаТаблицы, Префикс = "") Экспорт
	...
	Для каждого Колонка Из КолонкиТабличнойЧасти Цикл
		Для каждого ОписаниеПараметра Из ОписаниеПараметров Цикл
			...
			ОписаниеПараметра.Значение = СтрокаТаблицы[Колонка.Имя];
...
Функция ЗаменяемыеСимволы()
	
	ЗаменяемыеСимволы = Новый Соответствие;
	ЗаменяемыеСимволы.Вставить("""", "&quot;");
	ЗаменяемыеСимволы.Вставить("<", "&lt;");
	ЗаменяемыеСимволы.Вставить(">", "&gt;");
	
	Возврат ЗаменяемыеСимволы;
	
КонецФункции

Процедура ЗаменитьСлужебныеСимволы1С(Строка) Экспорт
	
	Для каждого КлючЗначение Из ЗаменяемыеСимволы() Цикл
		Строка = СтрЗаменить(Строка, КлючЗначение.Ключ, КлючЗначение.Значение);
	КонецЦикла;
	
КонецПроцедуры

Процедура ЗаменитьСлужебныеСимволыОфиснойПрограммы(Строка)
	
	Для каждого КлючЗначение Из ЗаменяемыеСимволы() Цикл
		Строка = СтрЗаменить(Строка, КлючЗначение.Значение, КлючЗначение.Ключ);
	КонецЦикла;
...
Функция НовыйЧтениеXML(ПутьКФайлу) Экспорт
...
Функция НовыйЗаписьXML(ПутьКФайлу) Экспорт
...
Процедура ЗаписатьФайлСвязейДокументаDOCX(СтруктураДокумента) Экспорт
...
Процедура ЗаписатьФайлТипыКонтентаДокументаDOCX(СтруктураДокумента) Экспорт
...
Процедура ОбновитьСвойстваЧтенияXMLДокументаDOCX(ЧтениеXML, ПараметрыЧтенияXML) Экспорт
	...
	Если НЕ ПустаяСтрока(ПараметрыЧтенияXML.ИмяПараметра) Тогда
		ЗаменитьСлужебныеСимволыОфиснойПрограммы(ПараметрыЧтенияXML.ИмяПараметра);
	КонецЕсли;
...
Функция ПараметрыИзДокументаDOCX(ПутьКФайлу)
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Пока ЧтениеXML.ПрочитатьАтрибут() Цикл
			...
			ЗаменитьСлужебныеСимволыОфиснойПрограммы(Параметр);
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			Пока ЧтениеXML.ПрочитатьАтрибут() Цикл
				...
				//ЭтоСсылкаНаКартинку = ПараметрыЧтенияXML.НайденоПоле И ПараметрыЧтенияXML.ЭлементТекущийИмя = "r:embed" И ПараметрыЧтенияXML.ЭлементРодительИмя = "a:blip";    // НайденоПоле должно быть истина
...
Процедура ПродолжитьКопированиеДокументаИсточникаDOCX(ЗаписьXML, ЧтениеXML, ПараметрыЧтенияXML = Неопределено) Экспорт
...
Процедура ЗаполнитьПараметрыДокументаDOCX(ПараметрыПечати, ОписаниеПараметров, СтруктураДокумента)
...
Процедура ОбновитьСвойстваЧтенияXMLДокументаODT(ЧтениеXML, ПараметрыЧтенияXML) Экспорт
...
Процедура ПродолжитьКопированиеДокументаИсточникаODT(ЗаписьXML, ЧтениеXML, ПараметрыЧтенияXML = Неопределено) Экспорт
...
Процедура ЗаполнитьПараметрыДокументаODT(ПараметрыПечати, ОписаниеПараметров, СтруктураДокумента)
