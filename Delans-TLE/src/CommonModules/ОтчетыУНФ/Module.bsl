		...
		Если ТипЗнч(ЭлементПараметра.Значение) = Тип("ИдентификаторРасшифровкиКомпоновкиДанных") Тогда
			...
			Для Каждого Поле Из Поля Цикл
				...
				Если Прав(Строка(Поле.Поле), 6)="Иконка" Тогда
					Если Поле.Значение=1 Тогда
						Картинка = БиблиотекаКартинок.ЗначениеУвеличилось;
					ИначеЕсли Поле.Значение=2 Тогда
...
Процедура ОграничитьИспользованиеПоля(Схема, ИмяПоля) Экспорт
	...
	ПолеСКД = ОтчетыУНФ.ПолеСхемы(Схема, ИмяПоля);
...
// Когда в форме выбирается значение "Все", то ЗначениеПараметраКД.Значение = Неопределено,
// Предполагается, что запросах СКД предусмотрено отключение отбора по организации, когда передана пустая ссылка:
// Организация = &Организация ИЛИ &Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
//
// Параметры:
//  НастройкиСКД - НастройкиКомпоновкиДанных - настройки отчета.
//
Процедура УстановитьОрганизациюПоУмолчанию(НастройкиСКД) Экспорт
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоОрганизаций") Тогда
		УстановитьПараметрОтчетаПоУмолчанию(НастройкиСКД, "Организация", Справочники.Организации.ПустаяСсылка());
	Иначе
		УстановитьПараметрОтчетаПоУмолчанию(НастройкиСКД, "Организация", Справочники.Организации.ОрганизацияПоУмолчанию());
	КонецЕсли;
	
КонецПроцедуры

// Если параметр не указан, то переопределяет его переданным значением.
//
// Параметры:
//  НастройкиСКД - НастройкиКомпоновкиДанных - настройки отчета,
...
// Здесь перед формированием объединяем пользовательские настройки, заданные в настройках рассылки с обычными настройками отчета.
//
// Параметры:
//  ПараметрыОтчета  - Структура - См. РассылкаОтчетов.ИнициализироватьОтчет(), возвращаемое значение.
//  ПараметрыФормирования  - Структура - Настройки подключения отчета.
//
Процедура ОбъединитьПользовательскиеИОбычныеНастройки(Знач ПараметрыОтчета, Знач ПараметрыФормирования) Экспорт
	
	Если Не ПараметрыОтчета.СКД Тогда
		Возврат;
	КонецЕсли;
