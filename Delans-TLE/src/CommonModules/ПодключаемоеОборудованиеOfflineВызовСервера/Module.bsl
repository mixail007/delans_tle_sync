...
Процедура ЗагрузитьОтчетыОПродажахИзККМ(ОфлайнОборудование, ДанныеОПродажах, Отказ, СообщениеОбОшибке) Экспорт
	...
	Для каждого ОтчетОПродажах Из ДанныеОПродажах Цикл
		...
		ТаблицаШтрихкодовАкцизов.Очистить();
		...
		Для каждого ЧекККМ Из ОтчетОПродажах.Чеки Цикл
			...
			Для каждого СтрокаТЧ Из ЧекККМ.Товары Цикл
				...
				// ЕФСОЛ Сережко А.С. 30.01.18 + 
				НоваяСтрока.ES_СтавкаНДС = УправлениеНебольшойФирмойПовтИсп.ПолучитьСтавкуНДС(СтрокаТЧ.ES_СтавкаНДС);
				НоваяСтрока.ES_ЭтоГруз  	= СтрокаТЧ.ES_ЭтоГруз;
				НоваяСтрока.ES_НомерЗаказа  = СтрокаТЧ.ES_НомерЗаказа;
				НоваяСтрока.ES_Содержание	= СтрокаТЧ.ES_Содержание;
				// ЕФСОЛ Сережко А.С. 30.01.18 -
		...
		КонецЦикла;
		//ЭР Несторук С.И. 07.09.2017 13:57:59 {
		//ТаблицаТоваров.Свернуть("Код, Цена, Заказ, СтавкаНДС, ШтрихкодАлкогольнойМарки, УникальныйИдентификатор", "Количество, Сумма, Скидка");
		//ТаблицаВозвратов.Свернуть("Код, Цена, Заказ, СтавкаНДС, ШтрихкодАлкогольнойМарки, УникальныйИдентификатор", "Количество, Сумма, Скидка");
		ТаблицаТоваров.Свернуть("Код, Цена, Скидка, СтавкаНДС, Заказ, ШтрихкодАлкогольнойМарки, УникальныйИдентификатор, ES_ЭтоГруз,ES_НомерЗаказа,ES_СтавкаНДС, ES_Содержание", "Количество, Сумма"); // добавил
		ТаблицаВозвратов.Свернуть("Код, Цена, Заказ, СтавкаНДС , ШтрихкодАлкогольнойМарки, УникальныйИдентификатор, ES_ЭтоГруз,ES_НомерЗаказа,ES_СтавкаНДС, ES_Содержание", "Количество, Сумма, Скидка");
...
Процедура ЗаполнитьСтруктуруПрайсЛистаИзДанныхКВыгрузке(ПрайсЛистККМ, ТоварыКВыгрузке, Параметры) Экспорт
	...
	|	ТоварыКВыгруке.Партия КАК Партия,
	...
	|	ТоварыКВыгруке.ПартияНаименование КАК ПартияНаименование,
	...
	|	ВЫБОР
	|		КОГДА ТоварыКВыгруке.Партия = ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
...
Функция ПустаяТаблицаТоваров()
	...
	//ЭР Несторук С.И. 07.09.2017 13:29:14 {
	ТаблицаТоваров.Колонки.Добавить("ES_ЭтоГруз", Новый ОписаниеТипов("Булево"));
	ТаблицаТоваров.Колонки.Добавить("ES_НомерЗаказа", Новый ОписаниеТипов("Строка"));
	ТаблицаТоваров.Колонки.Добавить("ES_СтавкаНДС", Новый ОписаниеТипов("СправочникСсылка.СтавкиНДС"));
	ТаблицаТоваров.Колонки.Добавить("ES_Содержание", Новый ОписаниеТипов("Строка"));
	//}ЭР Несторук С.И.
...
Функция ПолучитьВыборкуНоменклатурыПоSKU(ПравилоОбмена, ТаблицаТоваров)
	...
	//ЭР Несторук С.И. 07.09.2017 13:30:06 {
	|	,
	|	Товары.ES_ЭтоГруз,
	|	Товары.ES_НомерЗаказа,
	|	Товары.ES_СтавкаНДС,
	|	Товары.ES_Содержание
	//}ЭР Несторук С.И.
	|ПОМЕСТИТЬ Товары
	|ИЗ
	...
	//ЭР Несторук С.И. 07.09.2017 13:30:39 {
	|	Товары.ES_ЭтоГруз,
	|	Товары.ES_НомерЗаказа,
	|	Товары.ES_Содержание,
	//}ЭР Несторук С.И.
	|	ВЫБОР
	//ЭР Несторук С.И. 28.03.2018 15:48:02 {
	|    КОГДА  Товары.ES_ЭтоГруз ТОГДА
	|			Товары.ES_СтавкаНДС
	//}ЭР Несторук С.И.
...
Процедура СоздатьИЗаполнитьОтчетОПродажах(МассивСозданныхДокументов, РеквизитыККМ, ТаблицыДанных, Комментарий = "", СообщениеОбОшибке, Отказ)
	...
	Пока ВыборкаПоТоварам.Следующий() Цикл
		...
		//ЭР Несторук С.И. 07.09.2017 13:31:51 {
		Если ВыборкаПоТоварам.ES_ЭтоГруз Тогда
			НоваяСтрока.Номенклатура = Справочники.Номенклатура.Груз;
			НоваяСтрока.ЕдиницаИзмерения = Справочники.Номенклатура.Груз.ЕдиницаИзмерения;
		КонецЕсли;
		НоваяСтрока.ES_ЭтоГруз			= ВыборкаПоТоварам.ES_ЭтоГруз;
		НоваяСтрока.ES_Содержание		= ВыборкаПоТоварам.ES_Содержание;
		Если ЗначениеЗаполнено(ВыборкаПоТоварам.ES_НомерЗаказа) Тогда
			НоваяСтрока.Заказ			= Документы.ЗаказПокупателя.НайтиПоНомеру(ВыборкаПоТоварам.ES_НомерЗаказа);
		КонецЕсли;
		//}ЭР Несторук С.И.
