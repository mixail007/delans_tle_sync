...
Процедура ДобавитьИнформациюОНабореПриПолученииДанныхНоменклатуры(СтруктураДанные) Экспорт
	...
	Если СтруктураДанные.ЭтоНабор Тогда
		...
		|	СоставНаборов.НоменклатураНабора.СпособРасчетаЦеныНабора КАК СпособРасчетаЦеныНабора,
		...
		|	СоставНаборов.Номенклатура.Склад КАК Склад,
		|	СоставНаборов.Номенклатура.Ячейка КАК Ячейка,
		|	ВТСтавкиНДС.СтавкаНДС КАК СтавкаНДС,
		|	СоставНаборов.Номенклатура.Вес КАК ВесЕдиницыТовара,
		|	СоставНаборов.Номенклатура.Объем КАК ОбъемЕдиницыТовара,
		|	ВЫБОР
		...
		|	СоставНаборов.Номенклатура.ТипНоменклатуры КАК ТипНоменклатуры,
		|	СоставНаборов.Номенклатура.СтранаПроисхождения КАК СтранаПроисхождения
		|ПОМЕСТИТЬ СоставНаборов
		|ИЗ
		|	РегистрСведений.СоставНаборов КАК СоставНаборов
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТСтавкиНДС КАК ВТСтавкиНДС
		|		ПО (СоставНаборов.Номенклатура.ВидСтавкиНДС = ВТСтавкиНДС.ВидСтавкиНДС)
		|ГДЕ
		...
		|	СоставНаборов.ВесЕдиницыТовара КАК ВесЕдиницыТовара,
		|	СоставНаборов.ОбъемЕдиницыТовара КАК ОбъемЕдиницыТовара,
		...
		|		
		|		ИМЕЮЩИЕ
		|			КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ПартииКонтрагентов.Партия) = 1) КАК ПартииКонтрагентов
...
Процедура ПроверитьТабличнуюЧасть(Объект, ИмяТЧ, Отказ) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
			Объект,
			НСтр("ru = 'В табличной части присутствует набор'"),
			ИмяТЧ,
			Выборка.НомерСтроки,
			"Номенклатура",
			Отказ
		...
		Для каждого СтрокаНабора Из ДобавленныеНаборы Цикл
			...
			Если СтрокаНабора.ВыводитьИтоги Тогда
				...
				ИначеЕсли ДанныеСтроки.Свойство("Скидка") И ДанныеСтроки.Скидка>0 И ДанныеСтроки.Свойство("Всего") Тогда
					...
					ДанныеСтроки.Вставить("СуммаБезСкидки", ДанныеСтроки.Всего+ДанныеСтроки.Скидка);
				...
				ИначеЕсли ДанныеСтроки.Свойство("Всего") Тогда
					...
					ДанныеСтроки.Вставить("СуммаБезСкидки", ДанныеСтроки.Всего);
