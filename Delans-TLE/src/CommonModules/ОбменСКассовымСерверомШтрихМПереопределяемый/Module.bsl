...
//
Процедура ВыполнитьДействияПриУстановкеПараметровСеанса(ИменаПараметров) Экспорт
...
Процедура УстановитьИспользованиеРегламентногоЗадания() Экспорт
	...
	ВыполнятьОбменССерверомШтрихМ = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УзелОбмена, "ВыполнятьОбменССерверомШтрихМ")
		И ПолучитьФункциональнуюОпцию("ИспользоватьОбменСКассойMinikassir");
...
Функция ГруппыИТоварыИзмененные(КассовыйАппарат)
	...
	Запрос.УстановитьПараметр("ТоварыКВыгрузке",
		Обработки.ФормированиеПрайсЛистов.ПолучитьНоменклатуруПрайслиста(
			ИнтеграцияОбменШтрихМ.ПолучитьРеквизитыКассыШтрихМ(КассовыйАппарат).ПрайсЛист));
