...
//
// Параметры:
//  ХозяйствующийСубъект		 - СправочникСсылка.ХозяйствующиеСубъектыВЕТИС	 - хозяйствующий субъект, для которого требуется получить сопоставленый объект.
...
//  	по которому нужно делать отбор хозяйствуюещих субъетов грузоотправителя
...
//  	по которому нужно делать отбор хозяйствуюещих субъетов грузополучателя
...
//
// Параметры:
//  ТекстИсходящегоСообщенияSOAP - Строка - Текст оправляемого запроса в формате SOAP.
//  Сервис - Перечисление.СервисыВЕТИС - Сервис ВЕТИС.
//  SOAPAction - Строка - Операция.
//  НастройкаОбмена - Структура - Настрока обмена.
// 
// Возвращаемое значение:
...
Функция ПараметрыСозданияНоменклатуры(ПродукцияВЕТИС, ЕдиницаИзмеренияВЕТИС) Экспорт
	...
	РеквизитыЕдиницыИзмемренияВЕТИС = РезультатЗапроса[1].Выбрать();
	...
	Если РеквизитыЕдиницыИзмемренияВЕТИС.Следующий() Тогда
		ПараметрыНоменклатуры.ЕдиницаИзмеренияВЕТИС = РеквизитыЕдиницыИзмемренияВЕТИС.ЕдиницаИзмерения;
	КонецЕсли;
...
// Возвращаяет структуру с форматами дат для продукции/вида/типа продукции из макета "СпецифическаяПродукция" 
