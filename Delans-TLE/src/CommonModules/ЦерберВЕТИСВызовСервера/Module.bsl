...
//  Структура - см. функцию ИнтеграцияВЕТИСКлиентСервер.РезультатВыполненияЗапросаЭлементаКлассификатора().
//
Функция ХозяйствующийСубъектПоGUID(Идентификатор, Преообразовывать = Ложь, ХозяйствующийСубъект = Неопределено, ПараметрыОбмена = Неопределено) Экспорт
...
Функция ЗапросЭлементовКлассификатораЗарубежныхПредприятийXML(ПараметрыОтбора, НомерСтраницы)
	...
	Если ПараметрыОтбора <> Неопределено Тогда
		...
		Если ПараметрыОтбора.Свойство("НаименованиеАнлийское") Тогда
			Запрос.enterprise.englishName = ПараметрыОтбора.НаименованиеАнлийское;
		КонецЕсли;
		...
		Если ПараметрыОтбора.Свойство("Страна")
			...
			Если ПараметрыОтбора.Свойство("Страна") Тогда
				Запрос.enterprise.address.country = ПараметрыОтбора.Страна;
			КонецЕсли;
