/////////////////////////////////////////////////////////////////////
//
// Кешируемые серверные функции библиотеки ВетИС: 
...
Функция ПериодНахожденияЖивотныхНаТерриторииТС(КодИлиЗначение) Экспорт
	...
	Иначе
		ВызватьИсключение
			СтрШаблон(
				НСтр("ru = 'Неизвестный период нахождения животных на террритории ТС: %1'"),
				КодИлиЗначение);
	КонецЕсли;
...
#Область КешируемыеТаблицы

// Возвращаяет таблицу упаковок из классификатора ВетИС
// 
// Возвращаемое значение:
...
// Возвращаяет таблицу упаковок из классификатора ВетИС 1.4
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Таблица с колонками:
//   * Код                - ОпределяемыйТип.СтрокаВЕТИС - код упаковки
...
// Возвращаяет таблицу продукции/типов продукции с форматами даты выработки, срока годности
//
// Возвращаемое значение:
//	ТаблицаЗначений - Таблица с колонками:
//		* ИдентификаторПродукции	- ОпределяемыйТип.УникальныйИдентификаторВЕТИС	- УИД продкуции, типа продукции
...
#Область ПоискВКешируемыхТаблицах
