...
Функция Просклонять(Представление, Падеж, Объект = Неопределено, ЭтоФИО = Ложь, Пол = Неопределено) 
	...
	// УНФ
	Если ИспользоватьСклонениеСредствамиПлатформы() Тогда
		Возврат ПросклонятьСредствамиПлатформы(Представление, ИмяПадежа, Объект);
	КонецЕсли;
	// КонецУНФ
...
#Область СлужебныеПроцедурыУНФ

Функция ИспользоватьСклонениеСредствамиПлатформы() Экспорт
	
	СистемнаяИнформация = Новый СистемнаяИнформация();
	Возврат ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СистемнаяИнформация.ВерсияПриложения, "8.3.13.0") >= 0;
	
КонецФункции

Функция ПадежСтрокойПоИмени(ИмяПадежа)
	
	СоответствиеПадежей = Новый Соответствие;
	СоответствиеПадежей.Вставить("ИменительныйПадеж", "Именительный");
	СоответствиеПадежей.Вставить("РодительныйПадеж",  "Родительный");
	СоответствиеПадежей.Вставить("ДательныйПадеж",    "Дательный");
	СоответствиеПадежей.Вставить("ВинительныйПадеж",  "Винительный");
	СоответствиеПадежей.Вставить("ТворительныйПадеж", "Творительный");
	СоответствиеПадежей.Вставить("ПредложныйПадеж",   "Предложный");
	
	Возврат СоответствиеПадежей.Получить(ИмяПадежа);
	
КонецФункции

Функция ПросклонятьСредствамиПлатформы(Представление, ИмяПадежа, Объект = Неопределено) Экспорт
	
	Падеж = ПадежСтрокойПоИмени(ИмяПадежа);
	
	Если Падеж = Неопределено Тогда
		Возврат Представление;
	КонецЕсли;
	
	ФорматнаяСтрока = СтрШаблон("ПД=%1", Падеж);
	
	// Используется метод платформы, доступный только в 8.3.13
	// См. также ИспользоватьСклонениеСредствамиПлатформы()
	Склонения = Вычислить("ПолучитьСклоненияСтроки(Строка(Представление),, ФорматнаяСтрока)");
	
	Если Склонения.Количество() = 0 Тогда
		Возврат Представление;
	КонецЕсли;
	
	Результат = Склонения[0];
	
	Если ЗначениеЗаполнено(Объект) Тогда
		СтруктураСклонения = Новый Структура;
		СтруктураСклонения.Вставить(ИмяПадежа, Результат);
		ЗаписатьВРегистрСклонения(Представление, Объект, СтруктураСклонения);
	КонецЕсли;
