...
Процедура ЗаполнениеДокументовЕГАИС(Источник, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("СтатусПроверкиИПодбора", Источник.Метаданные()) Тогда
		Источник.СтатусПроверкиИПодбора = Перечисления.СтатусыПроверкиИПодбораЕГАИС.НеВыполнялось;
	КонецЕсли;
...
Процедура ОбработатьОснованияЕГАИС(Параметры) Экспорт
	...
	|		ПО (ДокументПриходнаяНакладная.УдалитьТоварноТранспортнаяНакладнаяЕГАИС = ДокументТТНВходящаяЕГАИС.Ссылка
	|				ИЛИ ДокументПриходнаяНакладная.ДокументОснование = ДокументТТНВходящаяЕГАИС.Ссылка)
...
Процедура ЗаполнитьРегистрСоотвествия()
	...
	|	ВТ_СопоставлениеНоменклатуры.Характеристика КАК Характеристика,
...
Процедура ДозаполнитьСоответствиеНоменклатурыЕГАИС(Параметры) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1000
	|	Вт_Соответствие.АлкогольнаяПродукция КАК АлкогольнаяПродукция,
	|	Вт_Соответствие.Номенклатура КАК Номенклатура,
	|	Вт_Соответствие.Характеристика КАК Характеристика,
	|	Вт_Соответствие.ИдентификаторУпаковки КАК ИдентификаторУпаковки
	|ИЗ
	|	Вт_Соответствие КАК Вт_Соответствие
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеНоменклатурыЕГАИС КАК СоответствиеНоменклатурыЕГАИС
	|		ПО Вт_Соответствие.АлкогольнаяПродукция = СоответствиеНоменклатурыЕГАИС.АлкогольнаяПродукция
	|			И Вт_Соответствие.Номенклатура = СоответствиеНоменклатурыЕГАИС.Номенклатура
	|			И Вт_Соответствие.Характеристика = СоответствиеНоменклатурыЕГАИС.Характеристика
	|ГДЕ
	...
	Для каждого СтрокаРезультата Из РезультатЗапроса Цикл
		...
		Попытка
			...
			Запись.АлкогольнаяПродукция = СтрокаРезультата.АлкогольнаяПродукция;
			Запись.ИдентификаторУпаковки = СтрокаРезультата.ИдентификаторУпаковки;
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось обработать запись: %1 по причине:
					|%2'"),
					СтрокаРезультата.Ссылка, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), 
				УровеньЖурналаРегистрации.Предупреждение,
				Метаданные.РегистрыСведений.СоответствиеНоменклатурыЕГАИС, СтрокаРезультата.Ссылка, ТекстСообщения);
		КонецПопытки; 
	КонецЦикла; 
	
	Если ОбъектовОбработано = 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Процедуре ДозаполнитьСоответствиеНоменклатурыЕГАИС
				|не удалось обработать некоторые записи (пропущены): %1'"),
				ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Процедура ДозаполнитьСоответствиеНоменклатурыЕГАИС
					|обработала очередную порцию записей: %1'"), ОбъектовОбработано));
	КонецЕсли;
