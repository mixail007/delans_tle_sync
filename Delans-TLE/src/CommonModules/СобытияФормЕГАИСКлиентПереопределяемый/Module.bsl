...
Процедура ВыполнитьПереопределяемуюКоманду(Форма, Команда) Экспорт
	...
	Если ТипЗнч(Команда) = Тип("ДекорацияФормы") Тогда 
		...
		Если Команда.Имя = "НадписьПолучитьИзЕГАИС" Тогда
			ДанныеСопоставленияОрганизацииЕГАИС = ИнтеграцияЕГАИСУНФВызовСервера.ДанныеСопоставленияОрганизацииЕГАИС(Команда.Подсказка);
...
Процедура ОткрытьФормуВыбораАлкогольнойПродукции(ФормаВладелец, Реквизиты, СтандартнаяОбработка) Экспорт
...
Функция ПараметрыСозданияКонтрагента() Экспорт
	...
	Результат = Новый Структура("ИНН,КПП, Наименование, СокращенноеНаименование",
		"ИНН", "КПП", "НаименованиеПолное", "Наименование");
	Возврат Результат;
