...
#Область ЗапросыОтчетаПоступленияБезУведомлений
...
Функция ТекстЗапросаПоПроблемнымПоступлениям(ИмяНабораДанных, ОтборОрганизация, ОтборКонтрагент) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПриходныеДокументы.Ссылка КАК Ссылка,
	|	ПриходныеДокументы.Организация КАК Организация,
	|	ПриходныеДокументы.Контрагент КАК Контрагент,
	|	СерийныеНомера.СерийныйНомер.НомерКиЗГИСМ КАК НомерКиЗ,
	|	СУММА(1) КАК КоличествоВДокументахПоступления,
	|	СерийныеНомера.Номенклатура.КиЗГИСМ КАК КиЗГИСМ,
	|	СерийныеНомера.Номенклатура.ВидМаркировки КАК ВидМаркировки
	|ПОМЕСТИТЬ СерийныеНомераПриходныхДокументов
	|ИЗ
	|	РегистрНакопления.СерийныеНомера КАК СерийныеНомера
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПриходныеДокументы КАК ПриходныеДокументы
	|		ПО СерийныеНомера.Регистратор = ПриходныеДокументы.Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	ПриходныеДокументы.Ссылка,
	|	ПриходныеДокументы.Организация,
	|	ПриходныеДокументы.Контрагент,
	|	СерийныеНомера.СерийныйНомер.НомерКиЗГИСМ,
	|	СерийныеНомера.Номенклатура.КиЗГИСМ,
	|	СерийныеНомера.Номенклатура.ВидМаркировки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ИЗ
	|	СерийныеНомераПриходныхДокументов КАК СерийныеНомераПриходныхДокументов
	|		ЛЕВОЕ СОЕДИНЕНИЕ ИмяТабличнойЧастиДокументаГИСМ КАК ТабличнаяЧастьДокументаГИСМ
	|		ПО СерийныеНомераПриходныхДокументов.Организация = ТабличнаяЧастьДокументаГИСМ.Ссылка.Организация
	|			И СерийныеНомераПриходныхДокументов.Контрагент = ТабличнаяЧастьДокументаГИСМ.Ссылка.Контрагент
	|			И СерийныеНомераПриходныхДокументов.НомерКиЗ = ТабличнаяЧастьДокументаГИСМ.НомерКиЗ
	|			И СерийныеНомераПриходныхДокументов.Ссылка = ТабличнаяЧастьДокументаГИСМ.ДокументПоступления
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СерийныеНомераПриходныхДокументов.Ссылка КАК ДокументПоступления,
	|	СерийныеНомераПриходныхДокументов.Организация КАК Организация,
	|	СерийныеНомераПриходныхДокументов.Контрагент КАК Контрагент,
	|	СерийныеНомераПриходныхДокументов.НомерКиЗ КАК НомерКИЗ
	|ИЗ
	...
	|		ПО СерийныеНомераПриходныхДокументов.Организация = ПроблемныеНомераКиЗ.Организация
	|			И СерийныеНомераПриходныхДокументов.Контрагент = ПроблемныеНомераКиЗ.Контрагент
	|			И СерийныеНомераПриходныхДокументов.НомерКиЗ = ПроблемныеНомераКиЗ.НомерКИЗ
	|ГДЕ
	|	ТабличнаяЧастьДокументаГИСМ.Ссылка ЕСТЬ NULL
	|	И &ТекстОтбораПоНоменклатуре
	|	И &ТекстОтбораДокументыПоступления
