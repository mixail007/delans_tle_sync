...
Функция ПолучитьРеквизитыКассира(Автор, ПодписьКассира = Неопределено) Экспорт
	...
	//ЭР_ +
	Результат.Вставить("ИмяКассираИДолжность", "Кассир");
...
Процедура ДополнитьТоварамиПоДокументу(ДокументОбъект, ОбщиеПараметры, ПризнакСпособаРасчета, ИмяТабличнойЧасти = "Запасы", НомерСтрокиТовара = 0) Экспорт
	...
	Иначе
		Для каждого ТабличнаяЧасть Из МассивТабличныхЧастей Цикл
			...
			Для каждого СтрокаТЧ Из ТаблицаДанных Цикл
				...
				СтавкаНДС = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеСтавкиНДСДляККТ(СтрокаТЧ.СтавкаНДС);
				...
				Если ТабличнаяЧасть = "ВнеоборотныеАктивы" Тогда
					...
					СтрокаПозицииЧека.ЦенаСоСкидками = СтрокаТЧ.Всего;
				...
				ИначеЕсли ТабличнаяЧасть = "Расходы" Тогда
					...
					Если ТаблицаДанных.Колонки.Найти("Содержание") <> Неопределено
						И НЕ ПустаяСтрока(СтрокаТЧ.Содержание) Тогда
						СтрокаПозицииЧека.Наименование = СокрЛП(СтрокаТЧ.Содержание);
					Иначе
						СтрокаПозицииЧека.Наименование = РеквизитыНоменклатуры.НаименованиеПолное;
					КонецЕсли;
					СтрокаПозицииЧека.Количество = СтрокаТЧ.Количество;
					СтрокаПозицииЧека.Цена = СтрокаТЧ.Цена + ?(ДокументОбъект.СуммаВключаетНДС, 0, СтрокаТЧ.СуммаНДС / ?(СтрокаТЧ.Количество = 0, 1, СтрокаТЧ.Количество)); 
					СтрокаПозицииЧека.ЦенаСоСкидками = СтрокаПозицииЧека.Цена;
				...
				Иначе
					...
					Если ТаблицаДанных.Колонки.Найти("Содержание") <> Неопределено
						...
						СтрокаПозицииЧека.Наименование = СокрЛП(СтрокаТЧ.Содержание);
					Иначе
						СтрокаПозицииЧека.Наименование = СтрокаПозицииЧекаНаименование(РеквизитыНоменклатуры, СтрокаТЧ);
					КонецЕсли;
					...
					Если ТабличнаяЧасть = "Работы" Тогда
						СтрокаПозицииЧека.Количество = СтрокаТЧ.Количество * СтрокаТЧ.Кратность * СтрокаТЧ.Коэффициент;
					Иначе
						СтрокаПозицииЧека.Количество = СтрокаТЧ.Количество;
					КонецЕсли;
					...
					Если ТабличнаяЧасть = "Запасы"
						...
						И ЗначениеЗаполнено(СтрокаТЧ.Партия) Тогда
	...
	// Доставка
	Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(ДокументОбъект)) 
		И ДокументОбъект.Метаданные().Реквизиты.Найти("НоменклатураДоставки") <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(СтруктураРеквизитов.НоменклатураДоставки) И СтруктураРеквизитов.СтоимостьДоставки>0 Тогда
			СтавкаНДС = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеСтавкиНДСДляККТ(СтруктураРеквизитов.СтавкаНДСДоставки);
			СтрокаПозицииЧека = МенеджерОборудованияКлиентСервер.ПараметрыФискальнойСтрокиЧека(); 
...
Процедура ДополнитьТоварамиПараметрыПриПробитииЧека(ДокументОбъект, ОбщиеПараметры) Экспорт
	...
	Для каждого ТекСтрока Из ТаблицаДокумента Цикл
		...
		Если ЗначениеЗаполнено(ДокументРасчетов) Тогда
			...
			РозничныеПродажиСервер.ДобавитьПозицииЧекаПоДокументуРасчетов(ДокументРасчетов, ОбщиеПараметры, ТекСтрока, СуммаПлатежа, НомерСтрокиТовара);
