...
Процедура ВыполнитьФормированиеПоДоходам(Организация, ПериодФормирования, Результат, МенеджерВТ) Экспорт
	...
	|	"""",
	|	"""",
	|	НАЧАЛОПЕРИОДА(ОтчетКомиссионера.Дата, ДЕНЬ) КАК ДатаНачалоДня,
	...
	|	"""",
	|	"""",
	|	НАЧАЛОПЕРИОДА(ОтчетКомитенту.Дата, ДЕНЬ) КАК ДатаНачалоДня,
	...
	Если ДокументФормирования <> Неопределено Тогда
		...
		Пока ВыборкаЗаписейУСН.Следующий() Цикл
			...
			ИначеЕсли ТипЗнч(ВыборкаЗаписейУСН.ПервичныйДокумент) = Тип("ДокументСсылка.ОтчетКомиссионера") Тогда
				НоваяСтрокаУСН.Содержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Получен отчет комиссионера ""%1""; вознаграждение комиссионера зачтено из выручки от реализации.'"), СокрЛП(ВыборкаЗаписейУСН.Контрагент));
			ИначеЕсли ТипЗнч(ВыборкаЗаписейУСН.ПервичныйДокумент) = Тип("ДокументСсылка.ОтчетКомитенту") Тогда
				НоваяСтрокаУСН.Содержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Оказание посреднических услуг в торговле (комиссионное вознаграждение); комитент ""%1"". Комиссионное вознаграждение удержано из выручки'"), СокрЛП(ВыборкаЗаписейУСН.Контрагент));
			Иначе
	...
	Если ДокументФормирования <> Неопределено Тогда
		...
		Для Каждого Строка Из РезультатЗапроса Цикл
			...
			Если ТипЗнч(НоваяСтрокаУСН.ПервичныйДокумент) = Тип("ДокументСсылка.ОтчетКомиссионера") Тогда
					НоваяСтрокаУСН.Содержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Получен отчет комиссионера ""%1""; вознаграждение комиссионера зачтено из выручки от реализации.'"), СокрЛП(Строка.Контрагент));
					...
					Если ЗначениеЗаполнено(Строка.СтавкаНДС) Тогда
						...
						Если ЗначениеСтавки > 0 Тогда
							...
							НоваяСтрокаУСН.Содержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Получен отчет комиссионера ""%1""; вознаграждение комиссионера зачтено из выручки от реализации. (НДС)'"), СокрЛП(Строка.Контрагент));
						КонецЕсли;
					КонецЕсли;
...
Функция ВыполнитьРасчетАвансовогоПлатежа(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено) Экспорт
	...
	ЗарегистрироватьВзаиморасчетыСБюджетом(СтруктураРасчетаАвансовогоПлатежа, ?(СтруктураРасчетаАвансовогоПлатежа.ВсегоКУплатеАП < НачисленоСНачалаГода, 0, СтруктураРасчетаАвансовогоПлатежа.ВсегоКУплатеАП - НачисленоСНачалаГода));
...
Функция СформироватьДекларациюПоУСН(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено) Экспорт
	...
	СтруктураДекларацииПоУСН.НалоговыйПериодПропущен = РегламентированнаяОтчетностьУСН.НалоговыйПериодПропущен(Организация,
...
Функция ПолучитьИсходнуюТаблицуДляФормированияДвиженийПоПартиям(Организация, ГраницаНачала, ГраницаВыполнения)
	...
	|	&ПустаяДата ДатаПервичногоДокумента,
	...
	|	""Поступление"",
	|	Закупки.ДатаДокумента КАК Дата,
	|	Закупки.ДатаДокумента КАК ДатаСортировки,
	|	Закупки.Ссылка КАК Регистратор,
	|	Закупки.Ссылка КАК ДокументПартии,
	|	Закупки.Ссылка КАК ДенежныйДокумент,
	|	Закупки.ДатаДокумента КАК ДатаДенежногоДокумента,
	|	&Организация КАК Организация,
	|	Закупки.Номенклатура КАК Номенклатура,
	|	СУММА(Закупки.Количество) КАК Количество,
	|	СУММА(Закупки.Сумма) КАК Сумма,
	|	СУММА(Закупки.СуммаНДС) КАК СуммаНДС,
	|	Закупки.ДатаДокумента ДатаПервичногоДокумента,
	|	Закупки.НомерДокумента НомерПервичногоДокумента
	|ИЗ
...
Процедура ОбработатьСтрокуДвиженияВозвратаОтПокупателя(ТаблицаПартионногоУчета,ОбщийМассивДвижений,ТаблицаДвижений, СтрокаДвижения, ТаблицаЗаказов)
	...
	Если СтрокиЗаказа.Количество() = 0 Или ТаблицаДвиженийДокументаРеализации = Неопределено Или СтрокиНоменклатурыВозврата.Количество() = 0 Тогда
		Для к = 0 По ОбщийМассивДвижений.ВГраница() Цикл
...
Процедура ВыполнитьРасчетЕдиногоНалогаПредварительный(Организация, ПериодРасчета, ДатаДокументаОбработкиСобытия, ЗаписьСобытия) Экспорт
	...
	СуммаЕдиногоНалога = НалоговаяБазаЕН *СтавкаНалогаЕН/100;
