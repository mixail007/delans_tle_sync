...
Процедура ЗаменитьСтрокуДаннымиНабора(Объект, ИмяТабличнойЧасти, СтрокаТабличнойЧасти, СтруктураДанные) Экспорт
	...
	ЭтоЗаказНаряд = (ТипЗнч(Объект.Ссылка)=Тип("ДокументСсылка.ЗаказПокупателя") И Объект.ВидОперации=ПредопределенноеЗначение("Перечисление.ВидыОперацийЗаказПокупателя.ЗаказНаряд"));
	...
	ИндексСтроки = Объект[ИмяТабличнойЧасти].Индекс(СтрокаТабличнойЧасти);
	...
	Если ДобавленныеСтроки.Количество()=0 Тогда
		...
		Если СтруктураДанные.СоставНабора.Количество()=0 Тогда
			#Если Клиент Тогда
			    ПоказатьПредупреждение(, НСтр("ru = 'Не заполнен состав набора'")); 	
			#КонецЕсли 
			Возврат;
		КонецЕсли; 
		...
		Для каждого ОписаниеСостава Из СтруктураДанные.СоставНабора Цикл
			...
			Если НайденныеСтроки.Количество()>0 Тогда
				...
				Если ЕстьКратность Тогда
					Если ОписаниеСостава.Свойство("ФиксированнаяСтоимость") И ОписаниеСостава.ФиксированнаяСтоимость Тогда
						НоваяСтрока.Кратность = НоваяСтрока.Кратность+ОписаниеСостава.Количество*?(СтрокаТабличнойЧасти.Количество=0, 1, СтрокаТабличнойЧасти.Количество);
					Иначе
						НоваяСтрока.Кратность = НоваяСтрока.Кратность+?(СтрокаТабличнойЧасти.Количество=0, 1, СтрокаТабличнойЧасти.Количество);
					КонецЕсли; 
				КонецЕсли; 
			Иначе
				НоваяСтрока = Объект[ИмяТабличнойЧасти].Вставить(ИндексСтроки);
				...
				Если ЕстьКратность Тогда
					НоваяСтрока.Коэффициент = 1;
					УправлениеНебольшойФирмойКлиентСервер.ЗаполнитьКлючСвязи(Объект[ИмяТабличнойЧасти], НоваяСтрока, "КлючСвязиДляСкидокНаценок");
				КонецЕсли; 
			...
			Если ЕстьКолонка(ТабличнаяЧасть, НоваяСтрока, "СтруктурнаяЕдиница") Тогда
				Если ЗначениеЗаполнено(ОписаниеСостава.Склад) И ПоложениеСклада=ПредопределенноеЗначение("Перечисление.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти") Тогда
	...
	Иначе
		...
		НаборыКлиентСервер.ДобавитьЭкземплярНабора(
		...
		СтрокиНабора = Объект[ИмяТабличнойЧасти].НайтиСтроки(СтруктураОтбора);
	...
	Объект[ИмяТабличнойЧасти].Удалить(СтрокаТабличнойЧасти);
