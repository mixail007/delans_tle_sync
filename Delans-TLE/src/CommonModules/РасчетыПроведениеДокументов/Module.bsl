...
Процедура СформироватьТаблицаОплатаДокументовНакладные(ДокументСсылка, СтруктураДополнительныеСвойства, МассивДоступныхОпераций) Экспорт
	...
	Если МассивДоступныхОпераций <> Неопределено
		...
		Возврат;
	КонецЕсли;
	Если НЕ ДокументСсылка.Контрагент.ВестиРасчетыПоДокументам Тогда
		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаОплатаДокументов", Новый ТаблицаЗначений);
		Возврат;
	КонецЕсли;
	
	Если СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Свойство("ТаблицаРасчетыСПокупателями")
	...
	Для Каждого ТекущаяСтрока Из ТекущаяТаблицаРасчетов Цикл
		...
		Если ТекущаяСтрока.ВидДвижения = ВидДвиженияНакопления.Расход И ТекущаяСтрока.ТипРасчетов = Перечисления.ТипыРасчетов.Долг Тогда
			...
			Иначе
				НоваяСтрока.СуммаАванса = ТекущаяСтрока.СуммаВал * ?(СтруктураЗначения.Курс = 0, 1, СтруктураЗначения.Курс) /
					?(СтруктураЗначения.Кратность = 0, 1, СтруктураЗначения.Кратность)
				;
			КонецЕсли;
		...
		ИначеЕсли ТекущаяСтрока.ВидДвижения = ВидДвиженияНакопления.Приход И ТекущаяСтрока.ТипРасчетов = Перечисления.ТипыРасчетов.Долг Тогда
			...
			Иначе
				НоваяСтрока.Сумма = ТекущаяСтрока.СуммаВал * ?(СтруктураЗначения.Курс = 0, 1, СтруктураЗначения.Курс) /
					?(СтруктураЗначения.Кратность = 0, 1, СтруктураЗначения.Кратность)
				;
			КонецЕсли;
...
Процедура СформироватьДвиженияПоВзаиморасчетамОплатаВручную(ДокументОбъект, СтруктураДополнительныеСвойства)
	...
	|			ТОГДА РасшифровкаПлатежа.СуммаРег
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаРег,
...
Процедура ОпределитьЗаказ(ДополнительныеСвойства, НоваяСтрокаПредоплата);
	...
	// 2*. 01.01. Задним числом зачитывается оплата в заказ-наряде. Заказ наряд нужно указывать в колонке "Заказ" и в колонке "Документ".
	// В РН вызывается Помощник и там отмечается оплата.
...
Процедура ИнициализироватьДополнительныеСвойстваДляПроведения(Документ, СтруктураДополнительныеСвойства, Отказ, ЭтоРасчетыСПоставщиком, СоответствиеТабличныхЧастейИРеквизитаЗаказ = Неопределено, СоответствиеВременныхТаблицИРеквизитаЗаказ = Неопределено) Экспорт
	
	СтруктураДополнительныеСвойства.Вставить("Отказ", Отказ);
	ПараметрыДокумента = Новый Структура("ЭтоЗаказ,ЭтоОплата,ЭтоОтгрузка,ЭтоРасчетыСПоставщиком,ЭтоЗаказНаряд,ЭтоКорректировка,ЭтоВозвратОплаты");
	...
	Если ТипЗнч(Документ.Ссылка) = Тип("ДокументСсылка.РасходИзКассы") Тогда
		ЭтоОплата 		 	   = Документ.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Поставщику;
	...
	ИначеЕсли ТипЗнч(Документ.Ссылка) = Тип("ДокументСсылка.РасходСоСчета") Тогда
		ЭтоОплата 		 	   = Документ.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Поставщику;
	...
	ИначеЕсли ТипЗнч(Документ.Ссылка) = Тип("ДокументСсылка.ЧекККМ") Тогда
		Если Документ.Ссылка.ОперацияСДенежнымиСредствами Тогда
			ЭтоОплата = Истина;
		КонецЕсли;
		ЭтоЧек = Истина;
	ИначеЕсли ТипЗнч(Документ.Ссылка) = Тип("ДокументСсылка.ЧекККМВозврат") Тогда
		Если Документ.Ссылка.ОперацияСДенежнымиСредствами > 0 Тогда
	...
	Если ПараметрыДокумента.ЭтоОтгрузка Тогда
		...
		Иначе
			СтруктураДополнительныеСвойства.Вставить("СпособЗачетаПредоплаты", Перечисления.СпособыЗачетаИРаспределенияПлатежей.Вручную);
		КонецЕсли;
		
		СтруктураДополнительныеСвойства.Вставить("СоответствиеТабличныхЧастейИРеквизитаЗаказ", СоответствиеТабличныхЧастейИРеквизитаЗаказ);
		...
		Если СоответствиеВременныхТаблицИРеквизитаЗаказ <> Неопределено Тогда
			СтруктураДополнительныеСвойства.Вставить("СоответствиеВременныхТаблицИРеквизитаЗаказ", СоответствиеВременныхТаблицИРеквизитаЗаказ);
		КонецЕсли;
...
Процедура ДобавитьКолонкиВТаблицуРасчетов(ТаблицаДвиженийПоВзаиморасчетам)
	...
	ТаблицаДвиженийПоВзаиморасчетам.Колонки.Добавить("ЭтоОплатаЗаказа", Новый ОписаниеТипов("Булево"));
...
Процедура СформироватьДвижениеРаспределенияАвтоАванса(ДокументОбъект, СтруктураДополнительныеСвойства, СуммаРаспределения, СтрокаДолги, СтрокаРасшифровки)
	...
	Движение.СуммаНДС = СтрокаРасшифровки.СуммаНДС; ///????
...
Процедура СформироватьДвижениеЗачетаАванса(ДокументОбъект, СтрокаКЗачету, СтрокаАванса, СтруктураДополнительныеСвойства)
	...
	Иначе //В национальной валюте
		...
		//В расчетах с поставщиками пока не используется данный ресурс
		Если ПараметрыДокумента.ЭтоРасчетыСПоставщиком Тогда
			Движение.СуммаРег = 0;
		КонецЕсли;
...
Процедура ДобавитьСтрокуЗадолженности(ДокументОбъект, СтрокаКЗачету, ДополнительныеСвойства)
	...
	Если НЕ ДополнительныеСвойства.УчетВалютныхОпераций Тогда
		Движение.СуммаРег = 0;
	КонецЕсли;
