...
// Формирует параметры, которые использются в функции ЗаполнитьЗаписиСкладскогоЖурнала.
// 
...
// Определеяет предполаегаемый документ-основание, на основании данных по связанным документам.
//	Сопостовляет данные из ТЗ "СвязанныеДокументы" и ВТ "ДокументыИнформационнойБазы" (см.  ИнтеграцияВЕТИСПереопределяемый.ЗаполнитьВременнуюТаблицуСвязанныхДокументовИнформационнойБазы)
...
Процедура ЗаполнитьЗаписиСкладскогоЖурналаПоСтрокеДокумента(СтрокаДокумента, СтруктураПоиска, ПараметрыЗаполнения, ОсталосьЗаполнить, РезультатЗаполнения)
	...
	Для каждого СтрокаОстатки Из ТаблицаОстатки Цикл
		...
		Если НЕ ПараметрыЗаполнения.ЭтоИнвентаризация
			...
			// 1. Подобор записи на указанное количество.
