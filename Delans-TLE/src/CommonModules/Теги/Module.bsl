...
Процедура ПрочитатьДанныеТегов(Форма, Объект)
	...
	ИмяСправочника = Строка(Объект.Ссылка.Метаданные().Имя);
...
Функция ЗаписатьДанныеОбщихТегов(Форма) Экспорт
	
	НачатьТранзакцию();
	
	Попытка
		
		Для Каждого ВыделенныйОбъект Из Форма.ВыделенныеОбъекты Цикл
			
			ВладелецОбъект = ВыделенныйОбъект.Объект.ПолучитьОбъект();
			
			Для Каждого Тег Из Форма.УдаляемыеТеги Цикл
			...
			Для Каждого Тег Из Форма.ДанныеТегов Цикл
