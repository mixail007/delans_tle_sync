...
Процедура ПередНачаломРаботыКлиента(ТребуетсяОбновлениеИнтерфейса = Ложь) Экспорт
	...
	Если НЕ ИспользуетсяСервисТарификации()
		...
		Если ЭтоМобильноеПриложение()
			ИЛИ ЭтоМобильноеПриложение20() Тогда
			УстановитьЭтоМобильноеПриложение(Ложь);
			УстановитьЭтоМобильноеПриложение20(Ложь);
			// Снимаем ограничения
	...
	Попытка
		...
		Если ЕстьАктивныйТариф <> ЭтоМобильноеПриложение() Тогда
			...
			КонецЦикла;
		КонецЕсли;
		ЕстьАктивныйТариф20 = ЕстьАктивныйТарифМобильногоУНФ20(СписокТарифиов);
		Если ЕстьАктивныйТариф20 <> ЭтоМобильноеПриложение20() Тогда
			УстановитьЭтоМобильноеПриложение20(ЕстьАктивныйТариф);
			// Устанавливаем/снимаем ограничения
			Если ЕстьАктивныйТариф20 Тогда
				УстановитьМинимальныйИнтерфейс(МассивПользователей);
			Иначе
				УстановитьОбычныйИнтерфейс(МассивПользователей);
			КонецЕсли;
...
Функция КодТарифаМобильногоУНФ20()
	
	Возврат "sbm_mobile_20";
...
Функция ЕстьАктивныйТарифМобильногоУНФ(Знач ОтветСтрокой)
	...
	Для Каждого ТекущийТариф Из АктивныеТарифы Цикл
		...
		Если ТипЗнч(ИдентификаторТарифа) = Тип("Строка") 
			И ИдентификаторТарифа = КодТарифаМобильногоУНФ()
			И НЕ ЕстьПланыОбменаСинхронизацияМП() Тогда
				Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
Функция ЕстьАктивныйТарифМобильногоУНФ20(Знач ОтветСтрокой)
	...
	Для Каждого ТекущийТариф Из АктивныеТарифы Цикл
		...
		ИдентификаторТарифа = Неопределено;
		ТекущийТариф.Свойство("ID", ИдентификаторТарифа);
		Если ТипЗнч(ИдентификаторТарифа) = Тип("Строка") Тогда
			Если ИдентификаторТарифа = КодТарифаМобильногоУНФ20() Тогда
				Возврат Истина;
			КонецЕсли;
			Если ИдентификаторТарифа = КодТарифаМобильногоУНФ()
				И ЕстьПланыОбменаСинхронизацияМП() Тогда
				Возврат Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ЕстьПланыОбменаСинхронизацияМП()
	
	ВыборкаПлановОбмена = ПланыОбмена.СинхронизацияМП.Выбрать();
...
Функция УстановитьЭтоМобильноеПриложение(Знач Значение)
	...
	Константы.ЭтоМобильноеПриложение20.Установить(НЕ Значение);
	
КонецФункции

Функция УстановитьЭтоМобильноеПриложение20(Знач Значение)
	
	УстановитьПривилегированныйРежим(Истина);
	Константы.ЭтоМобильноеПриложение20.Установить(Значение);
	Константы.ЭтоОбычноеПриложение.Установить(НЕ Значение);
	Константы.ЭтоМобильноеПриложение.Установить(НЕ Значение);
