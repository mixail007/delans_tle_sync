...
Функция ИдентификаторПользователя(Пользователь) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр("ОбсужденияСервер.ИдентификаторПользователя", "Пользователь", Пользователь, Тип("СправочникСсылка.Пользователи"));
...
//
Функция ПолучитьПоследнееСообщение(ОбсуждениеИдентификатор, Пользователь, После = Неопределено) Экспорт
	...
	СообщениеУказанногоАвтора = Неопределено;
	...
	Для каждого Сообщение Из Сообщения Цикл
		Если Сообщение.Автор = Автор Тогда
			Возврат Сообщение;
		КонецЕсли;
	КонецЦикла;
	#КонецЕсли
	...
	Если НЕ ПустаяСтрока(Сообщение) Тогда
		...
		Если ИнформационнаяБазаЗарегистрирована Тогда
			Если Источник.ДополнительныеСвойства.Свойство("Модифицированность") Тогда // Запись из формы фоновым заданием в ПослеЗаписи
				МоментВремени = МестноеВремя(ТекущаяУниверсальнаяДата(), РаботаВМоделиСервиса.ПолучитьЧасовойПоясОбластиДанных(ОбщегоНазначения.ЗначениеРазделителяСеанса()));
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект) Экспорт
	...
	ФоновоеЗадание = ФоновыеЗадания.Выполнить("ОбсужденияСервер.ПопытатьсяСоздатьСообщениеСистемыВзаимодействия",
...
Процедура ЗаписатьОтложенныеСообщенияВСистемуВзаимодействия() Экспорт
	...
	Если ВыборкаОбъект.Количество()=0 Тогда
	...
	Пока ВыборкаОбъект.Следующий() Цикл
		...
		Попытка
			Если ТипЗнч(ВыборкаОбъект.Ссылка) = Тип("Строка") Тогда
				Обсуждение = ПолучитьОбсуждение(ВыборкаОбъект.Ссылка);
			Иначе
				Обсуждение = ПолучитьКонтекстноеОбсуждение(ВыборкаОбъект.Ссылка);
			КонецЕсли;
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			ЗаписатьОшибкуВЖР(
				НСтр("ru = 'Получение сообщений системы взаимодействия'",ОбщегоНазначения.КодОсновногоЯзыка()),
				ТекстОшибки, ВыборкаОбъект.Ссылка);
		...
		НужноСнятьВернутьНаблюдение = Пользователи.АвторизованныйПользователь() = ВыборкаОбъект.Пользователь;
		...
		ВыборкаЗапись = ВыборкаОбъект.Выбрать();
		...
		Если НЕ ЕстьОшибки Тогда
			...
			НаборЗаписейОбъект.Отбор.Объект.Установить(ВыборкаОбъект.Ссылка);
			Попытка
...
Процедура ПопытатьсяСоздатьСообщениеСистемыВзаимодействия(ДанныеСообщения) Экспорт
	...
	Попытка
		Если ТипЗнч(ДанныеСообщения.Объект) = Тип("ИдентификаторОбсужденияСистемыВзаимодействия") Тогда
