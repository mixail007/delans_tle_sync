...
Функция ЗапросСпискаУсловийПеремещенияГрузаПоПравиламРегионализации(GUIDЗаболеванияУсловийПеремещения, НомерСтраницы, КоличествоЭлементовНаСтранице)
	...
	Если ЗначениеЗаполнено(GUIDЗаболеванияУсловийПеремещения) Тогда
		...
		Запрос.disease = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос, "disease");
	...
	Если ПараметрыОтбора <> Неопределено Тогда
		...
		Если ПараметрыОтбора.Свойство("ДанныеАдреса")
			...
			Если    (ДанныеАдреса.Свойство("РегионGUID")          И ЗначениеЗаполнено(ДанныеАдреса.РегионGUID))
				...
				Запрос.r13nZone = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос, "r13nZone");
				
				Если ДанныеАдреса.Свойство("СтранаGUID")
					И ЗначениеЗаполнено(ДанныеАдреса.СтранаGUID) Тогда
					Запрос.r13nZone.country = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос.r13nZone, "country");
				...
				Если ДанныеАдреса.Свойство("РегионGUID")
					...
					Запрос.r13nZone.region = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос.r13nZone, "region");
				...
				Если ДанныеАдреса.Свойство("РайонGUID")
					...
					Запрос.r13nZone.district = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос.r13nZone, "district");
				...
				Если ДанныеАдреса.Свойство("НаселенныйПунктGUID")
					...
					Запрос.r13nZone.locality = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос.r13nZone, "locality");
