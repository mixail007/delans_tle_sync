...
Процедура Подключаемый_СправочникКонтрагентыТегОтборНажатие(Элемент, СтандартнаяОбработка)
	...
	Пометка = КлассификацияКонтактовКлиент.ТегОтборНажатие(ЭтотОбъект, "СписокКонтрагенты", Элемент, "СправочникКонтрагенты", СтандартнаяОбработка);
...
Процедура Подключаемый_СправочникКонтактыТегОтборНажатие(Элемент, СтандартнаяОбработка)
	...
	Пометка = КлассификацияКонтактовКлиент.ТегОтборНажатие(ЭтотОбъект, "СписокКонтакты", Элемент, "СправочникКонтакты", СтандартнаяОбработка);
	Если НЕ Пометка = Неопределено Тогда
		ИзменитьЦветЭлементаСервер(Пометка, Элемент.Имя);
	КонецЕсли;
...
Процедура Подключаемый_СправочникЛидыТегОтборНажатие(Элемент, СтандартнаяОбработка)
	...
	Пометка = КлассификацияКонтактовКлиент.ТегОтборНажатие(ЭтотОбъект, "СписокЛиды", Элемент, "СправочникЛиды", СтандартнаяОбработка);
	Если НЕ Пометка = Неопределено Тогда
		ИзменитьЦветЭлементаСервер(Пометка, Элемент.Имя);
	КонецЕсли;
...
Процедура ДобавитьВыбранныхЛидов(знач Лиды)
	...
	|	ЛидыКонтактнаяИнформация.ИдентификаторСтрокиТабличнойЧасти КАК ИдентификаторСтрокиТабличнойЧасти
	|ПОМЕСТИТЬ втКИЛидов
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЛидыКонтакты.Ссылка КАК Лид,
	|	ВЫРАЗИТЬ(ЛидыКонтакты.Наименование КАК СТРОКА(1000)) КАК КонтактПредставление,
	...
	|		ПО ЛидыКонтакты.Ссылка = втКИЛидов.Ссылка
	|			И ЛидыКонтакты.ИдентификаторСтрокиТабличнойЧасти = втКИЛидов.ИдентификаторСтрокиТабличнойЧасти
	|ГДЕ
	|	ЛидыКонтакты.Ссылка В ИЕРАРХИИ(&Лиды)
	|
	|УПОРЯДОЧИТЬ ПО
	|	КонтактПредставление,
	|	втКИЛидов.Порядок
	...
	ВыборкаКонтактов = МассивРезультатов[2].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	...
	Пока ВыборкаЛидов.Следующий() Цикл
		...
		Пока ВыборкаКонтактов.НайтиСледующий(Отбор) Цикл
			
			СтрокаВторойУровень = СтрокиВторойУровень.Добавить();
			СтрокаВторойУровень.Пометка = Истина;
			СтрокаВторойУровень.Значение = ВыборкаКонтактов.Лид;
			СтрокаВторойУровень.ПредставлениеЗначения = ВыборкаКонтактов.КонтактПредставление;
			СтрокаВторойУровень.ИндексКартинки = 2;
			...
			Пока ВыборкаКИ_Контактов.Следующий() Цикл
				...
				СтрокаТретийУровень = СтрокиТретийУровень.Добавить();
				СтрокаТретийУровень.Пометка = Истина;
				СтрокаТретийУровень.Значение = ВыборкаКИ_Контактов.ВидКИ;
				СтрокаТретийУровень.ПредставлениеЗначения = ВыборкаКИ_Контактов.ЗначениеКИ;
				СтрокаТретийУровень.ИндексКартинки = ВыборкаКИ_Контактов.ИндексКартинки;
...
Функция ПреобразоватьДеревоАдресатовВТаблицуЗначений(Дерево)
	...
	ТаблицаАдресатов.Колонки.Добавить("Контакт", Новый ОписаниеТипов("СправочникСсылка.Контрагенты,СправочникСсылка.КонтактныеЛица,СправочникСсылка.Лиды,Строка"));
