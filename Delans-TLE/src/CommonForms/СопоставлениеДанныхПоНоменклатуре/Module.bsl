...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(ЭлектронныйДокумент) Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, , "ЭлектронныйДокумент", , Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НеОткрыватьФормуПриОтсутствииНеСопоставленнойНоменклатуры
		...
		Если ЗначениеЗаполнено(ЭлектронныйДокументСсылка) Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Номенклатура уже сопоставлена.'"), ЭлектронныйДокументСсылка);
		КонецЕсли;
...
Процедура СохранитьДанныеКаталога(СсылкаНаЭД)
	...
	Если ТипЗнч(СсылкаНаЭД) = Тип("ДокументСсылка.ЭлектронныйДокументВходящий")
		...
		ДополнительныеПараметры.ИдентификаторФормы = ПрисоединенныйФайл.УникальныйИдентификатор();
		ДополнительныеПараметры.ПолучатьСсылкуНаДвоичныеДанные = Истина;
		
		ДопИнформацияПоЭД = РаботаСФайлами.ДанныеФайла(ПрисоединенныйФайл, ДополнительныеПараметры);
	...
	Если ДопИнформацияПоЭД.Свойство("СсылкаНаДвоичныеДанныеФайла")
		...
		Если Не ЭтоОднократнаяСделка Тогда
			...
			Если ВыборкаЭДДопДанных.Следующий() Тогда
				...
				ДополнительныеПараметры = РаботаСФайламиКлиентСервер.ПараметрыДанныхФайла();
				ДополнительныеПараметры.ИдентификаторФормы = ВыборкаЭДДопДанных.Ссылка.УникальныйИдентификатор();
				ДополнительныеПараметры.ПолучатьСсылкуНаДвоичныеДанные = Истина;
				
				ДопДанныеЭД = РаботаСФайлами.ДанныеФайла(ВыборкаЭДДопДанных.Ссылка, ДополнительныеПараметры);
