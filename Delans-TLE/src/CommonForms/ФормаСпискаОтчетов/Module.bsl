...
Процедура ОбновитьЭлементыНаФорме(УдалятьРеквизиты)
	...
	Для Каждого Стр Из СписокОтчетов Цикл
		...
		Если ПараметрыФормы.ЭтоМобильныйКлиент Тогда
			Продолжить;
		КонецЕсли;
...
Функция НоваяГруппаДляДобавления(Знач Колонка)
	...
	Если Колонка = 0 Тогда
		...
		ГруппаГоризонтальная.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяЕслиВозможно;
...
Процедура ОбновитьПараметрыФормы()
	...
	СтруктураПараметров.Вставить("ЭтоМобильныйКлиент", ОбщегоНазначения.ЭтоМобильныйКлиент());
...
Процедура ОбновитьОтображениеОтметокФильтров()
	...
	ИначеЕсли ПараметрыФормы.ВариантИнтерфейса=ВариантИнтерфейсаКлиентскогоПриложения.Такси Тогда
...
Процедура ОткрытьСлужебныйОтчет(Вариант)
	...
	Если НЕ ЗначениеЗаполнено(ДанныеОтчета.КлючВарианта) ИЛИ НЕ ЗначениеЗаполнено(ДанныеОтчета.ИмяОтчета) Тогда
		// Для случая, когда отчет открыт из строки поиска и его описание не хранится в таблице следует получить данные отчета из БД
