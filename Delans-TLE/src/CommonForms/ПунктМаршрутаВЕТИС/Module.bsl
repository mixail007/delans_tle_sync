...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УказываетсяТранспортноеСредствоПредприятия = Параметры.УказываетсяТранспортноеСредствоПредприятия
	                                           И ИнтеграцияВЕТИСПереопределяемый.УказываетсяТранспортноеСредство();
...
Процедура ТранспортноеСредствоПриИзменении(Элемент)
	...
	Если ТипЗнч(РеквизитыТранспортногоСредства) = Тип("Структура") Тогда
		...
		КонецЕсли;
		Если РеквизитыТранспортногоСредства.Свойство("НомерАвтомобильногоПрицепа") Тогда
			НомерАвтомобильногоПрицепа = РеквизитыТранспортногоСредства.НомерАвтомобильногоПрицепа;
		КонецЕсли;
		Если РеквизитыТранспортногоСредства.Свойство("НомерАвтомобильногоКонтейнера") Тогда
			НомерАвтомобильногоПрицепа = РеквизитыТранспортногоСредства.НомерАвтомобильногоКонтейнера;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработатьПодтверждение(Отказ = Ложь)
	...
	Если (ОбязательностьНомераТранспортногоСредства <> Неопределено И ОбязательностьНомераТранспортногоСредства = Истина)
		...
		Если Не ПустаяСтрока(ПредставлениеНомераТранспортногоСредства) Тогда
			ПроверяемыеПоля.Вставить("НомерТранспортногоСредства",
				ПредставлениеНомераТранспортногоСредства);
		КонецЕсли;
...
Процедура НастроитьВидимостьДоступностьЭлементов()
	...
	Если ОбязательностьНомераТранспортногоСредства = Неопределено Тогда
		Элементы.НомерТранспортногоСредства.АвтоОтметкаНезаполненного = СПерегрузкой;
	Иначе 
		Элементы.НомерТранспортногоСредства.АвтоОтметкаНезаполненного = ОбязательностьНомераТранспортногоСредства;
	КонецЕсли;
...
Функция ЗначенияРеквизитовТранспортногоСредства(ТранспортноеСредство)
	...
	Возврат ИнтеграцияВЕТИСПереопределяемый.РеквизитыТранспортногоСредства(ТранспортноеСредство);
