...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ГуидПрофиляРМК = УправлениеДоступомПереопределяемый.ПрофильРабочееМестоКассира().УИД;
	...
	// Настройка доступность служебыных профилей
	Элементы.ДоступДокументыПланированияДенег.Доступность = НЕ ДоступДенежныеДокументы;
	МодифицироватьВидыДоступаПрофиляДеньгиПриЗагрузке();
	// Конец УНФ
...
Процедура ПрофилиПометкаПриИзменении(Элемент)
	...
	Если ИмяТекущегоПрофиля(ТекущиеДанные.Профиль) = "Деньги" Тогда
...
Процедура УстановитьУсловноеОформление()
	...
	ПрофильДеньги = Справочники.ПрофилиГруппДоступа.НайтиПоРеквизиту("ИдентификаторПоставляемыхДанных", УправлениеДоступомПереопределяемый.ПрофильДеньги().УИД);
	ПрофильПланированиеДенег = Справочники.ПрофилиГруппДоступа.НайтиПоРеквизиту("ИдентификаторПоставляемыхДанных", УправлениеДоступомПереопределяемый.ПрофильДобавлениеИзменениеПодсистемыПланированиеДенег().УИД);
...
Процедура ЗагрузитьДанные(ОтборПрофилейТолькоТекущегоПользователя)
	...
	Запрос.УстановитьПараметр("ИдентификаторПрофиляРедактированиеЦенДокументов",					УправлениеДоступомПереопределяемый.ПрофильРедактированиеЦенДокументов().УИД);
	Запрос.УстановитьПараметр("ИдентификаторПрофиляРедактированиеНоменклатуры", 					УправлениеДоступомПереопределяемый.ПрофильРедактированиеНоменклатуры().УИД);
	Запрос.УстановитьПараметр("ИдентификаторПрофиляВозвратыОтПокупателей", 	   						УправлениеДоступомПереопределяемый.ПрофильВозвратыОтПокупателей().УИД);
	Запрос.УстановитьПараметр("ИдентификаторПрофиляВозвратыПоставщикам", 	   						УправлениеДоступомПереопределяемый.ПрофильВозвратыПоставщикам().УИД);
	Запрос.УстановитьПараметр("ИдентификаторПрофиляСинхронизацияДанных", 	   						УправлениеДоступомПереопределяемый.ПрофильСинхронизацияДанных().УИД);
	Запрос.УстановитьПараметр("ИдентификаторПрофиляСебестоимостьВОтчетеПродажи",					УправлениеДоступомПереопределяемый.ПрофильСебестоимостьВОтчетеПродажи().УИД);
	Запрос.УстановитьПараметр("ИдентификаторПрофиляПроведениеДенежныхДокументовСверхРезерва", 		УправлениеДоступомПереопределяемый.ПрофильПроведениеДенежныхДокументовСверхРезерва().УИД);
	Запрос.УстановитьПараметр("ИдентификаторПрофиляРедактированиеРасходовПоИсточникам" , УправлениеДоступомПереопределяемый.ПрофильРедактированиеРасходовПоИсточникам().УИД);
	...
	РазрешитьВидимостьРасходовНаИсточникиПривлечения = РазрешитьВидимостьРасходовНаИсточникиПривлечения(Параметры.Пользователь);
	
	ПроведениеДенежныхДокументовСверхРезерва = РазрешитьПроведениеДенежныхДокументовСверхРезерва(Параметры.Пользователь);
	ДоступДенежныеДокументы 				 = ДоступДенежныеДокументы(Параметры.Пользователь);
	ДоступДокументыПланированияДенег 		 = ДоступДокументыПланированияДенег(Параметры.Пользователь);
...
Процедура ЗаписатьИзмененияНаСервере(Отказ)
	...
	// УНФ
	МодифицироватьВидыДоступаПрофиляДеньгиПередЗаписью();
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.Профиль = Справочники.ПрофилиГруппДоступа.Администратор Тогда
				...
				Иначе
					...
					КонецЕсли;
				КонецЕсли;
			ИначеЕсли Выборка.Профиль.ИдентификаторПоставляемыхДанных = УправлениеДоступомПереопределяемый.ПрофильВозвратыОтПокупателей().УИД Тогда
				...
				ИдентификаторПоставляемыхДанных = УправлениеДоступомПереопределяемый.ПрофильПродажи().УИД;
	...
	//УНФ
	МодифицироватьВидыДоступаПрофиляДеньгиПослеЗаписи();
	//УНФ
...
Процедура ПриИзмененииТекущегоПрофиля(Знач Форма, Знач ОбработкаНаКлиенте = Истина)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		КонецЕсли;
		Если ИмяТекущегоПрофиля = "Деньги" Тогда
...
&НаСервереБезКонтекста
Функция РазрешитьВидимостьРасходовНаИсточникиПривлечения(Пользователь)
...
&НаСервереБезКонтекста
Функция РазрешитьПроведениеДенежныхДокументовСверхРезерва(Пользователь)
...
&НаСервереБезКонтекста
Функция ДоступДенежныеДокументы(Пользователь)
	...
	Запрос.УстановитьПараметр("Пользователь", Пользователь);
	Запрос.УстановитьПараметр("ИдентификаторПрофиляДеньги", УправлениеДоступомПереопределяемый.ПрофильДеньги().УИД);
...
&НаСервереБезКонтекста
Функция ДоступДокументыПланированияДенег(Пользователь)
	...
	Запрос.УстановитьПараметр("Пользователь", Пользователь);
	Запрос.УстановитьПараметр("ИдентификаторПрофиляДеньги", УправлениеДоступомПереопределяемый.ПрофильДеньги().УИД);
	Запрос.УстановитьПараметр("ИдентификаторПрофиляДобавлениеИзменениеПодсистемыПланированиеДенег", УправлениеДоступомПереопределяемый.ПрофильДобавлениеИзменениеПодсистемыПланированиеДенег().УИД);
...
&НаСервере
Процедура МодифицироватьВидыДоступаПрофиляДеньгиПриЗагрузке()
...
&НаСервере
Процедура МодифицироватьВидыДоступаПрофиляДеньгиПередЗаписью()
	...
	ЗначенияДоступаПланированиеДенег = ЗначенияДоступа.НайтиСтроки(Новый Структура("ГруппаДоступа", ПрофильПланированиеДенег));
	Для Каждого СтрокаЗначенияДоступа Из ЗначенияДоступаПланированиеДенег Цикл
		ЗначенияДоступа.Удалить(СтрокаЗначенияДоступа);
	КонецЦикла;
...
&НаСервере
Процедура МодифицироватьВидыДоступаПрофиляДеньгиПослеЗаписи()
...
Функция ДобавитьДополнительныеПрофили(ВыбранныеПрофили)
	...
	ЕстьПрофильДеньги  = Ложь;
	ЕстьПрофильПродажи = Ложь;
	ЕстьПрофильЗакупки = Ложь;
	...
	Запрос.УстановитьПараметр("ИдентификаторПрофиляАдминистратор", УправлениеДоступомПереопределяемый.ПрофильАдминистратор().УИД);
	Запрос.УстановитьПараметр("ИдентификаторПрофиляПродажи", УправлениеДоступомПереопределяемый.ПрофильПродажи().УИД);
	Запрос.УстановитьПараметр("ИдентификаторПрофиляЗакупки", УправлениеДоступомПереопределяемый.ПрофильЗакупки().УИД);
	Запрос.УстановитьПараметр("ИдентификаторПрофиляДеньги",  УправлениеДоступомПереопределяемый.ПрофильДеньги().УИД);
	...
	Если Выборка.Следующий() Тогда
		...
		ЕстьПрофильЗакупки = Выборка.ЕстьПрофильЗакупки;
		ЕстьПрофильДеньги  = Выборка.ЕстьПрофильДеньги;
	...
	ИдентификаторПоставляемыхДанных = УправлениеДоступомПереопределяемый.ПрофильРедактированиеЦенДокументов().УИД;
	...
	ИдентификаторПоставляемыхДанных = УправлениеДоступомПереопределяемый.ПрофильРедактированиеНоменклатуры().УИД;
	...
	ИдентификаторПоставляемыхДанных = УправлениеДоступомПереопределяемый.ПрофильРедактированиеРасходовПоИсточникам().УИД;
	ПрофильРедактированиеРасходовНаИсточники = Справочники.ПрофилиГруппДоступа.НайтиПоРеквизиту("ИдентификаторПоставляемыхДанных", ИдентификаторПоставляемыхДанных);
	Если ЗначениеЗаполнено(ПрофильРедактированиеРасходовНаИсточники) Тогда
		НоваяСтрока = ВыбранныеПрофили.Добавить();
		НоваяСтрока.Профиль = ПрофильРедактированиеРасходовНаИсточники;
		НоваяСтрока.Пометка = ЕстьПрофильПродажи И РазрешитьВидимостьРасходовНаИсточникиПривлечения;
	КонецЕсли;
	
	ИдентификаторПоставляемыхДанных = УправлениеДоступомПереопределяемый.ПрофильВозвратыОтПокупателей().УИД;
	...
	ИдентификаторПоставляемыхДанных = УправлениеДоступомПереопределяемый.ПрофильВозвратыПоставщикам().УИД;
	...
	ИдентификаторПоставляемыхДанных = УправлениеДоступомПереопределяемый.ПрофильСебестоимостьВОтчетеПродажи().УИД;
	ПрофильСебестоимостьВОтчетеПродажи = Справочники.ПрофилиГруппДоступа.НайтиПоРеквизиту("ИдентификаторПоставляемыхДанных", ИдентификаторПоставляемыхДанных);
	Если ЗначениеЗаполнено(ПрофильСебестоимостьВОтчетеПродажи) Тогда
		НоваяСтрока = ВыбранныеПрофили.Добавить();
		НоваяСтрока.Профиль = ПрофильСебестоимостьВОтчетеПродажи;
		НоваяСтрока.Пометка = ЕстьПрофильПродажи И ЕстьПрофильЗакупки;
	КонецЕсли;
	
	ИдентификаторПоставляемыхДанных = УправлениеДоступомПереопределяемый.ПрофильПроведениеДенежныхДокументовСверхРезерва().УИД;
	ПрофильПроведениеДенежныхДокументовСверхРезерва = Справочники.ПрофилиГруппДоступа.НайтиПоРеквизиту("ИдентификаторПоставляемыхДанных", ИдентификаторПоставляемыхДанных);
	Если ЗначениеЗаполнено(ПроведениеДенежныхДокументовСверхРезерва) Тогда
		НоваяСтрока = ВыбранныеПрофили.Добавить();
		НоваяСтрока.Профиль = ПрофильПроведениеДенежныхДокументовСверхРезерва;
		НоваяСтрока.Пометка = ЕстьПрофильДеньги И ПроведениеДенежныхДокументовСверхРезерва;
	КонецЕсли;
	
КонецФункции

&НаКлиенте
Процедура ДоступДенежныеДокументыПриИзменении(Элемент)
...
&НаКлиенте
Процедура ДоступДокументыПланированияДенегПриИзменении(Элемент)
