...
Процедура ПериодПриИзмененииСервер()
	...
	Если НЕ ПустаяСтрока(НастройкиОтчета.ИдентификаторВарианта) И НЕ НастройкиОтчета.РежимПериода="БезПериода" Тогда
		ХранилищеСистемныхНастроек.Сохранить("ПериодОтчета", НастройкиОтчета.ИдентификаторВарианта + ?(НастройкиОтчета.Контекстный, "_Контекстный", ""), Период);
	КонецЕсли; 
