...
Процедура УправлениеДоступностьюЭлементовФормы()
	...
	УказанРеквизитТочногоСоответствия = УказанИдентификатор Или УказанGLN Или УказанНомер;
...
Процедура ПроверитьКорректностьЗаполненияРеквизитовПоиска(Отказ)
	...
	Если Не ПустаяСтрока(НомерПредприятия)
		И СтрДлина(НомерПредприятия) < 6 Тогда
		
		ТекстОшибки = НСтр("ru = 'Для поиска по номеру предприятия необходимо указать по крайней мере 5 символов'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, ,"НомерПредприятия",, Отказ);
