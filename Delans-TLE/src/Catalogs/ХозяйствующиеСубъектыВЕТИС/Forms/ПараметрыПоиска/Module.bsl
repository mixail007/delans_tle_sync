...
Процедура ВыполнитьПоиск(Команда)
	...
	Если ЗначениеЗаполнено(Идентификатор) Тогда
		...
		Исключение
			ТекстСообщения = НСтр("ru = 'Идентификатор должен быть указан в формате """"XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX""""'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ,"Идентификатор");
			Возврат;
		КонецПопытки;
...
Процедура ПроверитьКорректностьЗаполненияРеквизитовПоиска(Отказ)
	...
	Если Не ПустаяСтрока(Идентификатор)
		...
		ТекстОшибки = НСтр("ru = 'Идентификатор должен содержать 36 символов'");
