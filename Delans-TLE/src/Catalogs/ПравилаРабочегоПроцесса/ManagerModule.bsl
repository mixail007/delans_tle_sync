...
Процедура ЗаполнитьПоставляемыеПравила() Экспорт
	...
	Попытка
		...
		// 3. Действие "Отправить E-mail по ошибке печати онлайн-чеков"
		...
		// 4. Правило "Оповещение автора заказа покупателя при поступлении предоплаты по нему"
		Правило = Справочники.ПравилаРабочегоПроцесса.СоздатьЭлемент();
		...
		// 4. Напоминание о поступление предоплаты по заказу покупателя.
	Исключение
