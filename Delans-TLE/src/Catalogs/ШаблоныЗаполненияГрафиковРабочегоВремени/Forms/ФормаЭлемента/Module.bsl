...
Процедура УстановитьНастройкиТаблиц(ОткрытияФормы = Ложь)
	...
	Для Каждого СтрокаРасписания Из Объект.РасписаниеРаботы Цикл
		...
		Если Не ОткрытияФормы Тогда
			ПереназначитьНомерДняПерерывам(СтрокаРасписания.НомерДняЦикла, НомерДня);
		...
		Если ЗначениеЗаполнено(СтрокаРасписания.ВремяНачала) и ЗначениеЗаполнено(СтрокаРасписания.ВремяОкончания) Тогда
...
&НаКлиенте
Процедура ПереназначитьНомерДняПерерывам(НомерДняЦикла, НомерДня)
	
	ПараметрыОтбора = Новый Структура("НомерДня", НомерДняЦикла);
	
	ПереназначаемыеСтроки = Объект.Перерывы.НайтиСтроки(ПараметрыОтбора);
	
	Для Каждого СтрокаПерерыва из ПереназначаемыеСтроки Цикл
