...
Процедура НастроитьАвтоматически(Команда)
	...
	ДлительнаяОперация = ПодобратьНастройкиУчетнойЗаписиВФоне();
	
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ПриЗавершенииАвтоматическойНастройки", ЭтотОбъект);
	
	ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтотОбъект);
	ПараметрыОжидания.ТекстСообщения = НСтр("ru = 'Поиск настроек почтового сервера.'");
...
&НаСервере
Функция ПодобратьНастройкиУчетнойЗаписиВФоне()
	...
	ПараметрыПроцедуры.Вставить("АдресЭлектроннойПочты", Объект.АдресЭлектроннойПочты);
	ПараметрыПроцедуры.Вставить("Пароль", ?(ПарольИзменен, Пароль, Пароли.Пароль));
	ПараметрыПроцедуры.Вставить("ПарольSMTP", ?(ПарольSMTPИзменен, ПарольSMTP, Пароли.ПарольSMTP));
	ПараметрыПроцедуры.Вставить("ДляОтправки", Объект.ИспользоватьДляОтправки);
	ПараметрыПроцедуры.Вставить("ДляПолучения", Объект.ИспользоватьДляПолучения);
...
&НаКлиенте
Процедура ПриЗавершенииАвтоматическойНастройки(РезультатФоновогоЗадания, Параметры) Экспорт
	
	Если Не ЗначениеЗаполнено(РезультатФоновогоЗадания) Тогда
		Возврат;
	КонецЕсли;
	
	Если РезультатФоновогоЗадания.Статус = "Ошибка" Тогда
		ПоказатьПредупреждение(, РезультатФоновогоЗадания.ПодробноеПредставлениеОшибки);
		Возврат;
	КонецЕсли;
	
	НайденныеНастройки = ПолучитьИзВременногоХранилища(РезультатФоновогоЗадания.АдресРезультата);
	Если Не УдалосьОпределитьНастройки(НайденныеНастройки) Тогда
		ПоказатьПредупреждение(,
		НСтр("ru = 'Не удалось подобрать параметры подключения автоматически.
		|Удостоверьтесь, что адрес электронной почты и пароль указаны корректно.'"));
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Функция УдалосьОпределитьНастройки(Знач НайденныеНастройки)
	
	Если ТипЗнч(НайденныеНастройки) <> Тип("Структура") Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если НайденныеНастройки.Свойство("ДляОтправки")
		И НайденныеНастройки.Свойство("ДляПолучения")
		И НайденныеНастройки.ДляОтправки = Ложь
		И НайденныеНастройки.ДляПолучения = Ложь Тогда
		Возврат Ложь;
	КонецЕсли;
