...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
			РаботаСОтборами.НастроитьПанельОтборовМобильныйКлиент(ЭтотОбъект,,,"ОтборСостояние,ОтборРезультат,ОтборТеги,ОтборИсточник,ОтборОтветственный");
		КонецЕсли;
...
Процедура УстановитьОтборПоРезультату()
	...
	ЭлементыОтбора = ОбщегоНазначенияКлиентСервер.НайтиЭлементыИГруппыОтбора(Список.КомпоновщикНастроек.Настройки.Отбор, "ВариантЗавершения");
	ОтборНеУстановлен = ЭлементыОтбора.Количество() = 0 ИЛИ (ТипЗнч(ЭлементыОтбора[0].ПравоеЗначение) = Тип("Массив") И ЭлементыОтбора[0].ПравоеЗначение.Количество() = 0);
	УстановленОтборПоНекачественнымИПокупателю = ЭлементыОтбора.Количество() <> 0
		И (ТипЗнч(ЭлементыОтбора[0].ПравоеЗначение) = Тип("Массив") И ЭлементыОтбора[0].ПравоеЗначение.Количество() = 2);
		
	УстановленОтборПоПереведеннымВПокупателя = НЕ ОтборНеУстановлен И НЕ УстановленОтборПоНекачественнымИПокупателю И (ЭлементыОтбора.Количество() <> 0 И ТипЗнч(ЭлементыОтбора[0].ПравоеЗначение) = Тип("Массив") 
		И ЭлементыОтбора[0].ПравоеЗначение[0] = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Перечисления.ВариантЗавершенияРаботыСЛидом.ПереведенВПокупателя, "Ссылка"));
	УстановленОтборПоНекачественным = НЕ ОтборНеУстановлен И НЕ УстановленОтборПоНекачественнымИПокупателю И ЭлементыОтбора.Количество() <> 0 И ТипЗнч(ЭлементыОтбора[0].ПравоеЗначение) = Тип("Массив") 
		И ЭлементыОтбора[0].ПравоеЗначение[0] = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Перечисления.ВариантЗавершенияРаботыСЛидом.НекачественныйЛид, "Ссылка");
	
	Если ОтборНеУстановлен Тогда
		МассивОтбора = Новый Массив;
		МассивОтбора.Добавить(Перечисления.ВариантЗавершенияРаботыСЛидом.ПустаяСсылка());
		УправлениеНебольшойФирмойКлиентСервер.УстановитьЭлементОтбораСписка(Список, "ВариантЗавершения", МассивОтбора, Истина, ВидСравненияКомпоновкиДанных.ВСписке);
	КонецЕсли;
	
	Элементы.Контрагент.Видимость = УстановленОтборПоПереведеннымВПокупателя;
	Элементы.ВариантЗавершения.Видимость = УстановленОтборПоНекачественнымИПокупателю;
	Элементы.ПричинаНеуспешногоЗавершенияРаботы.Видимость = УстановленОтборПоНекачественным;
...
Функция ТекстЗапросаКанбан(Операция, НаправлениеСкролла, Количество = 14)
	...
	Если Операция = "Скролл" И НаправлениеСкролла = "СкроллНазад" Тогда
		...
		|	Лиды.Контакты.(
		|		Наименование КАК НаименованиеКонтакта,
...
Функция ПредставлениеЛидаКанбан(Строка)
	...
	КонецЕсли;
	Контакты = Строка.Контакты.Выгрузить();
	...
	Для Каждого Контакт Из Контакты Цикл
		Если ЗначениеЗаполнено(Контакт.НаименованиеКонтакта) И Контакт.НаименованиеКонтакта <> Строка.Наименование Тогда
			МассивСтрок.Добавить(Контакт.НаименованиеКонтакта);
				
		КонецЕсли;
		//Заполнение таблицы для панели контактной информации
		Если ТекущийКонтакт <> Контакт.НаименованиеКонтакта Тогда
			НоваяСтрока = ДанныеПанелиКИКанбан.Добавить();
			НоваяСтрока.Отображение = Контакт.НаименованиеКонтакта;
			НоваяСтрока.ИндексПиктограммы = -1;
			НоваяСтрока.ТипОтображаемыхДанных = "КонтактноеЛицо";
			НоваяСтрока.ВладелецКИ = Контакт.НаименованиеКонтакта;
			НоваяСтрока.СсылкаНаЛид = Строка.Лид;
			ТекущийКонтакт = Контакт.НаименованиеКонтакта;
		КонецЕсли;
		Для Каждого СтрокаКИ Из КИ Цикл
			Если СтрокаКИ.ИдентификаторСтрокиТабличнойЧасти <> Контакт.ИдентификаторСтрокиТабличнойЧасти Тогда
				Продолжить;
			КонецЕсли;
