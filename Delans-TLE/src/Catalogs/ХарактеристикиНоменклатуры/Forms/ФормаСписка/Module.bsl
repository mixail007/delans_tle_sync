...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДоступноРедактированиеНоменклатуры = ПравоДоступа("Изменение", Метаданные.Справочники.Номенклатура, ПараметрыСеанса.ТекущийПользователь);
...
Процедура УправлениеФормой(Форма)
	...
	ЗапретРедактирования = ((Не ЗначениеЗаполнено(Форма.Номенклатура) и Не ЗначениеЗаполнено(Форма.КатегорияНоменклатуры)) Или Не Форма.ДоступноРедактированиеНоменклатуры);
	...
	Форма.ТолькоПросмотр = ЗапретРедактирования;
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаИспользоватьКакОсновную", 				"Доступность", 		Форма.ИспользоватьХарактеристики И НЕ ЗапретРедактирования И НЕ Форма.ЭтоНабор);
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПолеИспользоватьХарактеристики", 			"Доступность", 		НЕ ЗапретРедактирования);
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПолеПроверятьЗаполнениеХарактеристики", 	"Доступность", 		Форма.ИспользоватьХарактеристики И НЕ ЗапретРедактирования);
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Режим", 									"Видимость", 		НЕ ЗапретРедактирования И НЕ ЭтоКатегория);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаКопирование", 						"Доступность", 		Форма.ИспользоватьХарактеристики И НЕ Форма.СкрытьРеквизиты И НЕ ЗапретРедактирования);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаЗагрузкаДанныхИзВнешнегоИсточника", 	"Доступность", 		Форма.ИспользоватьХарактеристики И НЕ Форма.СкрытьРеквизиты И НЕ ЗапретРедактирования);
...
Процедура ЗагрузкаДанныхИзВнешнегоИсточникаОбработкаРезультата(РезультатЗагрузки, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(РезультатЗагрузки) = Тип("Структура") Тогда
		...
		ИначеЕсли РезультатЗагрузки.ОписаниеДействия = "ОбработатьПодготовленныеДанные" Тогда
			...
			ОбработатьПодготовленныеДанные(РезультатЗагрузки, ДополнительныеПараметры);
...
&НаСервере
Процедура ОбработатьПодготовленныеДанные(РезультатЗагрузки, ДополнительныеПараметры)
	...
	Для каждого СтрокаТаблицы Из РезультатЗагрузки.ТаблицаСопоставленияДанных Цикл
		...
		Если ЗагрузкаВПриложениеВозможна И СогласованноеСостояниеСтроки Тогда
			...
			Если СтрокаТаблицы.ПрименитьЦеныНоменклатуры Тогда
				
				ДеталиЗаписи = Новый Структура; 
				ДеталиЗаписи.Вставить("Период",			ТекущаяДатаСеанса());
				ДеталиЗаписи.Вставить("Номенклатура",	ЭлементСправочника.Владелец);
				ДеталиЗаписи.Вставить("Характеристика", ЭлементСправочника.Ссылка);
				ДеталиЗаписи.Вставить("Автор", 			ТекущийПользователь);
