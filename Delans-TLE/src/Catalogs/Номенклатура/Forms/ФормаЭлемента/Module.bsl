...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого ЗначениеСоответствия Из СоответствиеСтавок Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Элементы.ДекорацияСтавкиНДСПодсказка.Видимость = Элементы.ВидСтавкиНДС.Видимость И (Объект.ВидСтавкиНДС = ПредопределенноеЗначение("Перечисление.ВидыСтавокНДС.Общая")
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Запись_Файл" И Объект.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Запас") Тогда
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	// ЭлектронноеВзаимодействие.РаботаСНоменклатурой
	Если РаботаСНоменклатурой.ПравоИзмененияДанных(Ложь) Тогда
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если НЕ ЭтоФормаНовогоЭлемента Тогда
...
Процедура НаименованиеПриИзменении(Элемент)
	...
	Если ДоступенСервисРаботаСНоменклатурой Тогда
		// ЭлектронноеВзаимодействие.РаботаСНоменклатурой
...
&НаКлиенте
Процедура ЕдиницаИзмеренияExtendedTooltipОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
Процедура УправлениеФормой()
	...
	Элементы.Поставщик.Видимость = ЭтоЗапас() Или ЭтоУслуга() И НЕ Объект.ЭтоНабор;
	...
	КолонкаИзображенияЦеныПустая = НЕ Элементы.ПереключениеИзображений.Видимость И НЕ Элементы.АктуальныеЦены.Видимость И НЕ Элементы.СоздатьНовуюЦену.Видимость;
...
Функция ОписаниеЗапросаОстатки(Запрос, Области)
	...
	|	ВЫБОР
	|		КОГДА ЗапасыОстатки.СтруктурнаяЕдиница ССЫЛКА Справочник.Контрагенты
	|				И НЕ ЗапасыОстатки.ЗаказПокупателя = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|			ТОГДА ЗапасыОстатки.КоличествоОстаток
	|		ИНАЧЕ 0
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ЗапасыОстатки.ЗаказПокупателя ССЫЛКА Документ.ЗаказПокупателя
	...
	|ИЗ
	|	ВТОстаткиИРезервы КАК ОстаткиРезервы
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	СтруктурнаяЕдиница
	|АВТОУПОРЯДОЧИВАНИЕ";
	ЗапросТекстОстаткиНомерВПакете = 2;
...
&НаКлиенте
Процедура ES_КоэффициентПриИзменении(Элемент)
	//EFSOL_Шаповал Олег Анатольевич 20 сентября 2017 г. 8:38:12 +
	ES_УстановитьВидимостьЦенообразования();
	//EFSOL Шаповал О.А. -
КонецПроцедуры

&НаКлиенте
Процедура ES_УстановитьВидимостьЦенообразования()
	Если Объект.ES_Коэффициент Тогда
		Элементы.ES_Коэффициент.Видимость = Истина;
		Элементы.ES_ИспользоватьИНтервалыВеса.Видимость = Ложь;
		Элементы.ES_ГруппаВес.Видимость = Ложь;
	ИначеЕсли Объект.ES_ИспользоватьИнтервалыВеса Тогда
		Элементы.ES_ИспользоватьИНтервалыВеса.Видимость = Истина;
		Элементы.ES_ГруппаВес.Видимость = Истина;
		Элементы.ES_Коэффициент.Видимость = Ложь;
	Иначе
		Элементы.ES_ИспользоватьИНтервалыВеса.Видимость = Истина;
		Элементы.ES_ГруппаВес.Видимость = Ложь;
		Элементы.ES_Коэффициент.Видимость = Истина;
...
&НаКлиенте
Процедура ES_ИспользоватьИнтервалыВесаПриИзменении(Элемент)
	//EFSOL_Шаповал Олег Анатольевич 20 сентября 2017 г. 8:38:41 +
	ES_УстановитьВидимостьЦенообразования();
	//EFSOL Шаповал О.А. -
КонецПроцедуры
