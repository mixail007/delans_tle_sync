...
Процедура ВариантОграниченияСертификатаПриИзменении(Элемент)
	...
	Если НоменклатураГруппыЦеновыеГруппы.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Описание, "Таблица ограничений будет очищена. Продолжить?", РежимДиалогаВопрос.ДаНет,,КодВозвратаДиалога.Нет,"Изменение варианта уточнения");
	Иначе
...
Процедура УправлениеВидимостьюНаСервере()
	...
	Если ВариантОграниченияСертификата = Перечисления.ВариантыОграниченийСкидокПоНоменклатуре.ПоНоменклатуре Тогда
		...
		Элементы.ДекорацияПодсказкаУточнения.Заголовок = "Заполните уточнения, если требуется, чтобы сертификат действовал, либо не действовал "
			+ "при оплате определенных товаров или групп товаров. Если список не заполнен, то оплата сертификатом возможна для всех позиций номенклатуры.";
	...
	ИначеЕсли ВариантОграниченияСертификата = Перечисления.ВариантыОграниченийСкидокПоНоменклатуре.ПоКатегориям Тогда
		...
		Элементы.ДекорацияПодсказкаУточнения.Заголовок = "Заполните уточнения, если требуется, чтобы сертификат действовал, либо не действовал "
			+ "при оплате товаров определённых категорий. Если список не заполнен, то оплата сертификатом возможна для всех категорий.";
	...
	ИначеЕсли ВариантОграниченияСертификата = Перечисления.ВариантыОграниченийСкидокПоНоменклатуре.ПоЦеновымГруппам Тогда
		...
		Элементы.ДекорацияПодсказкаУточнения.Заголовок = "Заполните уточнения, если требуется, чтобы сертификат действовал, либо не действовал "
			+ "при оплате товаров определённых ценовых групп. Если список не заполнен, то оплата сертификатом возможна для всех ценовых групп.";
...
Процедура ДобавитьГруппуНоменклатуры(Команда)
	...
	Если ВариантОграниченияСертификата = ПредопределенноеЗначение("Перечисление.ВариантыОграниченийСкидокПоНоменклатуре.ПоНоменклатуре") Тогда
		ОткрытьФорму("Справочник.Номенклатура.ФормаВыбораГруппы",, ЭтотОбъект,,,, ОповещениеОЗакрытии);
	ИначеЕсли ВариантОграниченияСертификата = ПредопределенноеЗначение("Перечисление.ВариантыОграниченийСкидокПоНоменклатуре.ПоКатегориям") Тогда
