...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		Если НЕ ПустаяСтрока(Параметры.ТекстЗаполнения) Тогда
			...
			Объект.Наименование = СформироватьНаименование(Объект.НомерДоговора, Объект.ДатаДоговора, Объект.ВалютаРасчетов, УчетВалюты);
		КонецЕсли;
	...
	Иначе
		...
		НаименованиеОтредактировано = (Объект.Наименование <> СформироватьНаименование(Объект.НомерДоговора, Объект.ДатаДоговора, Объект.ВалютаРасчетов, УчетВалюты));
	КонецЕсли;
	...
	//EFSOL_Шаповал Олег Анатольевич 21 сентября 2017 г. 10:05:24 +
	Элементы.ES_Группа.Видимость = Константы.ES_СлужбаДоставки.Получить();
	Если Параметры.Ключ.Пустая() Тогда
		Объект.ES_УчетПриРасчетеЦенТК = False;
		Объект.ES_РасчетТарифовОтМаксимальногоВеса = Истина;  
	КонецЕсли;
	//EFSOL Шаповал О.А. -
...
Процедура НомерДоговораПриИзменении(Элемент)
	...
	Если Не НаименованиеОтредактировано Тогда
		Объект.Наименование = СформироватьНаименование(Объект.НомерДоговора, Объект.ДатаДоговора, Объект.ВалютаРасчетов, УчетВалюты);
	КонецЕсли;
	ПроверитьНомерНаДубли();
...
Процедура НаименованиеАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	СформированноеНаименование = СформироватьНаименование(Объект.НомерДоговора, Объект.ДатаДоговора, Объект.ВалютаРасчетов, УчетВалюты);
	
	Если ПустаяСтрока(СформированноеНаименование) Тогда
		Возврат;
	КонецЕсли;
	
	СтандартнаяОбработка = Ложь;
	ДанныеВыбора = Новый СписокЗначений;
	Если Не ПустаяСтрока(Текст) И Текст <> СформированноеНаименование Тогда
		ДанныеВыбора.Добавить(Текст);
	КонецЕсли;
	
	ДанныеВыбора.Добавить(СформированноеНаименование);
КонецПроцедуры
...
Процедура ВзаиморасчетыПриСозданииНаСервере();
	...
	Если Объект.Ссылка.Пустая()
		И Объект.Владелец.ВестиРасчетыПоДоговорам
		И НЕ Объект.Владелец.ВалютаРасчетовПоУмолчанию.Пустая()
		Тогда
		Объект.ВалютаРасчетов = Объект.Владелец.ВалютаРасчетовПоУмолчанию;
	КонецЕсли;
