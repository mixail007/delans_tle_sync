...
//
Функция ПолучитьДоговорПоУмолчаниюПоОрганизацииВидуДоговора(Контрагент, Организация, СписокВидовДоговора = Неопределено, ВалютаРасчетов = Неопределено) Экспорт
...
Функция ДоговорСоответствуетУсловиямДокумента(ТекстСообщения, Договор, Организация, Контрагент, СписокВидовДоговора) Экспорт
	...
	Если Не Контрагент.ВестиРасчетыПоДоговорам Или (ПолучитьФункциональнуюОпцию("КонтрольДоговоров") = Перечисления.ВидыКонтроляДоговоровПриПроведении.НеКонтролировать)Тогда
		Возврат Истина;
	КонецЕсли;
...
Процедура ДобавитьПараметрыКонтрагентаВШаблон(ПараметрыШаблона)
	...
	ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрШаблона(ПараметрыШаблона, "ДоговорыКонтрагентов.Владелец.ВладелецРегистрационныйНомер", МетаданныеКонтрагента.Реквизиты.РегистрационныйНомер.Синоним);
...
Функция ПечатьДоговораDOC(МассивОбъектов, ОбъектыПечати, ДополнительныеПараметрыПечати = Неопределено) Экспорт
	...
	Для каждого ОбъектПечати Из МассивОбъектов Цикл
		...
		ПечатнаяФормаДоговораСтруктура.Вставить("ВысотаТаблицы", 1);
