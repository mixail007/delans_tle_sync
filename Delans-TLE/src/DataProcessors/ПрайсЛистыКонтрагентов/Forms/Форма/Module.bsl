...
Процедура ОбновитьНаСервере()
	...
	Если Актуальность Тогда
		...
		ДополнительноеУсловие = "И ЦеныНоменклатурыКонтрагентовСрезПоследних.Актуальность = Истина";
	...
	|ПОМЕСТИТЬ Колонки
	|ИЗ
	|	РегистрСведений.ЦеныНоменклатурыКонтрагентов.СрезПоследних(&ПараметрыВиртуальнойТаблицы) КАК ЦеныНоменклатурыКонтрагентовСрезПоследних
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	КОНЕЦ КАК Родитель,
	|	ЦеныНоменклатурыКонтрагентовСрезПоследних.Номенклатура.ЦеноваяГруппа.РеквизитДопУпорядочивания КАК Порядок,
	|	ВЫБОР
	|		КОГДА ЦеныНоменклатурыКонтрагентовСрезПоследних.Номенклатура.ЦеноваяГруппа.Родитель = ЗНАЧЕНИЕ(Справочник.ЦеновыеГруппы.ПустаяСсылка)
	|			ТОГДА ЦеныНоменклатурыКонтрагентовСрезПоследних.Номенклатура.ЦеноваяГруппа.РеквизитДопУпорядочивания
	|		ИНАЧЕ ЦеныНоменклатурыКонтрагентовСрезПоследних.Номенклатура.ЦеноваяГруппа.Родитель.РеквизитДопУпорядочивания
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатурыКонтрагентов.СрезПоследних(&ПараметрыВиртуальнойТаблицы) КАК ЦеныНоменклатурыКонтрагентовСрезПоследних
	...
	|			И Группировки.ВидЦенКонтрагента = ЦеныНоменклатурыКонтрагентовСрезПоследних.ВидЦенКонтрагента
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПараметрыВиртуальнойТаблицы", ПараметрыВиртуальнойТаблицы); 
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент() Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Добавить", "Отображение", ОтображениеКнопки.Картинка);
