	...
	Для каждого Строка Из КоллекцияСтрок Цикл
		...
		Если ВыводитьКартинки Тогда
			...
			Если ЗначениеЗаполнено(Строка.Картинка) Тогда
				...
				НастройкиПрайсЛиста.РазмерКартинокБайт = НастройкиПрайсЛиста.РазмерКартинокБайт + Число(Строка.Картинка.Размер);
...
Процедура ЗаполнитьВВидеПолотноИерархияНоменклатуры(НастройкиПрайсЛиста, ТабличныйДокумент, ДеревоИерархии)
	...
	Если НастройкиПрайсЛиста.ВыводитьКонтактнуюИнформацию
		И ЗначениеЗаполнено(НастройкиПрайсЛиста.Логотип) Тогда
	...
	Если НастройкиПрайсЛиста.ВыводитьКонтактнуюИнформацию Тогда
		...
		ЗначенияПараметров.Вставить("Сайт",			НСтр("ru ='Сайт: '") + УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(НастройкиПрайсЛиста.Организация, Справочники.ВидыКонтактнойИнформации.СайтОрганизации));
...
Процедура ЗаполнитьВВидеДвеКолонкиИерархияНоменклатуры(НастройкиПрайсЛиста, ТабличныйДокумент, ДеревоИерархии)
	...
	Если НастройкиПрайсЛиста.ВыводитьКонтактнуюИнформацию Тогда
		...
		ЗначенияПараметров.Вставить("Адрес", 		НСтр("ru ='Адрес: '") + УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(НастройкиПрайсЛиста.Организация, Справочники.ВидыКонтактнойИнформации.ФактАдресОрганизации));
		ЗначенияПараметров.Вставить("Телефон", 		НСтр("ru ='Телефон, факс: '") + УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(НастройкиПрайсЛиста.Организация, Справочники.ВидыКонтактнойИнформации.ТелефонОрганизации) + ", " + УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(НастройкиПрайсЛиста.Организация, Справочники.ВидыКонтактнойИнформации.ФаксОрганизации));
		ЗначенияПараметров.Вставить("Сайт", 		НСтр("ru ='Site: '") + УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(НастройкиПрайсЛиста.Организация, Справочники.ВидыКонтактнойИнформации.СайтОрганизации));
		ЗначенияПараметров.Вставить("Email", 		НСтр("ru ='E-mail: '") + УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(НастройкиПрайсЛиста.Организация, Справочники.ВидыКонтактнойИнформации.EmailОрганизации));
...
Процедура ПолучитьЭлементыИИерархиюНоменклатуры(НастройкиПрайсЛиста, ДеревоИерархии)
	...
	Если НастройкиПрайсЛиста.ВариантыСортировки = 1 Тогда
		...
		СхемаКомпоновкиДанных.НастройкиПоУмолчанию.Структура[0].Структура[0].Порядок.Элементы[0].ТипУпорядочивания = НаправлениеСортировкиКомпоновкиДанных.Убыв;
		СхемаКомпоновкиДанных.НастройкиПоУмолчанию.Структура[0].Структура[0].Порядок.Элементы[1].ТипУпорядочивания = НаправлениеСортировкиКомпоновкиДанных.Убыв;
		
		Если НастройкиПрайсЛиста.ИспользоватьХарактеристики Тогда
			
			СхемаКомпоновкиДанных.НастройкиПоУмолчанию.Структура[0].Структура[0].Порядок.Элементы[2].ТипУпорядочивания = НаправлениеСортировкиКомпоновкиДанных.Убыв;
...
Процедура ПолучитьЭлементыИИерархиюКатегорийНоменклатуры(НастройкиПрайсЛиста, ДеревоИерархии)
	...
	Если НастройкиПрайсЛиста.ВариантыСортировки = 1 Тогда
		...
		СхемаКомпоновкиДанных.НастройкиПоУмолчанию.Структура[0].Порядок.Элементы[0].ТипУпорядочивания = НаправлениеСортировкиКомпоновкиДанных.Убыв;
		СхемаКомпоновкиДанных.НастройкиПоУмолчанию.Структура[0].Порядок.Элементы[1].ТипУпорядочивания = НаправлениеСортировкиКомпоновкиДанных.Убыв;
		
		СхемаКомпоновкиДанных.НастройкиПоУмолчанию.Структура[0].Структура[0].Порядок.Элементы[0].ТипУпорядочивания = НаправлениеСортировкиКомпоновкиДанных.Убыв;
		СхемаКомпоновкиДанных.НастройкиПоУмолчанию.Структура[0].Структура[0].Порядок.Элементы[1].ТипУпорядочивания = НаправлениеСортировкиКомпоновкиДанных.Убыв;
		
		СхемаКомпоновкиДанных.НастройкиПоУмолчанию.Структура[0].Структура[0].Структура[0].Порядок.Элементы[0].ТипУпорядочивания = НаправлениеСортировкиКомпоновкиДанных.Убыв;
		СхемаКомпоновкиДанных.НастройкиПоУмолчанию.Структура[0].Структура[0].Структура[0].Порядок.Элементы[1].ТипУпорядочивания = НаправлениеСортировкиКомпоновкиДанных.Убыв;
		
		Если НастройкиПрайсЛиста.ИспользоватьХарактеристики Тогда
			
			СхемаКомпоновкиДанных.НастройкиПоУмолчанию.Структура[0].Структура[0].Структура[0].Порядок.Элементы[2].ТипУпорядочивания = НаправлениеСортировкиКомпоновкиДанных.Убыв;
...
Процедура СформироватьПЛ(СтруктураПараметров, ФоновоеЗаданиеАдресХранилища = "") Экспорт
	...
	РезультатВыполнения.Вставить("РазмерКартинокБайт",	НастройкиПрайсЛиста.РазмерКартинокБайт);
...
Процедура ПрочитатьНастройкиПрайсЛиста(ПрайсЛист, НастройкиПрайсЛиста)
	...
	Если УправлениеДоступом.ЧтениеРазрешено(ПрайсЛист.Организация) Тогда
	...
	Иначе
		...
		НастройкиПрайсЛиста.Вставить("Организация",		Неопределено);
		НастройкиПрайсЛиста.Вставить("Логотип",			Неопределено);
	...
	НастройкиПрайсЛиста.Вставить("РазмерКартинокБайт",		0);
...
Функция ПолучитьНоменклатуруПрайслиста(ПрайсЛист) Экспорт
	
	НастройкиПрайсЛиста = Неопределено;
	ПрочитатьНастройкиПрайсЛиста(ПрайсЛист, НастройкиПрайсЛиста);
	
	ДеревоИерархии = Неопределено;
	...
	Если ТипЗнч(ДеревоИерархии) = Тип("ДеревоЗначений") Тогда
		Возврат ПолучитьМассивПоДеревуИерархии(ДеревоИерархии.Строки);
	Иначе
...
Функция ПолучитьМассивПоДеревуИерархии(СтрокиДереваИерархии, ВозвращаемыйМассив = Неопределено)
	...
	Для Каждого Строка Из СтрокиДереваИерархии Цикл
		ПолучитьМассивПоДеревуИерархии(Строка.Строки, ВозвращаемыйМассив);
		Если НЕ ЗначениеЗаполнено(Строка.НоменклатураСсылка) И ЗначениеЗаполнено(Строка.ПредставлениеГруппыВИерархии) Тогда
			ВозвращаемыйМассив.Добавить(Строка.ПредставлениеГруппыВИерархии);
		ИначеЕсли ЗначениеЗаполнено(Строка.НоменклатураСсылка) Тогда
			ВозвращаемыйМассив.Добавить(Строка.НоменклатураСсылка);
		КонецЕсли;
	КонецЦикла;
