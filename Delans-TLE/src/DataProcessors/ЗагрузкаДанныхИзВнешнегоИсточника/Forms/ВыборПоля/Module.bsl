...
&НаСервере
Процедура ВыполнитьОбязательныеДействияНаСервере(ИмяВыбранногоПоля, ВернутьЦветКолонки = Ложь)
	...
	ИначеЕсли ВернутьЦветКолонки
		И НайденныеСтрокиДереваПолей.Количество() = 1 Тогда
...
Процедура ЗапомнитьВыборИЗакрытьФорму(Поле, ДополнительныйРеквизит = Неопределено)
	...
	ВыполнитьОбязательныеДействияНаСервере(Поле.ИмяПоля, ВернутьЦветКолонки);
