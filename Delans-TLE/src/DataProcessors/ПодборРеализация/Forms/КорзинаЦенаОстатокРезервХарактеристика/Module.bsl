...
Процедура КонтекстныйПоискНаКлиенте(РезультатПоиска)
	...
	РезультатПоиска = ПодборНоменклатурыВДокументахПереопределяемый.МассивыОтборовНоменклатурыИХарактеристик(ТекстПоиска);
	
	Если РезультатПоиска.Номенклатура.Количество() = 0 Тогда
		
		РезультатПоиска = Неопределено;
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Ничего не найдено'"), 5, "Поиск...");
		
	ИначеЕсли РезультатПоиска.Номенклатура.Количество() > 200 Тогда
...
Функция ПолучитьСтавкуНДС(СтавкаНДС)
	...
	Если Объект.НалогообложениеНДС = ПредопределенноеЗначение("Перечисление.ТипыНалогообложенияНДС.ОблагаетсяНДС") Тогда
		...
		Иначе
			...
			Возврат ПодборНоменклатурыВДокументахПовтИсп.ПолучитьСтавкуНДСОрганизации(Объект.Организация, Объект.ПериодЦен);
...
Процедура ДобавитьНоменклатуруВКорзину(СтратегияЗаполненияСтруктурнойЕдиницы = "Общая")
	...
	Если ДанныеТекущейСтроки.ТабачнаяПродукция Тогда
	...
	Иначе
		...
		СтрокаКорзины.НеобходимостьВводаКодаМаркировки = ДанныеСтрокиКорзины.ТабачнаяПродукция;
...
Процедура ВвестиАкцизнуюМарку(СтрокаТЧ)
	...
	Если СтрокаТЧ.НеобходимостьВводаКодаМаркировки Тогда
		...
		ПараметрыФормы.Вставить("ОткрытоИзФормыПодбора", Истина);
	Конецесли;
