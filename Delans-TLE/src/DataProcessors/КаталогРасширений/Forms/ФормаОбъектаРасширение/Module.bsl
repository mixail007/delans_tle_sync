...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ КаталогРасширений.ПолучитьДанныеВерсииРасширенияВФоне(АдресХранилищаРезультат, Параметры.ИдентификаторВерсии) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не удалось отправить запрос в МС.'"),,,, Отказ);
		Возврат;
	КонецЕсли;
...
Процедура Оценить(Команда)
	...
	Если Тек_Оценка = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Выберите оценку'"),, "ПиктограммаОценка_3");	
		Возврат;	
	КонецЕсли;
