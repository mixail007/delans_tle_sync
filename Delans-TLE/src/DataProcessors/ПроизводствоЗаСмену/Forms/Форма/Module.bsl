	...
	Для каждого СтрокаЗаказ Из Заказы.ПолучитьЭлементы() Цикл
		Для каждого СтрокаПродукция Из СтрокаЗаказ.ПолучитьЭлементы() Цикл
			...
			ИначеЕсли СтрокаПродукция.ЕстьВыполненныеЭтапы 
				И НЕ СтрокаПродукция.ЕстьСохраненныеДокументыы 
...
Функция ПодразделенияПоЭтапуСервер(Этап)
	...
	|	ЭтапыПроизводстваСтруктутрныеЕдиницы.ПоУмолчанию КАК ПоУмолчанию
	|ИЗ
	|	Справочник.ЭтапыПроизводства.СтруктутрныеЕдиницы КАК ЭтапыПроизводстваСтруктутрныеЕдиницы
	|ГДЕ
	|	ЭтапыПроизводстваСтруктутрныеЕдиницы.Ссылка = &Этап";
	Выборка = Запрос.Выполнить().Выбрать();
