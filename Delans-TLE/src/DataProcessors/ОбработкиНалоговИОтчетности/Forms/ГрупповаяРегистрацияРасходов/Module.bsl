	...
	Если БезДокументаВзаиморасчетов Тогда
		...
		Для Каждого СтрокаРегистрации Из ТаблицаРегистрации Цикл
			...
			Если СтрокаСоответствия = Неопределено Тогда
				СоответствиеВидаНалогаИДанных.Вставить(СтрокаРегистрации.ВзаиморасчетыСБюджетом.ВидНалога, Новый Структура("ВзаиморасчетыСБюджетом, Представление,Сумма", СтрокаРегистрации.ВзаиморасчетыСБюджетом, СтрокаРегистрации.Представление, СтрокаРегистрации.СуммаНачислений));
			Иначе
				ОбщаяСумма = СтрокаСоответствия.Сумма + СтрокаРегистрации.СуммаНачислений;
				СоответствиеВидаНалогаИДанных.Вставить(СтрокаРегистрации.ВзаиморасчетыСБюджетом.ВидНалога, Новый Структура("ВзаиморасчетыСБюджетом, Представление,Сумма", СтрокаРегистрации.ВзаиморасчетыСБюджетом, СтрокаРегистрации.Представление, ОбщаяСумма));
			КонецЕсли;
		КонецЦикла;
		Запрос = Новый Запрос(
		"ВЫБРАТЬ
