...
Процедура ОтразитьРасходыВПрограмме(Команда)
	...
	ДанныеПоНачислениям.Добавить(Новый Структура("НачислениеНалогов,Сумма,Представление", ДокументВзаиморасчетовСБюджетомФФОМС, ФФОМСЗаСебяЧисло, НСтр("ru='ФФОМС'")));
	
	Если Год(ДатаДокументаОбработкиСобытия) < 2014 Тогда
		ДанныеПоНачислениям.Добавить(Новый Структура("НачислениеНалогов,Сумма,Представление", ДокументВзаиморасчетовСБюджетомПФРСтраховая, ПФРСтраховаяЗаСебяЧисло, НСтр("ru='ПФР страховая'")));
		ДанныеПоНачислениям.Добавить(Новый Структура("НачислениеНалогов,Сумма,Представление", ДокументВзаиморасчетовСБюджетомПФРНакопительная, ПФРНакопительнаяЗаСебяЧисло, НСтр("ru='ПФР накопительная'")));
	Иначе
		ДанныеПоНачислениям.Добавить(Новый Структура("НачислениеНалогов,Сумма,Представление", ДокументВзаиморасчетовСБюджетомПФРСтраховая, ПФРСтраховаяЗаСебяЧисло, НСтр("ru='ПФР'")));
	КонецЕсли;
