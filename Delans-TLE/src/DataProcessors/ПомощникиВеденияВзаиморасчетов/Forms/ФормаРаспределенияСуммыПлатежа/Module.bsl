...
Процедура НастроитьЭлементыПоВидуКлиента()
	...
	РаботаСОтборами.НастроитьПанельОтборовМобильныйКлиент(ЭтотОбъект,,,
		ПолучитьСтрокуПроверяемыхРеквизитов()
	);
...
Процедура ПредставлениеДокументаНажатие(Элемент, СтандартнаяОбработка)
	...
	Иначе
		ПерейтиПоНавигационнойСсылке(ПолучитьНавигационнуюСсылку(ТекущаяСтрокаДерева.Объект));
	КонецЕсли;
...
Процедура УчестьДанныеРасшифровкиПлатежаИСвязанныхДокументов()
	...
	// Если в ТЧ по каждому договору только одна комбинация курс+кратность, то курс для всех строк по договору установим, как в документе.
	// В текущей форме пользователь может поменять курс в форме вручную или через контекстное меню.
		...
		ИначеЕсли ПолеГруппировки = "Документ" Тогда
			...
			Если ЗначениеЗаполнено(ТекущийОбъект) Тогда
				...
				ТекущаяСтрокаДерева.ДокументНеОбрабатывается = (ТекущаяВыборка.ДокументПозжеТекущего
					...
					ИЛИ (ТекущаяВыборка.ДокументНеПроведен И ЭтоЗачетПредоплаты)
				;
...
Процедура РаспределитьСуммуПлатежаРекурсивно(СтрокаРодитель, СуммаКРаспределению, ОбрабатыватьДолги, ОбнулятьСуммуПодчиненных, ПервыйПроходПриЗачетеПредоплаты = Ложь, НакопленнаяСумма = 0)
	...
	Для Каждого ТекущаяСтрока Из СтрокиДляРаспределения Цикл
		...
		ИначеЕсли ЭтоЗачетПредоплаты
			...
			// Когда Помощник вызывается из накладных сначала обрабатываются строки с заказами (берем МИНИМУМ общего остатка распределения остатка в строке.
			// А на втором проходе выполняем распредление только по строкам, в которых заказ не выбран.
