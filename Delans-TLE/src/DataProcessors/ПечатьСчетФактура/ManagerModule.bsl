...
Функция ЕстьЗапасы(ТабличнаяЧастьДокумента)
	...
	Возврат (ТабличнаяЧастьДокумента.Количество() > 0)
		И (ТабличнаяЧастьДокумента.Найти(Перечисления.ТипыНоменклатуры.Запас) <> Неопределено);
...
Процедура УчестьРазличияСводногоСФ(ДанныеОбъекта, ДанныеПечати, ЭтоУПД)
	...
	Для каждого Строка Из ДанныеОбъекта.ТаблицаПокупателей Цикл
		...
		ИНН_КПП_Покупателя = ИНН_КПП_Покупателя + ?(ПустаяСтрока(ИНН_КПП_Покупателя), "", "; ") 
			+ УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(Строка.СведенияОПокупателе, "ИНН,", Ложь) + ?(ПустаяСтрока(КПП), "", "/" + КПП);
...
Процедура ОбработатьПустыеПоляШапки(ДанныеПечати, ЕстьЗапасы, ЭтоУПД)
	...
	Если ПустаяСтрока(ДанныеПечати.ПоДокументу) Тогда
		...
		ДанныеПечати.ПоДокументу = ДанныеПечати.ПоДокументу + Символы.НПП + СтрШаблон(ШаблонТекста, ?(МассивВидовОпераций.Найти(ДанныеПечати.ВидОперации) <> Неопределено, Прочерк, Символы.НПП));
...
Процедура ОбработатьПустыеЗначенияДанныхПечатиСтроки(ДанныеПечати)
	...
	//
	Если ДанныеПечати.СтранаСсылка = Справочники.СтраныМира.Россия
		ИЛИ ПустаяСтрока(ДанныеПечати.ПредставлениеСтраны) 
		Тогда
	...
	Если ДанныеПечати.Свойство("ТоварКодТНВЭД")
		И НЕ ЗначениеЗаполнено(ДанныеПечати.ТоварКодТНВЭД) Тогда
...
Процедура ЗаменитьПустыеЗначенияНаПрочерки(ДанныеПечати, ИменаПолей)
	...
	Для каждого ЭлементСтруктуры Из ДанныеПечати Цикл
		...
		Если МассивИмен <> Неопределено 
			И МассивИмен.Найти(ЭлементСтруктуры.Ключ) = Неопределено Тогда
...
Функция ИнформацияОГрузеПрописью(ЗначениеДляПечати, ЭтоВес)
	...
	Если ЭтоВес Тогда
		...
		Возврат НРег(ЧислоПрописью(ЗначениеДляПечати, "ДП=Истина", "килограмм,килограмма,килограммов,м,грамм,грамма,граммов,м,3"));
...
Функция СформироватьПредставлениеОснования(ДанныеПечати)
	...
	ДанныеОДоверенности = СокрЛП(ДанныеПечати.ДоверенностьНомер) + СокрЛП(ДанныеПечати.ДоверенностьВыдана) + СокрЛП(ДанныеПечати.ДоверенностьЛицо);
	Если НЕ ПустаяСтрока(ДанныеОДоверенности)
		ИЛИ ЗначениеЗаполнено(ДанныеПечати.ДоверенностьДата) Тогда
		
		Основание = Основание + ?(ПустаяСтрока(Основание), "", ";")
			+ НСтр("ru =' по доверенности №'") + ДанныеПечати.ДоверенностьНомер
			+ НСтр("ru =' от '") + Формат(ДанныеПечати.ДоверенностьДата, "ДЛФ=DD")
			+ НСтр("ru =' выданной '") + ДанныеПечати.ДоверенностьВыдана
			+ Символы.НПП + ДанныеПечати.ДоверенностьЛицо;
...
Функция УниверсальноеПредставлениеСубъекта(СведенияОСубъекте)
	
	ПредставлениеСубъекта = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОСубъекте, "ПолноеНаименование,");;
	Если НЕ ПустаяСтрока(СведенияОСубъекте.ИНН) Тогда
		
		Если ПустаяСтрока(СведенияОСубъекте.КПП) Тогда
			
			ПредставлениеСубъекта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1, ИНН %2'"), ПредставлениеСубъекта, СведенияОСубъекте.ИНН);
			
		Иначе
			
			ПредставлениеСубъекта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1, ИНН/КПП %2/%3'"), ПредставлениеСубъекта, СведенияОСубъекте.ИНН, СведенияОСубъекте.КПП);
...
Функция СтрокаКорректноРазмещаетсяНаСтранице(ТабличныйДокумент, ОбластиМакета, СтруктураИтогов, Ошибки) Экспорт
	...
	Для каждого ЭлементСтруктуры Из ОбластиМакета Цикл
		...
		Если НЕ СтруктураИтогов.ЭтоУПД
			И ЭлементСтруктуры.Ключ = "ОбластьПодвалНакладной" Тогда
			
			Продолжить;
			
		ИначеЕсли СтруктураИтогов.ЭтоУПД
			И ЭлементСтруктуры.Ключ = "ИтогоПоСтранице" Тогда
...
Функция ПредставлениеПФ(ЭтоУПД = Ложь, ИспользоватьФаксимиле = Ложь) Экспорт
	...
	Если ЭтоУПД Тогда
		...
		Возврат НСтр("ru = 'Универсальный передаточный документ'") + ?(ИспользоватьФаксимиле, НСтр("ru =' (факсимиле)'"), "");
	...
	Для каждого ДанныеОбъекта Из ДанныеОбъектовПечати Цикл
		...
		Если ЭтоУПД Тогда
			...
			Если ОбластиМакета.ОбластьПодвалНакладной <> Неопределено Тогда 
				...
				ДанныеПечати.Вставить("ПредставлениеОрганизации", УниверсальноеПредставлениеСубъекта(СведенияОПоставщике));
				ДанныеПечати.Вставить("ПредставлениеКонтрагента", УниверсальноеПредставлениеСубъекта(СведенияОПокупателе));
...
Процедура УчестьОсобенностиДокументаСчетФактура(ТаблицаДанных, СчетФактура, ДополнительныеСвойства)
	...
	Для каждого СтрокаТаблицы Из ТаблицаДанных Цикл
		...
		ЗаполнитьРеквизитПоСчетуФактуре(СтрокаТаблицы, "ДолжностьРуководителя", СчетФактура.ПодписьРуководителя.Должность);
		ЗаполнитьРеквизитПоСчетуФактуре(СтрокаТаблицы, "ДолжностьКладовщика", СчетФактура.ПодписьКладовщика.Должность);
		
		ЗаполнитьРеквизитПоСчетуФактуре(СтрокаТаблицы, "РасшифровкаПодписиРуководителя", СчетФактура.ПодписьРуководителя.РасшифровкаПодписи);
		ЗаполнитьРеквизитПоСчетуФактуре(СтрокаТаблицы, "РасшифровкаПодписиГлавногоБухгалтера", СчетФактура.ПодписьГлавногоБухгалтера.РасшифровкаПодписи);
		ЗаполнитьРеквизитПоСчетуФактуре(СтрокаТаблицы, "РасшифровкаПодписиКладовщика", СчетФактура.ПодписьКладовщика.РасшифровкаПодписи);
		
		ЗаполнитьРеквизитПоСчетуФактуре(СтрокаТаблицы, "ФаксимилеРуководителя", СчетФактура.ПодписьРуководителя.Факсимиле);
		ЗаполнитьРеквизитПоСчетуФактуре(СтрокаТаблицы, "ФаксимилеГлавногоБухгалтера", СчетФактура.ПодписьГлавногоБухгалтера.Факсимиле);
	...
	Если СчетФактура.СводныйКомиссионный Тогда
		...
		Для каждого СтрокаТаблицы Из ТаблицаДанных Цикл
			...
			Для каждого СтрокаПокупателя Из СтрокаТаблицы.ТаблицаПокупателей Цикл
				...
				СтрокаПокупателя.СведенияОПокупателе		= УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(СтрокаПокупателя.Покупатель, СтрокаТаблицы.ДатаДокумента,,);
				СтрокаПокупателя.СведенияОГрузополучателе	= УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(СтрокаПокупателя.Покупатель, СтрокаТаблицы.ДатаДокумента,,);
	...
	Если ДополнительныеСвойства.Свойство("ЭтоЗаказНаряд")
		И ДополнительныеСвойства.ЭтоЗаказНаряд Тогда
	...
	Если ДополнительныеСвойства.Свойство("ЭтоОтчетОпереработке")
		И ДополнительныеСвойства.ЭтоОтчетОпереработке Тогда
...
Функция РазобратьДокументыНаСчетаФактуры(МассивОбъектов, ЭтоУПД, ЭтоВходящийДокумент = Ложь) Экспорт
	...
	Для каждого ОбъектПечати Из МассивОбъектов Цикл
		...
		Если ТипЗнч(ОбъектПечати) = Тип("ДокументСсылка.СчетФактура")
			...
			СтруктураДанных.Вставить("Версия", ?(СтруктураДанных.Ссылка.УдалитьЗапасы.Количество() > 0, "1.6.11", "1.6.12"));
		...
		Если СтруктураДанных <> Неопределено Тогда
			...
			Если ЗначениеЗаполнено(ОписаниеСчетФактуры.СчетФактура)Тогда
				...
				Если ОписаниеСчетФактуры.СчетФактура.ВидОперации = Перечисления.ВидыОперацийСчетФактура.Аванс
					ИЛИ ОписаниеСчетФактуры.СчетФактура.ВидОперации = Перечисления.ВидыОперацийСчетФактураПолученный.Аванс
					Тогда
				...
				ИначеЕсли ОписаниеСчетФактуры.СчетФактура.ВидОперации = Перечисления.ВидыОперацийСчетФактура.Корректировка
					ИЛИ ОписаниеСчетФактуры.СчетФактура.ВидОперации = Перечисления.ВидыОперацийСчетФактураПолученный.Корректировка
					ИЛИ (ЭтоВходящийДокумент
						 И ТипЗнч(ОписаниеСчетФактуры.СчетФактура.ДокументОснование) = Тип("ДокументСсылка.КорректировкаПоступления")) Тогда
					
					ОписаниеСчетФактуры.ОписаниеДокумента = "ЭтоКорректировка";
					
				ИначеЕсли ЭтоВходящийДокумент
					И
					(ТипЗнч(ОписаниеСчетФактуры.СчетФактура.ДокументОснование) = Тип("ДокументСсылка.ОтчетКомитенту")
						ИЛИ ОписаниеСчетФактуры.СчетФактура.ВидОперации = Перечисления.ВидыОперацийСчетФактураПолученный.СуммовыеРазницы) Тогда
					
					ОписаниеСчетФактуры.ОписаниеДокумента = "НетВозможностиКорректноСформироватьДокумент";
					
				ИначеЕсли НЕ ЭтоВходящийДокумент
					И ОписаниеСчетФактуры.СчетФактура.СводныйКомиссионный
					И ЭтоУПД Тогда
		...
		ИначеЕсли ЭтоУПД
			И ОбъектПечати.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.НеОблагаетсяНДС Тогда
...
Функция ТаблицаДанныхВходящегоДокументаФормата1_6_12(СтрокаОписаниеСФ, ИспользоватьФаксимиле, ЭтоУПД, Ошибки)
	...
	Если СтрокаОписаниеСФ.ОписаниеДокумента = "ЭтоАванс" Тогда
		...
		Если НЕ ЭтоУПД Тогда
			...
			ТаблицаДанных = МенеджерДокумента.ДанныеПолученныхАвансовыхДокументовРегУчет(СтрокаОписаниеСФ.СчетФактура, ИспользоватьФаксимиле);
	...
	ИначеЕсли СтрокаОписаниеСФ.ОписаниеДокумента = "ЭтоКорректировка" Тогда
		...
		Иначе
			...
			ТаблицаДанных = Документы.СчетФактура.ПолучитьДанныеДляПечатиКорректировочныхСчетовФактур(СтрокаОписаниеСФ.СчетФактура, ТекстЗапроса, ЭтоУниверсальныйКорректировочныйДокумент);
	...
	Иначе
		...
		МассивДокументов.Добавить(СтрокаОписаниеСФ.СчетФактура.ДокументОснование);
		
		МенеджерДокумента = ОбщегоНазначения.МенеджерОбъектаПоСсылке(СтрокаОписаниеСФ.СчетФактура.ДокументОснование);
		ТаблицаДанных = МенеджерДокумента.ДанныеПолученныхДокументовРегУчет(МассивДокументов, Ложь, СтрокаОписаниеСФ.ПФТолькоВРублях, Ошибки);
...
Функция ТаблицаДанныхИсходящегоДокументаФормата1_6_11(СтрокаОписаниеСФ, ИспользоватьФаксимиле, ЭтоУПД, Ошибки)
	...
	Если СтрокаОписаниеСФ.ОписаниеДокумента = "ЭтоКорректировка" Тогда
		...
		ЭтоУниверсальныйКорректировочныйДокумент = Ложь;
		ТекстЗапроса = Документы.СчетФактура.ТекстЗапросаПечатьКорректировочныхСчетовФактур(ЭтоУниверсальныйКорректировочныйДокумент);
		ДанныеДокумента = Документы.СчетФактура.ПолучитьДанныеДляПечатиКорректировочныхСчетовФактур(СтрокаОписаниеСФ.СчетФактура, ТекстЗапроса, ЭтоУниверсальныйКорректировочныйДокумент);
		
		Если ТипЗнч(ДанныеДокумента) = Тип("ТаблицаЗначений")
	...
	Для каждого СтрокаОснования Из СтрокаОписаниеСФ.СчетФактура.ДокументыОснования Цикл
		...
		Если СтрокаОписаниеСФ.ОписаниеДокумента = "ЭтоАванс" Тогда
			...
			Если НЕ ЭтоУПД Тогда
				...
				МенеджерДокумента = ОбщегоНазначения.МенеджерОбъектаПоСсылке(СтрокаОписаниеСФ.СчетФактура);
				ДанныеДокумента = МенеджерДокумента.ДанныеАвансовыхДокументовРегУчет(СтрокаОписаниеСФ.СчетФактура, ИспользоватьФаксимиле);
				Прервать;
		...
		ИначеЕсли СтрокаОписаниеСФ.ОписаниеДокумента = "ЭтоСуммоваяРазница" Тогда
			...
			Если НЕ ЭтоУПД Тогда
				...
				МенеджерДокумента = ОбщегоНазначения.МенеджерОбъектаПоСсылке(СтрокаОписаниеСФ.СчетФактура);
				ТаблицаДанных = МенеджерДокумента.ДанныеДокументовСуммовойРазницыРегУчет(СтрокаОписаниеСФ.СчетФактура, ИспользоватьФаксимиле);
		...
		Иначе
			...
			ТаблицаДанных = МенеджерДокумента.ДанныеДокументовРегУчет(МассивДокументов, ИспользоватьФаксимиле, СтрокаОписаниеСФ.ПФТолькоВРублях, Ошибки);
...
Процедура УточнитьНомерПостановленияПоПолученнымДанным(СтрокаОписаниеСФ)
	...
	ДокументФормируетсяСогласноСФ981 = (СтрокаОписаниеСФ.ДанныеДокумента[0].ДатаДокумента >= Константы.НачалоИспользованияСФ981.Получить());
	Если ДокументФормируетсяСогласноСФ981 Тогда
...
Функция ЗаполнитьДанныеДокументов(ТаблицаОписанияСчетовФактур, ОбъектыПечати, ЭтоУПД, ИспользоватьФаксимиле, Ошибки) Экспорт
	...
	Для каждого СтрокаОписаниеСФ Из ТаблицаОписанияСчетовФактур Цикл
		...
		Если СтрокаОписаниеСФ.ВерсияДокумента = "1.6.11" Тогда
			...
			Если НЕ СтрокаОписаниеСФ.ЭтоВходящийДокумент
				И СтрокаОписаниеСФ.СчетФактура.СводныйКомиссионный
				И ЭтоУПД Тогда
				
				ТекстОшибки = НСтр("ru ='Нет данных для формирования. Используйте документ счет-фактура.'");
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, , ТекстОшибки, "");
				Продолжить;
				
			КонецЕсли;
			
			Если СтрокаОписаниеСФ.ОписаниеДокумента = "ЭтоКорректировка"
				И ЭтоУПД Тогда
				
				ТекстОшибки = НСтр("ru ='Нет оснований для формирования документа. Используйте печатную форму счет-фактура.'");
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, , ТекстОшибки, "");
				Продолжить;
				
			КонецЕсли;
			
			СтрокаОписаниеСФ.ДанныеДокумента = ТаблицаДанныхИсходящегоДокументаФормата1_6_11(СтрокаОписаниеСФ, ИспользоватьФаксимиле, ЭтоУПД, Ошибки)
			
		ИначеЕсли СтрокаОписаниеСФ.ВерсияДокумента = "1.6.12" Тогда
			
			Если СтрокаОписаниеСФ.ЭтоВходящийДокумент Тогда
				
				Если НЕ ЗначениеЗаполнено(СтрокаОписаниеСФ.СчетФактура.ДокументОснование) Тогда
					
					ТекстОшибки = НСтр("ru ='Не указан документ, который является основанием полученной счет-фактуры'");
					ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, , ТекстОшибки, "");
					Продолжить;
					
				КонецЕсли;
				
				СтрокаОписаниеСФ.ДанныеДокумента = ТаблицаДанныхВходящегоДокументаФормата1_6_12(СтрокаОписаниеСФ, ИспользоватьФаксимиле, ЭтоУПД, Ошибки);
				
			ИначеЕсли ЗначениеЗаполнено(СтрокаОписаниеСФ.СчетФактура) Тогда
				...
				СтрокаОписаниеСФ.ДанныеДокумента = ТаблицаДанныхИсходящегоДокументаФормата1_6_12(СтрокаОписаниеСФ, ИспользоватьФаксимиле, ЭтоУПД, Ошибки);
				
			ИначеЕсли ЭтоУПД
				И ЗначениеЗаполнено(СтрокаОписаниеСФ.ОснованиеУПД) Тогда
				...
				СтрокаОписаниеСФ.ДанныеДокумента = МенеджерДокумента.ДанныеДокументовРегУчет(МассивДокументов, ИспользоватьФаксимиле, СтрокаОписаниеСФ.ПФТолькоВРублях, Ошибки);
				
				ДополнительныеСвойства = Новый Структура;
				ДополнительныеСвойства.Вставить("ЭтоЗаказНаряд", (ТипЗнч(СтрокаОписаниеСФ.ОснованиеУПД) = Тип("ДокументСсылка.ЗаказПокупателя")));
				ДополнительныеСвойства.Вставить("ЭтоОтчетОпереработке", (ТипЗнч(СтрокаОписаниеСФ.ОснованиеУПД) = Тип("ДокументСсылка.ОтчетОПереработке")));
		...
		Иначе
			...
			ТекстОшибки = НСтр("ru ='Нет данных для формирования документа. Возможно отсутствует подчиненный счет-фактура.'");
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, , ТекстОшибки, "");
			Продолжить;
...
Процедура ВывестиТабличныеДокументы(ОписаниеПечатнойФормы, ТаблицаОписанияСчетовФактур, ОбъектыПечати, ЭтоУПД)
	...
	Для каждого СтрокаОписания Из ТаблицаОписанияСчетовФактур Цикл
		...
		Если СтрокаОписания.ОписаниеДокумента = "ЭтоКорректировка" Тогда
			...
			Документы.СчетФактура.ВывестиКорректировочныйСчетФактуруВТабличныйДокумент(ОписаниеПечатнойФормы, СтрокаОписания.ДанныеДокумента, ЭтоУПД);
		...
		СсылкаОбластиПечати = ?(НЕ ЗначениеЗаполнено(СтрокаОписания.СчетФактура) И ЭтоУПД, СтрокаОписания.ОснованиеУПД, СтрокаОписания.СчетФактура);
		УправлениеПечатью.ЗадатьОбластьПечатиДокумента(ОписаниеПечатнойФормы.ТабличныйДокумент, НомерСтрокиНачало, ОбъектыПечати, СсылкаОбластиПечати);
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	ПечатнаяФорма = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм, "УниверсальныйПередаточныйДокументФаксимиле");
	Если ПечатнаяФорма <> Неопределено Тогда
