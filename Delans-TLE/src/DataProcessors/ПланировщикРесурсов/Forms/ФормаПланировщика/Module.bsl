...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РасписаниеРаботПереключатель = ?(Не ЗначениеЗаполнено(РасписаниеРаботПереключатель), "Интервал планирования", РасписаниеРаботПереключатель);
	...
	Если Не НастройкиЗагружены Тогда
		...
		Если Не ЭтоПодбор Тогда
			Элементы.ОтобразитьИнтревалПланирования.Пометка = Истина;
			УстановитьНастройкиЭлементовФормыПриСоздании();
			ГраницыПланирования = Неопределено;
		КонецЕсли;
	КонецЕсли;
	
	ЭтаФорма.Заголовок = "Планировщик ресурсов";
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Не ЭтоПодбор и Не ТолькоПросмотрФормы и Не ЗавершениеРаботы Тогда
		...
		СтрокиПоВарианту = ДанныеРекизитовФормыКэш.НайтиСтроки(ПараметрыОтбора);
		ОсновнойВариант = (СтрокиПоВарианту.Количество() и СтрокиПоВарианту[0].ОсновнойВариант) или Не ДанныеРекизитовФормыКэш.Количество();
...
Процедура ЗагрузкаРесурсовВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	КоординатаКолонки = ?(РасписаниеРаботПереключатель = "Месяц",Формат(Область.Лево,"ЧГ="),1);
	
	РасшифровкаЯчейки = ЗагрузкаРесурсов.Область("R"+Строка(КоординатаСтроки)+"C"+Строка(КоординатаКолонки)).Расшифровка;
	...
	Если ТипЗнч(РасшифровкаЯчейки) = Тип("Структура") и РасшифровкаЯчейки.Свойство("Ресурс") и Не РасшифровкаЯчейки.Свойство("СсылкаНаДокумент") Тогда
		...
		Если (РасписаниеРаботПереключатель = "Интервал планирования" и ЗначениеЗаполнено(ДеньПериода))
			ИЛИ (РасписаниеРаботПереключатель = "День" И ЗначениеЗаполнено(ДеньПериода)) или РасписаниеРаботПереключатель = "Месяц" Тогда
			СтандартнаяОбработка = Ложь;
...
&НаКлиенте
Процедура ЗагрузкаРесурсовПриАктивизацииОбласти(Элемент)
	
	ПриАктивизацииОбластиТабличногоДокумента();
...
Процедура ПредставлениеПериодаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ПереключательКалендарьОтборы = "Интервал планирования" Тогда
...
Процедура ИспользоватьВариантыПриИзменении(Элемент)
	...
	Если Не ИспользоватьВарианты Тогда
		ДанныеРекизитовФормыКэш.Очистить();
...
Процедура ТолькоРабочееВремяПриИзменении(Элемент)
	...
	Если Не ТолькоРабочееВремя Тогда
		Элементы.ГруппаИтнервалОтображения.Доступность = Истина;
		НачалоИнтервалаОтображения = Дата(1,1,1);
		ОкончаниеИнтервалаОтображения = Дата(1,1,1);
	Иначе
		Элементы.ГруппаИтнервалОтображения.Доступность = Ложь;
	КонецЕсли;
...
Процедура КомандаУдалитьВариант(Команда)
	...
	Если Элементы.ВариантОтображения.СписокВыбора.Количество() = 2 или УдаленОсновнойВариант Тогда 
		...
		ДанныеРекизитовФормыКэш[0].ОсновнойВариант = Истина;
	...
	ДанныеРекизитовФормыКэш.Сортировать("ВариантОтображения Возр");
	...
	Для ИндексСписка = 1 по КоличествоВариантов Цикл
		...
		ДанныеРекизитовФормыКэш[ИндексСписка-1].ВариантОтображения = ИндексСписка;
...
Процедура КомандаОсновнойВариант(Команда)
	...
	Для ИндексСписка = 1 по КоличествоВариантов Цикл
		...
		Если ДанныеРекизитовФормыКэш[ИндексСписка-1].ВариантОтображения = ВариантОтображения Тогда
		...
		Если ДанныеРекизитовФормыКэш[ИндексСписка-1].ОсновнойВариант Тогда
			ДанныеРекизитовФормыКэш[ИндексСписка-1].ОсновнойВариант = Ложь;
			Элементы.ВариантОтображения.СписокВыбора[ИндексСписка-1].Представление = ПредставлениеВарианта;
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Процедура ОтобразитьИнтревалПланирования(Команда)
	
	РасписаниеРаботПереключатель = "Интервал планирования";
	
	Элементы.ОтобразитьИнтревалПланирования.Пометка = Истина;
	Элементы.ОтобразитьДень.Пометка = Ложь;
	Элементы.ОтобразитьМесяц.Пометка = Ложь;
	Элементы.ГруппаНастройкаИнтревалов.Доступность = Истина;
...
Процедура ОтобразитьДень(Команда)
	...
	Элементы.ОтобразитьИнтревалПланирования.Пометка = Ложь;
...
Процедура ОтобразитьМесяц(Команда)
	...
	РасписаниеРаботПереключатель = "Месяц";
	
	Элементы.ОтобразитьИнтревалПланирования.Пометка = Ложь;
	Элементы.ОтобразитьДень.Пометка = Ложь;
	Элементы.ОтобразитьМесяц.Пометка = Истина;
	Элементы.ГруппаНастройкаИнтревалов.Доступность = Ложь;
...
Процедура КомандаПодобратьВКорзину(Команда)
	...
	ВыделенныеОбласти = ЗагрузкаРесурсов.ВыделенныеОбласти;
	
	ЗавершатьПодбор = Ложь;

	Если РасписаниеРаботПереключатель = "Интервал планирования" Тогда
...
Процедура ВывестиТабличныйДокументКлиент()
	...
	Если МассивДат.Количество()>14 и РасписаниеРаботПереключатель = "Интервал планирования" Тогда
		Элементы.ОтобразитьИнтревалПланирования.Пометка = Ложь;
		Элементы.ОтобразитьДень.Пометка = Истина;
		РасписаниеРаботПереключатель = "День"
	КонецЕсли;
	...
	Если Не РасписаниеРаботПереключатель = "Месяц" и Не ЗначениеЗаполнено(ПериодПозиционирования) Тогда
...
Процедура ВывестиТабличныйДокумент(МассивДат = Неопределено, ОткрытиеФормы = Ложь, ВыводСКалендаря = Ложь)
	...
	Если РасписаниеРаботПереключатель = "Месяц" и Не ВыводСКалендаря Тогда
	...
	Если СтруктураШагаПланирования.Количество() Тогда
		...
		РесурсМинимальногоИнтревала = СтруктураШагаПланирования.Ресурс;
	Иначе
		РесурсМинимальногоИнтревала = Неопределено;
		МинимальныйШагПланирования = 1200
	КонецЕсли;
	
	МинимальныйИнтервал = 5;
	
	Элементы.ГруппаИтнервалОтображения.Видимость = ?(РасписаниеРаботПереключатель = "Интервал планирования", Истина, Ложь);
	Элементы.ГруппаОтборКонтакт.Видимость = ?(РасписаниеРаботПереключатель = "Интервал планирования", Истина, Ложь);
	
	РесурсыИДокументы = СтруктураРесурсовИДокументов();
	
	ТаблицаРесурсовПредпприятия = РесурсыИДокументы.ТаблицаРесурсов;
	
	Если Не ТаблицаРесурсовПредпприятия.Количество() Тогда
	...
	ПакетДанныхРесурсов = ПолучитьРасписаниеЗагрузкиРесурсов(ТаблицаРесурсовПредпприятия.ВыгрузитьКолонку("Ресурс"), РесурсыИДокументы.СписокДокументов);
	...
	Если РасписаниеРаботПереключатель = "Интервал планирования" и СписокДатПериода.Количество() <= 14 Тогда
		
		Элементы.ОтобразитьИнтревалПланирования.Пометка = Истина;
		
		ВывестиРасписаниеКратноИнтервалам(МакетПланированиеРесурсов, ОбластьМакета, СтруктураЦветов
		, ТаблицаРесурсовПредпприятия, РабочиеПериоды, РасписаниеЗагрузки, РесурсМинимальногоИнтревала);
	...
	Иначе
		...
		Если РасписаниеРаботПереключатель = "Интервал планирования" Тогда
			...
			, ТаблицаРесурсовПредпприятия, РабочиеПериоды, РасписаниеЗагрузки);
			
		ИначеЕсли РасписаниеРаботПереключатель = "День" Тогда
			ВывестиРасписаниеПоДням(МакетПланированиеРесурсов, ОбластьМакета, СтруктураЦветов
			, ТаблицаРесурсовПредпприятия, РабочиеПериоды, РасписаниеЗагрузки);
	...
	Элементы.ПредставлениеПериода.Доступность = ?(СписокДатПериода.Количество()=1, Ложь, Истина);
	Элементы.ПредставлениеПериода.Видимость = Не РасписаниеРаботПереключатель = "Месяц";
...
Процедура ВывестиРасписаниеПоМесяцам(МакетПланированиеРесурсов, ОбластьМакетаЗаголовка, СтруктураЦветов
	...
	КоличествоСтрокВОбласти = МаксимальноеКоличествоЗадействованыхРесурсовВЕдиницуПериода(ТаблицаРесурсовПредпприятия.ВыгрузитьКолонку("Ресурс"));
	...
	Для Каждого СтрокаГод из ТЗГодыПериода Цикл
		...
		Для Каждого СтрокаПериод из НайденныеСтрокиМесяцы Цикл
			...
			Для Каждого РесурсПредприятия из ТаблицаРесурсовПредпприятия Цикл
...
Процедура ВывестиРасписаниеКратноИнтервалам(МакетПланированиеРесурсов, ОбластьМакетаЗаголовка, СтруктураЦветов
	...
	Для Каждого ДеньСписка из ОбрабатываемыйСписокДат Цикл
		...
		ПараметрыОтбора = Новый Структура("РесурсПредприятия, Период", РесурсМинимальногоИнтревала,НачалоДня(НачалоИнтервала));
		...
		Для Каждого НайденнаяСтрока из НайденныеСтроки Цикл
			...
			Если Найти(Строка(НайденнаяСтрока.Ссылка), "<Объект не найден>" ) = 0 Тогда
				...
				ИначеЕсли ТипЗнч(НайденнаяСтрока.Ссылка) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
					...
					Если ЗначениеЗаполнено(НайденнаяСтрока.Контрагент.НомерТелефона) Тогда
						ДополнениеКонтрагент = ДополнениеКонтрагент +" (" + НайденнаяСтрока.Контрагент.НомерТелефона+")";
					Иначе
...
Процедура ВывестиДокументыРасшифровки(ДокументыРасшифровки,ИнтервалыВыводаПоДокументам,СоответствиеДокументовРасшифровкиСтрокам
	...
	Для Каждого ДокументРасшифровки из ДокументыРасшифровки Цикл
		...
		КаждыйИнтревалНоваяСтрока = ?(НЕ МассивДокументовСПересечениями.Найти(ДокументРасшифровки.Ссылка) = Неопределено, Истина, Ложь);
		
		Если КаждыйИнтревалНоваяСтрока Тогда
			ПараметрыОтбора = Новый Структура("Документ, НачалоИнтервала, ОкончаниеПредыдущегоИнтервалаЗагрузки, НомерСтрокиРасшифровки"
			,ДокументРасшифровки.Ссылка, ДокументРасшифровки.ВремяНачала, ДокументРасшифровки.ВремяОкончания, ДокументРасшифровки.НомерСтроки);
		...
		КоличестовЯчеекВИнтервале = (ОкончанияИнтервала - НачалоИнтервала)/60/5;
		...
		ЯчеекПоДокументуВИнтревале = (ВремяОкончанияДокументаВИнтервале - ВремяНачалаДокументаВИнтервале)/60/5;
		СмещениеОкончанияИнтервалаПоДокументу = КоличествоЯчеекДляОбъединения - ЯчеекПоДокументуВИнтревале;
		
		Если СтрокиПоДокументу.Количество() Тогда
			СтрокаПоДокументу = СтрокиПоДокументу[СтрокиПоДокументу.Количество()-1];
			
			СтрокаРасшифровки = ?(КаждыйИнтревалНоваяСтрока, СтрокаРасшифровки, СтрокаПоДокументу.НомерСтрокиРасшифровки);
			...
			Если СтрокаПоДокументу.КоличествоЯчеек = 0 Тогда
				...
				НомерЯчейкиОкончанияДокументаВИнтревале = (ВремяОкончанияДокументаВИнтервале - НачалоИнтервала)/60/5;
				...
				КолонкаОкончанияИнтревала = 2+ ИнтервалВыводаОбласти - КоличествоЯчеекДляОбъединения + НомерЯчейкиОкончанияДокументаВИнтревале+ДопСмещениеКолонки;
				
				ОбластьРасшифровки = ОбластьРасшифровкаПоДокументам.Область(СтрокаРасшифровки,КолонкаНачалаИнтервала,СтрокаРасшифровки,КолонкаОкончанияИнтревала);
				ОбластьРасшифровки.Объединить();
				...
				СтрокаПоДокументу.КоличествоЯчеек = ?(Не ВыходитЗаГраницуИнтервала, 0, ЯчеекПоДокументуВИнтревале);
				СтрокаПоДокументу.КоличествоОбъединенныхЯчеек = КолонкаОкончанияИнтревала - КолонкаНачалаИнтервала;
			...
			Иначе
				...
				КоличествоЯчеекДляОбъединенияРасшифровки = ЯчеекПоДокументуВИнтревале+СтрокаПоДокументу.КоличествоЯчеек;
				
				Если СтрокаПоДокументу.ОкончаниеПредыдущегоИнтервалаЗагрузки >= ДокументРасшифровки.ВремяНачала Тогда
					КоличествоЯчеекДляОбъединенияРасшифровки = СтрокаПоДокументу.КоличествоОбъединенныхЯчеек+ЯчеекПоДокументуВИнтревале+1;
				...
				КолонкаОкончанияИнтревала = 2+ИнтервалВыводаОбласти+ДопСмещениеКолонки - СмещениеОкончанияИнтервалаПоДокументу;
		...
		СтрокаРасшифровки = ?(КаждыйИнтревалНоваяСтрока, СтрокаРасшифровки+1, СтрокаРасшифровки);
...
Процедура СформироватьПоЗначениюПериодаОтбор()
	...
	Если СписокДатПериода.Количество()>14 и РасписаниеРаботПереключатель = "Интервал планирования" Тогда
		РасписаниеРаботПереключатель = "День";
		Элементы.ОтобразитьДень.Пометка = Истина;
		Элементы.ОтобразитьИнтревалПланирования.Пометка = Ложь;
	КонецЕсли;
...
&НаСервере
Функция МаксимумПоЗагрузке(ДокументыРасшифровки, НачалоИнтервала, ОкончанияИнтервала, КоличестоЯчеекВИнтервале, ТаблицаИнтервалаПо5Мин)
	...
	Для Каждого СтрокаДокументаРасшифровки из ДокументыРасшифровки Цикл
		...
		НомерЯчейкиОкончанияИнтревала = ?(НомерЯчейкиОкончанияИнтревала > КоличестоЯчеекВИнтервале, КоличестоЯчеекВИнтервале, НомерЯчейкиОкончанияИнтревала);
...
Процедура ОбновитьОтображениеОбластиСогласноЗагрузке(Область, ЗагрузкаРесурса, Мощность, ВремяНачалаИнтервала = Неопределено
	...
	Иначе
		...
		Если ВремяНачалаИнтервала = Неопределено Тогда
			...
			Если РасписаниеРаботПереключатель = "День" и Не СтруктураИтоговЗаДень = Неопределено и ЗначениеЗаполнено(Мощность) Тогда
				Область.Текст = Строка(ЗагрузкаРесурса)+"|"+Строка(Мощность)+"
				|
...
&НаСервере
Функция МаксимальноеКоличествоЗадействованыхРесурсовВЕдиницуПериода(СписокРесурсов)
...
Функция ГраницыРабочегоПериода(ПериодПоиска, Знач РабочиеПериоды, Знач РасписаниеЗагрузки, Ресурс = Неопределено)
	...
	Если КоличествоПоНайденнымСтрокамРабПер Тогда
		...
		Для Каждого НайденнаяСтрока из НайденныеСтрокиПоГрафику Цикл
			Если ЗначениеЗаполнено(НайденнаяСтрока.НачалоРабочегоПериодаЗаДень) и ЗначениеЗаполнено(НайденнаяСтрока.НачалоРабочегоПериодаЗаДень) Тогда
...
Процедура СохранитьНастройкиФормы()
	...
	СтруктураНастроек.Вставить("ДанныеРекизитовФормыКэш", ДанныеРекизитовФормыКэш.Выгрузить());
...
Процедура ЗагрузитьНастройкиФормы()
	...
	Если ТипЗнч(СтруктураНастроек) = Тип("Структура") Тогда
		Если СтруктураНастроек.Свойство("ДанныеМетокКэш") и СтруктураНастроек.Свойство("ДанныеРекизитовФормыКэш") Тогда
			ДанныеМетокКэш.Загрузить(СтруктураНастроек.ДанныеМетокКэш);
			ДанныеРекизитовФормыКэш.Загрузить(СтруктураНастроек.ДанныеРекизитовФормыКэш);
		КонецЕсли;
	КонецЕсли;
	
	
	Для Каждого СтрокаВариантов из ДанныеРекизитовФормыКэш Цикл
	...
	Если РасписаниеРаботПереключатель = "День" Тогда
		Элементы.ГруппаШагИнтервалаМин.Видимость = Ложь
	КонецЕсли;
...
Процедура УстановитьНаименованиеВарианта(НовоеНаименование, Параметры) Экспорт
	...
	Если ЗначениеЗаполнено(НовоеНаименование) Тогда
		...
		ПараметрыОтбора = Новый Структура("ВариантОтображения", ВариантОтображения);
		НайденныеСтроки = ДанныеРекизитовФормыКэш.НайтиСтроки(ПараметрыОтбора);
...
Процедура СоздатьВариант(Копирование = Ложь)
	...
	ЭтоОсновнойВариант = (НайденныеСтроки.количество() и НайденныеСтроки[0].ОсновнойВариант) или Не ДанныеРекизитовФормыКэш.Количество();
...
Процедура СохранитьДанныеВариантовОтображенияКлиент(НомерВарианта, ОсновнойВариант = Ложь)
	...
	НоваяСтрока = ДанныеРекизитовФормыКэш.Добавить();
...
Процедура ВосстановитьДанныеВариантовОтображения(НомерВарианта, ОткрытиеФормы = Ложь)
	...
	Элементы.ГруппаНастройкаИнтревалов.Доступность = ?(Не РасписаниеРаботПереключатель = "Месяц", Истина, Ложь);;
	
	Элементы.ОтобразитьДень.Пометка = ?(РасписаниеРаботПереключатель = "День", Истина, Ложь);
	Элементы.ОтобразитьМесяц.Пометка = ?(РасписаниеРаботПереключатель = "Месяц", Истина, Ложь);
	Элементы.ОтобразитьИнтревалПланирования.Пометка = ?(РасписаниеРаботПереключатель = "Интервал планирования", Истина, Ложь);
	
	Элементы.ГруппаИтнервалОтображения.Доступность = Не ТолькоРабочееВремя;
...
Процедура УдалитьСтрокиПоВариантуОтображения(НомерВарианта)
	...
	НайденныеСтроки = ДанныеРекизитовФормыКэш.НайтиСтроки(ПараметрыОтбора);
	Для Каждого СтрокаКэш из НайденныеСтроки Цикл
		ДанныеРекизитовФормыКэш.Удалить(СтрокаКэш);
	КонецЦикла;
...
Процедура ПодобратьВКорзину(Ресурс, Область, КонтрольГрафика)
	...
	Если РасписаниеРаботПереключатель = "Интервал планирования" и (ЗначениеЗаполнено(ВремяОкончания) и ЗначениеЗаполнено(ДеньПериода)) 
		или (РасписаниеРаботПереключатель = "День" И ЗначениеЗаполнено(ДеньПериода))Тогда 
...
Процедура ПодобратьВКорзинуКонтекстноеМеню(Ресурс, Область, ВремяНачала, ВремяОкончания, СтруктураДанные)
	...
	НачалоПериода = ?(РасписаниеРаботПереключатель = "День", НачалоДня(ВремяНачала), ВремяНачала); 
	...
	Если РасписаниеРаботПереключатель = "Интервал планирования" Тогда
		...
		Для Каждого ПериодСписка из СписокВыбранныхДат Цикл
			...
			ОкончаниеПериода = ОкончаниеСмежногоИнтревала(ПериодСписка.Значение, СписокВыбранныхДат);
	...
	Иначе
		...
		Для Каждого ПериодСписка из СписокВыбранныхДат Цикл
			...
			Иначе
				...
				ОкончаниеПериода = ОкончаниеСмежногоИнтревала(ПериодСписка.Значение, СписокВыбранныхДат, СоответствиеРабочихПериодовПоДням);
...
&НаКлиенте
Функция ОкончаниеСмежногоИнтревала(ДеньПериода, СписокВыбранныхДат, СоответствиеРабочихПериодовПоДням = Неопределено)
...
Процедура ПослеВыбораИзМеню(ВыбранныйЭлемент, Параметры) Экспорт
	...
	Если Не ВыбранныйЭлемент.Значение = "Перейти" и Не РасписаниеРаботПереключатель = "Месяц" Тогда
	...
	ИначеЕсли ВыбранныйЭлемент.Значение = "Перейти" и Не РасписаниеРаботПереключатель = "Месяц" Тогда
		ВывестиКратноИнтервалам(Параметры.День);
	ИначеЕсли ВыбранныйЭлемент.Значение = "Перейти" и РасписаниеРаботПереключатель = "Месяц" Тогда
...
Процедура ЗаполнитьСписокПериодаПозиционирования()
	...
	Если НЕ РасписаниеРаботПереключатель = "Интервал планирования" или Не СписокДатПериода.Количество() Тогда
		Возврат
	КонецЕсли;
...
Процедура ОбработатьСобытиеВыбораИнтервала(Ресурс, Область, День, КонтрольГрафика)
	...
	Если Не РасписаниеРаботПереключатель = "Месяц" 
		и (ТолькоПоПодсистеме1 или ТолькоПоПодсистеме2 или ТолькоПоПодсистеме3) Тогда
		Список.Добавить("Подобрать");
	КонецЕсли;
	
	Если Не РасписаниеРаботПереключатель = "Интервал планирования" Тогда
		Список.Добавить("Перейти",,Ложь, БиблиотекаКартинок.ПерейтиВперед);
	КонецЕсли;
	
	Если Не ЭтоПодбор и Не РасписаниеРаботПереключатель = "Месяц" Тогда
...
Процедура ЗаполнитьБуфер()
	...
	Для Каждого ВыделеннаяОбласть из ВыделенныеОбласти Цикл
		...
		Для ИндексГраницы = ВерхняяГраницаОбласти по НижняяГраницаОбласти Цикл
			...
			КоординатаКолонки = ?(РасписаниеРаботПереключатель = "Месяц",Формат(ТекОбласть.Лево,"ЧГ="),1);
...
&НаКлиенте
Процедура ПриАктивизацииОбластиТабличногоДокумента()
	...
	Если Не СписокДатПериода.Количество() Тогда Возврат КонецЕсли;
	
	Если РасписаниеРаботПереключатель = "Месяц" Тогда Возврат КонецЕсли;
	...
	Если РасписаниеРаботПереключатель = "Интервал планирования" и Не ВыборПериода Тогда
