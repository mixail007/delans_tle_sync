...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПоказыватьПотенциальныеНастройки = НЕ ОбщегоНазначения.ЭтоМобильныйКлиент();
	...
	Для Каждого Настройка Из НастройкиПрограммы Цикл
		...
		Если Настройка.Потенциальная И НЕ ПоказыватьПотенциальныеНастройки Тогда
			Продолжить;
		КонецЕсли;
		
		Если Настройка.ПравоРедактирования Тогда
			ОпределитьДоступностьНастройки(Настройка.Идентификатор);
		Иначе
	...
	// ИПП
	БИПДанныеАутентификации = Неопределено;
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		НастроитьВидимостьДеревоНастройки(Элементы, Истина);
		Элементы.ДеревоРазделов.Ширина = 0;
		Элементы.ДеревоРазделов.РастягиватьПоГоризонтали = Истина;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "РегистрацияВзаимодействия" Тогда
		...
		РегистрацияВзаимодействияДляТелефонии = Параметр;
	...
	ИначеЕсли ИмяСобытия = "ИнтернетПоддержкаПодключена" Тогда
		...
		Если Параметр <> Неопределено Тогда
			БИПДанныеАутентификации = Параметр;
		КонецЕсли;
		
	КонецЕсли;
	
	НастройкаПрограммыКлиентПереопределяемый.ОбработкаОповещения(ЭтотОбъект, ИмяСобытия, Параметр, Источник);
...
Процедура Подключаемый_ПриИзмененииПоля(Элемент)
	...
	НастройкаПрограммыКлиентПереопределяемый.ПриУстановкеЗначенияНастройки(Элемент.Имя, ЭтотОбъект[Элемент.Имя]);
	...
	Для Каждого Оповещение Из ОповещенияНаКлиенте Цикл
		Если Оповещение.Свойство("Действие") Тогда
			Если Оповещение.Действие = "ПерейтиПоНавигационнойСсылке" Тогда
				ПерейтиПоНавигационнойСсылке(Оповещение.Параметр);
			КонецЕсли;
		Иначе
...
Процедура СформироватьОписанияРазделов(СтрокиДерева)
	...
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		...
		Если СтрокаДерева.Строки.Количество() = 0 Тогда
			...
			Если ПоказыватьПотенциальныеНастройки Тогда
...
Процедура СгенерироватьЭлементыНастроек(ТаблицаНастроек)
	...
	Для каждого СтрокаНастроек Из ТаблицаНастроек Цикл
		...
		Если СтрокаНастроек.Потенциальная И НЕ ПоказыватьПотенциальныеНастройки Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(СтрокаНастроек.РасширеннаяПодсказка) Тогда
			Элементы[СтрокаНастроек.Идентификатор + "РасширеннаяПодсказка"].Заголовок = СтрокаНастроек.РасширеннаяПодсказка;
		КонецЕсли;
...
Процедура ИзменениеНастройкиСервер(Идентификатор, ОповещенияНаКлиенте)
	...
	Если Отказ Тогда
		...
		УстановитьЗначениеРеквизита(Идентификатор);
		Если Не ПустаяСтрока(ТекстОшибки) Тогда
			ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, , Идентификатор);
		КонецЕсли;
	...
	Если ОписаниеНастройки.ВлияетНа <> Неопределено Тогда
		Для Каждого Настройка Из ОписаниеНастройки.ВлияетНа Цикл
			ОпределитьДоступностьНастройки(Настройка);
		КонецЦикла;
	КонецЕсли;
...
&НаСервере
Процедура ОпределитьДоступностьНастройки(Настройка)
	
	НастройкаПрограммыПереопределяемый.ПриОпределенииДоступности(Настройка, Элементы[Настройка], ЭтотОбъект);
	
	ОтступНастройки = Элементы.Найти("Отступ_" + Настройка);
	Если ОтступНастройки <> Неопределено Тогда
		ОтступНастройки.Видимость = Элементы[Настройка].Видимость;
	КонецЕсли;
	
	ГруппаНастройки = Элементы.Найти("ГруппаГоризонтальная_" + Настройка);
	Если ГруппаНастройки <> Неопределено Тогда
		ГруппаНастройки.Видимость = Элементы[Настройка].Видимость;
	КонецЕсли;
...
Процедура НастроитьВидимостьДеревоНастройки(Элементы, ПоказатьВыборРаздела)
	
	Элементы.СнятьОтбор.Видимость = НЕ ПоказатьВыборРаздела;
	Элементы.ДеревоРазделов.Видимость = ПоказатьВыборРаздела;
	Элементы.КолонкаСправа.Видимость = НЕ ПоказатьВыборРаздела;
