...
Процедура ПоискНечитаемойАкцизнойМарки(Команда)
	...
	ПараметрОткрытияФормы = Новый Структура("СоответствиеШтрихкодов", Новый ФиксированноеСоответствие(СоответствиеШтрихкодовСтрокДерева));
...
Процедура ОбновитьВыводимоеПредставлениеПроверкиСодержимого(Форма, ТекущиеДанные)
	...
	Форма.Элементы.СтраницыПредставлениеПроверкиПодчиненных.Ширина = СтрДлина(Форма.ВыводимоеПредставлениеПроверкиСодержимого) + 3;
...
Процедура РучнойВводШтрихкодаЗавершение(ДанныеШтрихкода, ДополнительныеПараметры) Экспорт
	...
	АкцизныеМаркиЕГАИСКлиент.ОбработатьДанныеШтрихкода(Новый ОписаниеОповещения("ПоискПоШтрихкодуЗавершение", ЭтотОбъект, ДополнительныеПараметры),
	                                                   ЭтотОбъект, ДанныеШтрихкода, ПараметрыСканированияАкцизныхМарок());
...
Функция РезультатДобавленияСуществующейУпаковкиВДеревоМаркированнойПродукции(ДанныеШтрихкода, СоответствиеШтрихкодовСтрокДерева, ОбработкаДанныхТСД)
	...
	ДанныеУпаковки = Справочники.ШтрихкодыУпаковокТоваров.ВложенныеШтрихкодыУпаковок(ДанныеШтрихкода.ШтрихкодУпаковки, ОрганизацияЕГАИС);
	ПреобразоватьСчитаннуюУпаковкуЕслиНеобходимо(ДанныеУпаковки.ДеревоУпаковок);
	
	ЕстьОшибки = ЕстьДублиВОсканированнойУпаковке(ДанныеУпаковки.ДеревоУпаковок, СоответствиеШтрихкодовСтрокДерева);
	
	Если ЕстьОшибки Тогда
		
		Результат.ЕстьОшибки                   = Истина;
		Результат.АдресХранилищаДереваУпаковки = ПоместитьВоВременноеХранилище(ДанныеУпаковки.ДеревоУпаковок, УникальныйИдентификатор);
		Возврат Результат;
	...
	Для Каждого СтрокаДерева Из ДанныеУпаковки.ДеревоУпаковок.Строки Цикл
	...
	ИначеЕсли СледующаяОперация = "МаркируемаяАлкогольнаяПродукция" Тогда 
		...
		Если ЗагрузкаДанныхТСД.МаркируемаяАлкогольнаяПродукция.Количество() <> ЗагрузкаДанныхТСД.ИндексОбрабатываемогоЭлемента Тогда
			...
			Если ПараметрыСканирования.ЗапрашиватьНоменклатуру 
				...
				Если ДанныеВыбораПоАлкогольнойПродукции <> Неопределено 
					И ДанныеВыбораПоАлкогольнойПродукции.АлкогольнаяПродукция = ДанныеШтрихкода.АлкогольнаяПродукция Тогда
					
					ДанныеШтрихКода.Номенклатура   = ДанныеВыбораПоАлкогольнойПродукции.Номенклатура;
					ДанныеШтрихКода.Характеристика = ДанныеВыбораПоАлкогольнойПродукции.Характеристика;
					ДанныеШтрихКода.Серия          = ДанныеВыбораПоАлкогольнойПродукции.Серия;
