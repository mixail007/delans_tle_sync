...
Процедура УстановитьВидимостьЭлементовДляМобильногоКлиента()
	...
	Если УправлениеНебольшойФирмойСервер.ЭтоМобильныйКлиент() Тогда
		...
		Элементы.СброситьВсеФлаги.Видимость = Ложь;
	КонецЕсли;
...
Процедура УстановитьЗначениеФлага(ЗначениеФлага)
	...
	ТекущаяСтраницаИмя = Элементы.Месяцы.ТекущаяСтраница.Имя;
	Окончание = Сред(ТекущаяСтраницаИмя, 2);
	
	ЭтотОбъект["НачислениеАмортизации" + Окончание] = ЗначениеФлага И УчетИмущества;
	ЭтотОбъект["РасчетПрямыхЗатрат" + Окончание] = ЗначениеФлага;
	ЭтотОбъект["РаспределениеЗатрат" + Окончание] = ЗначениеФлага;
	ЭтотОбъект["РасчетФактическойСебестоимости" + Окончание] = ЗначениеФлага;
	ЭтотОбъект["РасчетСебестоимостиВРознице" + Окончание] = ЗначениеФлага И УчетРозничныхПродаж;
	ЭтотОбъект["РасчетКурсовыхРазниц" + Окончание] = ЗначениеФлага И УчетВалютныхОпераций;
	ЭтотОбъект["РасчетФинансовогоРезультата" + Окончание] = ЗначениеФлага;
