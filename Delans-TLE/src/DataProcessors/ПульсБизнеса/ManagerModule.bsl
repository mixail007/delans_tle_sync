...
Функция РассчитатьПоказатели(Параметры)
	...
	Для каждого ОписаниеПоказателя Из Параметры.Показатели Цикл
		...
		ОбщегоНазначения.ВыполнитьВБезопасномРежиме("Обработки.ПульсБизнеса.Подключаемый_"+ОписаниеПоказателя.Показатель+"Расчет(Параметры.Запрос, Параметры.ОписаниеПоказателя, Параметры.Индекс, Параметры.Параметры)", ПараметрыВызова);
