...
Процедура ДобавитьПоказатель(
	...
	Если ЗначениеЗаполнено(ИмяОтчета) И ЗначениеЗаполнено(КлючВарианта) Тогда
		Стр.Вариант = ВариантыОтчетов.ВариантОтчета(ОбщегоНазначения.ИдентификаторОбъектаМетаданных(Метаданные.НайтиПоПолномуИмени(ИмяОтчета)), КлючВарианта);
	КонецЕсли; 
...
Процедура УстановитьВидимостьЭлементовДляМобильногоКлиента()
	...
	Если УправлениеНебольшойФирмойСервер.ЭтоМобильныйКлиент() И НЕ ЭтоБольшойЭкранИЛИПланшет() Тогда
...
&НаСервере
Функция ЭтоБольшойЭкранИЛИПланшет()
