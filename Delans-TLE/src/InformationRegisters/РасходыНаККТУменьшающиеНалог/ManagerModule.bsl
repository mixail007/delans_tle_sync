...
//  ОбъектУменьшенияНалога        - СправочникСсылка.Патенты,СправочникСсылка.ВидыДеятельностиЕНВД - ссылка на патент или вид деятельности, налоговый платеж по которому уменьшается.
...
//
Процедура ЗаписатьРасходыУменьшающиеНалог(ЗаписьКалендаря, ОбъектУменьшенияНалога = Неопределено, СуммаРасходов, КодПООКТМО = "", КодИФНС = "") Экспорт
...
Процедура ПолучитьСуммуРасходовУменьшающиеНалог(ЗаписьКалендаря, ОбъектУменьшенияНалога = Неопределено, СуммаРасходов, КодПООКТМО = "", КодИФНС = "") Экспорт
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
