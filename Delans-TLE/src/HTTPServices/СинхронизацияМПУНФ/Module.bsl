...
Функция CheckNewNumbersPost(Запрос)
	...
	Если НЕ ФоновоеЗаданиеПоЗагрузкеАктивно(КодУзла) Тогда
		//Синхронизация.СоздатьСообщениеСПодтвержденнымиНомерами(КодУзла);
		//СтруктураОтвета = РегистрыСведений.СформированныеСообщенияСИзмененнымиНомерамиМПУНФ.ПолучитьСообщенияДляОтправки(КодУзла);
...
Функция GetPrimaryDataPost(Запрос)
	...
	//СтруктураОтвета = Новый Структура("МассивСтавкиНДСМП, МассивСтатьиМП, МассивСтруктурныеЕдиницыМП, МассивКассыККММП",МассивСтавкиНДСМП, МассивСтатьиМП, МассивСтруктурныеЕдиницыМП, МассивКассыККММП);
	
	СтруктураОтвета = Новый Структура("МассивСПервичнымиДанными", МассивСПервичнымиДанными);
...
Функция СформироватьОтветДляКлиента(СтруктураЗапроса)
	...
	Если ФормированиеПакетовНаСервере = Перечисления.СтатусыФормированияПакетовМП.Завершено И ФормированиеПакетовНаКлиенте = Перечисления.СтатусыФормированияПакетовМП.Завершено Тогда
		КлиентскийУзел = ПланыОбмена.СинхронизацияМП.НайтиПоКоду(СтруктураЗапроса.КодКлиентскогоУзла);
		СтруктураЗаписи = Новый Структура("Узел, ДатаИВремяПоследнейСинхронизации, СтатусСинхронизации", КлиентскийУзел, ТекущаяДата(), Перечисления.СтатусыСинхронизацииМП.ОбменПервымСообщением);
		РегистрыСведений.ЖурналСинхронизацииМП.ЗаписатьИнформацию(СтруктураЗаписи);
	КонецЕсли;
...
Функция ПроверитьРаботуФоновогоЗаданияПоВыгрузкеПакетов(КодКлиентскогоУзла)
	...
	//Результат = ФоновыеЗадания.НайтиПоУникальномуИдентификатору(РегистрыСведений.ЖурналСинхронизацииМП.ПолучитьИдентификаторФоновогоЗадания(КодКлиентскогоУзла));
	ПросмотрТекЗадания = ФоновыеЗадания.ПолучитьФоновыеЗадания(Новый Структура("Ключ", РегистрыСведений.ЖурналСинхронизацииМП.ПолучитьИдентификаторФоновогоЗадания(КодКлиентскогоУзла)));
	
	ЗаданиеВыполняется = Ложь;
	//МассивЗаданий = ФоновыеЗадания.ПолучитьФоновыеЗадания(Новый Структура("Наименование", "Формирование пакетов для " + КодКлиентскогоУзла));
	//МассивЗаданий2 = ФоновыеЗадания.ПолучитьФоновыеЗадания();
...
Функция ФоновоеЗаданиеПоЗагрузкеАктивно(КодКлиентскогоУзла)
	...
	МассивЗаданий = ФоновыеЗадания.ПолучитьФоновыеЗадания(Новый Структура("Наименование", "Загрузка пакета от " + КодКлиентскогоУзла));
