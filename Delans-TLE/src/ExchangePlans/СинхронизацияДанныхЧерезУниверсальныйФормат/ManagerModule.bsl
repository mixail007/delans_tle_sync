...
Функция ВыполнитьПереходНаСинхронизациюДанныхЧерезУниверсальныйФормат(НастройкаСинхронизацииДанных) Экспорт
	...
	Попытка
		...
		НастройкаСинхронизацииДанныхЭтотУзел = ПланыОбмена.ОбменУправлениеНебольшойФирмойБухгалтерия30.ЭтотУзел();
		Запись = РегистрыСведений.ПсевдонимыПредопределенныхУзлов.СоздатьМенеджерЗаписи();
		Запись.Корреспондент = НастройкаСинхронизацииЧерезУниверсальныйФормат;
		Запись.КодУзла = НастройкаСинхронизацииДанныхЭтотУзел.Код;
