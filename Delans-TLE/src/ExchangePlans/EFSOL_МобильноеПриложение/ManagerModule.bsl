...
Функция ПолучитьИспользуемыеУзлыПланаОбмена() Экспорт
	...
	Запрос = Новый Запрос(
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	EFSOL_МобильноеПриложение.Ссылка КАК Ссылка
		|ИЗ
		|	ПланОбмена.EFSOL_МобильноеПриложение КАК EFSOL_МобильноеПриложение
		|ГДЕ
		|	НЕ EFSOL_МобильноеПриложение.ПометкаУдаления
		|	И EFSOL_МобильноеПриложение.Ссылка <> &ЭтотУзел");
		
	Запрос.УстановитьПараметр("ЭтотУзел", ПланыОбмена.EFSOL_МобильноеПриложение.ЭтотУзел());
