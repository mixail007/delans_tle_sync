...
Процедура ШапкаТабличнаяЧасть(Команда)
	...
	СтруктураПараметров.Вставить("ПоложениеЗаказаПокупателяВДокументахПоступления", Объект.ПоложениеЗаказаПокупателя);
...
Процедура ШапкаТабличнаяЧастьЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(СтруктураНастройкаДокумента) = Тип("Структура") И СтруктураНастройкаДокумента.БылиВнесеныИзменения Тогда
		...
		Если Объект.ПоложениеДатыПоступления<>СтруктураНастройкаДокумента.ПоложениеДатыПоступленияВЗаказеПоставщику Тогда
			Если СтруктураНастройкаДокумента.ПоложениеДатыПоступленияВЗаказеПоставщику=ПредопределенноеЗначение("Перечисление.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти") Тогда
				Для каждого СтрокаТабличнойЧасти Из Объект.Запасы Цикл
					СтрокаТабличнойЧасти.ДатаПоступления = Объект.ДатаПоступления;
				КонецЦикла; 
			ИначеЕсли Объект.Запасы.Количество() > 0 Тогда
				Объект.ДатаПоступления = Объект.Запасы[0].ДатаПоступления;
			КонецЕсли;
		КонецЕсли;
		Если Объект.ПоложениеЗаказаПокупателя<>СтруктураНастройкаДокумента.ПоложениеЗаказаПокупателяВДокументахПоступления Тогда
			Если СтруктураНастройкаДокумента.ПоложениеЗаказаПокупателяВДокументахПоступления=ПредопределенноеЗначение("Перечисление.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти") Тогда
				Для каждого СтрокаТабличнойЧасти Из Объект.Запасы Цикл
				...
				КонецЦикла; 
			ИначеЕсли Объект.Запасы.Количество() > 0 Тогда
				Объект.ЗаказПокупателя = Объект.Запасы[0].ЗаказПокупателя;
