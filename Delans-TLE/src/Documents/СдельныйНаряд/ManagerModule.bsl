...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаСдельныйНаряд, СтруктураДополнительныеСвойства) Экспорт
	...
	|	ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка) КАК Характеристика,
	|	СУММА(ТаблицаОперации.Нормочасы) КАК Количество
	|ИЗ
	...
	|	И ТаблицаОперации.ЗаказНаПроизводство.ЗапланированыОперации
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаОперации.Период,
	|	ТаблицаОперации.ЗаказНаПроизводство,
	...
	ТаблицаИтогов = МассивРезультатов[10].Выгрузить();
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗаказыНаПроизводство", МассивРезультатов[8].Выгрузить());
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати)
	...
	Для каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		ИмяОбластиОснова = ?(Шапка.ПоложениеИсполнителя=Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти, "ОсноваИсполнитель", "Основа");
