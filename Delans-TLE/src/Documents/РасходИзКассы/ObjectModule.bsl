...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	//EFSOL_oks_kry_25.03.2015 13:37:26_BEGIN
	Если ТипЗнч(ДанныеЗаполнения)= Тип("ДокументСсылка.ES_Возвраты") Тогда
		ЗаполнитьПоESВозвраты(ДанныеЗаполнения);
	ИначеЕсли ТипЗнч(ДанныеЗаполнения)= Тип("ДокументСсылка.ES_ОтчетОДоставке") Тогда
		ЗаполнитьПоESОтчетОДоставке(ДанныеЗаполнения);
	КонецЕсли;
	//EFSOL_oks_kry_25.03.2015 13:37:26_END
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Налоги Тогда
		...
		//EFSOL_oks_kry_25.03.2015 13:27:24_BEGIN
	ИначеЕсли  ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.ES_ВозвратНаложенныхПлатежей тогда
		УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "Подотчетник");
		УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "Корреспонденция");
		УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "ВидНалога");
		УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "Подразделение");
		...
		//EFSOL_oks_kry_25.03.2015 13:27:24_END	
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//EFSOL_oks_kry_07.04.2015 9:49:24_BEGIN
	Если НЕ Отказ И ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.ES_ВозвратНаложенныхПлатежей Тогда
		ДвиженияПоРегистрамДоставки();
	КонецЕсли; 
	//EFSOL_oks_kry_07.04.2015 9:49:24_END
...
Процедура ПриЗаписи(Отказ)
	...
	Если НЕ ПометкаУдаления И ЗначениеЗаполнено(ДокументОснование) И СуммаДокумента > 0 Тогда
		Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ES_Возвраты") Тогда
			
			Если ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.ES_ВозвратНаложенныхПлатежей Тогда
				Для каждого СтрокаЗаказ Из ES_ВозвратыПлатежей Цикл
					...
					НоваяЗаписьПризнакУчета.Заказ 			= СтрокаЗаказ.Заказ;
					НоваяЗаписьПризнакУчета.РасходДенег 	= Истина;
				КонецЦикла; 
			КонецЕсли; 
...
//EFSOL_oks_kry_25.03.2015 13:38:13_BEGIN
Процедура ЗаполнитьПоESВозвраты(ДокументОснование) Экспорт
	
	Если ДокументОснование.ВидОплаты = Перечисления.ТипыДенежныхСредств.Безналичные Тогда
		ВызватьИсключение НСтр("ru = 'Нельзя ввести расход денег. В возврате указан не верный способ оплаты (тип денежных средств)!'");
	КонецЕсли; 
	...
	|	ES_ВозвратыПлатежи.СуммаКВозврату КАК СуммаПлатежа,
	//ЭР Несторук С.И. 11.08.2017 16:25:35 {
	|	ES_ВозвратыПлатежи.Ссылка.Касса,
	...
	|ВЫБРАТЬ
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(&Ссылка) КАК Основание,
	//ЭР Несторук С.И. 11.08.2017 16:50:30 {
	//|	ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходИзКассы.ES_ВозвратНаложенныхПлатежей) КАК ВидОперации,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходИзКассы.Прочее) КАК ВидОперации,
	//}ЭР Несторук С.И.
	|	&Ссылка КАК ДокументОснование,
	//ЭР Несторук С.И. 11.08.2017 16:51:49 { 
	//|	ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ES_ВозвратНаложенныхПлатежей) КАК Статья,
	|	ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.Прочее) КАК Статья,
	//}ЭР Несторук С.И.
		|	ТаблицаДокумента.Организация КАК Организация,
	...
	|	ТаблицаДокумента.Заказчик КАК Контрагент,
	|	ЕСТЬNULL(ТаблицаДокумента.Договор, ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)) КАК Договор,
	|	ВЫРАЗИТЬ(ТаблицаДокумента.СуммаДокумента * КурсыВалютУчета.Курс * 1 / (1 * ЕСТЬNULL(КурсыВалютУчета.Кратность, 1)) КАК ЧИСЛО(15, 2)) КАК СуммаРасчетов,
	|	ТаблицаДокумента.СуммаДокумента КАК СуммаПлатежа,
	|	ТаблицаДокумента.СуммаДокумента КАК СуммаДокумента,
	|	ТаблицаДокумента.ВидОплаты КАК ВидОплаты,
	|	ТаблицаДокумента.Касса
	|ИЗ
	|	Документ.ES_Возвраты КАК ТаблицаДокумента
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&Дата, Валюта = &КонстантаВалютаУчета) КАК КурсыВалютУчета
	|		ПО (ИСТИНА)
	|ГДЕ
	|	ТаблицаДокумента.Ссылка = &Ссылка";
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, Выборка);
		 //ЭР Несторук С.И. 11.08.2017 16:27:19 {
		//Корреспонденция 	= ПланыСчетов.Управленческий.ES_НаложенныеПлатежи;
         Корреспонденция 	= ПланыСчетов.Управленческий.РасчетыСПрочимиПоставщикамиИТаможеннымиОрганами;
		 //}ЭР Несторук С.И.
		НалогообложениеНДС 	= УправлениеНебольшойФирмойСервер.НалогообложениеНДС(Организация, , Дата);
	...
	// Заполнение ТЧ по выдачам
	РезультатПоЗаказам = МассивРезультатов[0];
	Если НЕ РезультатПоЗаказам.Пустой() Тогда
		ВыборкаЗаказ = РезультатПоЗаказам.Выбрать();
		СуммаРасчетов = 0;
		//ЭР Несторук С.И. 11.08.2017 16:30:15 {
		Пока ВыборкаЗаказ.Следующий() Цикл
		//	ЗаполнитьЗначенияСвойств(ES_ВозвратыПлатежей.Добавить(), ВыборкаЗаказ);
			 //НоваяСтрока = РасшифровкаПлатежа.Добавить();
			 //НоваяСтрока.СуммаРасчетов = ВыборкаЗаказ.СуммаПлатежа;
			 СуммаРасчетов = СуммаРасчетов + ВыборкаЗаказ.СуммаПлатежа; 
		 КонецЦикла;
		 
		 Если РасшифровкаПлатежа.Количество() = 0 Тогда
			 НоваяСтрока = РасшифровкаПлатежа.Добавить();
			 НоваяСтрока.СуммаРасчетов 	= СуммаРасчетов;
			 НоваяСтрока.СуммаПлатежа	= СуммаРасчетов;
		 Иначе
			 РасшифровкаПлатежа[0].СуммаРасчетов 	= СуммаРасчетов;
			 РасшифровкаПлатежа[0].СуммаПлатежа 	= СуммаРасчетов;
		 КонецЕсли;
		 
		 Контрагент = ВыборкаЗаказ.Заказчик;
		 Касса 		= ВыборкаЗаказ.Касса;
		//}ЭР Несторук С.И.
...
Процедура ЗаполнитьПоESОтчетОДоставке(ДокументОснование)
	
	ДокументОснование = ДокументОснование;
	ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Подотчетнику;
	Подотчетник = ДокументОснование.Курьер;
...
Процедура ДвиженияПоРегистрамДоставки()
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	РасходИзКассыES_ВозвратыПлатежей.Заказ,
		|	РасходИзКассыES_ВозвратыПлатежей.СуммаПлатежа
		|ИЗ
		|	Документ.РасходИзКассы.ES_ВозвратыПлатежей КАК РасходИзКассыES_ВозвратыПлатежей
		|ГДЕ
		|	РасходИзКассыES_ВозвратыПлатежей.Ссылка = &Ссылка";
