...
Функция ТекстЗапросаИнициализироватьДанныеДокумента()
	...
	|	КорректировкаРеализацииЗапасы.НоменклатураНабора,
	|	КорректировкаРеализацииЗапасы.ХарактеристикаНабора
...
Функция ДанныеДокументовРегУчет(МассивОбъектов, ИспользоватьФаксимиле, ПечатнаяФормаТолькоВРублях = Истина, Ошибки = Неопределено) Экспорт
	...
	|	,Выбор когда КорректировкаРеализации.Организация.ГоловнаяОрганизация <> Значение(Справочник.Организации.ПустаяСсылка)
	|		Тогда КорректировкаРеализации.Организация.ГоловнаяОрганизация
	|		Иначе КорректировкаРеализации.Организация Конец КАК Организация
	...
	|		Иначе КорректировкаРеализации.Организация Конец КАК ОбособленноеПодразделениеПоставщика
	...
	|	,Выбор Когда КорректировкаРеализации.НалогообложениеНДС = Значение(Перечисление.ТипыНалогообложенияНДС.НеОблагаетсяНДС)
	|		Тогда 2
	...
	|				И КорректировкаРеализации.Подразделение.ГоловнаяОрганизация = КорректировкаРеализации.Организация
	|			Тогда КорректировкаРеализации.Подразделение
	|		Иначе КорректировкаРеализации.Организация Конец КАК Грузоотправитель
	|	,Выбор когда КорректировкаРеализации.Контрагент.ГоловнойКонтрагент <> Значение(Справочник.Контрагенты.ПустаяСсылка)
	...
	|		Иначе КорректировкаРеализации.Контрагент Конец КАК Контрагент
	|	,Выбор когда КорректировкаРеализации.Контрагент.ГоловнойКонтрагент <> Значение(Справочник.Контрагенты.ПустаяСсылка)
	|			И КорректировкаРеализации.Контрагент.ГоловнойКонтрагент.СтранаРегистрации = Значение(Справочник.СтраныМира.Россия)
	|		Тогда КорректировкаРеализации.Контрагент.ГоловнойКонтрагент
	|		Иначе КорректировкаРеализации.Контрагент Конец КАК Покупатель
	|	,Выбор когда КорректировкаРеализации.Грузополучатель.ГоловнойКонтрагент <> Значение(Справочник.Контрагенты.ПустаяСсылка)
	...
	|		Иначе КорректировкаРеализации.Контрагент Конец КАК ОбособленноеПодразделениеПокупателя
	...
	|		Тогда КорректировкаРеализации.Грузополучатель
	|		Иначе КорректировкаРеализации.Контрагент Конец КАК Грузополучатель
	...
	|		Иначе КорректировкаРеализации.ВалютаДокумента Конец КАК ВалютаДокумента
	|	,Выбор когда &ПечатнаяФормаТолькоВРублях = Истина 
	|		Тогда НациональнаяВалюта.Значение.НаименованиеПолное
	|		Иначе КорректировкаРеализации.ВалютаДокумента.НаименованиеПолное Конец КАК ВалютаНаименование
	|	,Выбор когда &ПечатнаяФормаТолькоВРублях = Истина 
	|		Тогда НациональнаяВалюта.Значение.Код
	|		Иначе КорректировкаРеализации.ВалютаДокумента.Код Конец КАК ВалютаКод
	...
	|	,ВложенныйЗапрос.СтранаСсылка
	|	,ВложенныйЗапрос.СтранаПредставление
	|	,ВложенныйЗапрос.СтранаКод
	|	,ВложенныйЗапрос.ПредставлениеГТД
	|	,ВложенныйЗапрос.Вес
	...
	|			Тогда
	|				Выбор Когда КорректировкаРеализацииЗапасы.Ссылка.ВалютаДокумента = &НациональнаяВалюта
	...
	|					Иначе
	|						Выбор Когда КорректировкаРеализацииЗапасы.Количество = 0
	|							Тогда (СуммыДокументовРегламентированныйУчет.Всего - СуммыДокументовРегламентированныйУчет.Всего)
	|							Иначе Выразить((СуммыДокументовРегламентированныйУчет.Всего - СуммыДокументовРегламентированныйУчет.Всего) / КорректировкаРеализацииЗапасы.Количество КАК ЧИСЛО(15, 2)) Конец
	|				Конец
	|			Иначе КорректировкаРеализацииЗапасы.Цена Конец КАК Цена
	|		,Выбор когда &ПечатнаяФормаТолькоВРублях 
	|			Тогда
	|				Выбор Когда КорректировкаРеализацииЗапасы.Ссылка.ВалютаДокумента = &НациональнаяВалюта
	|						Тогда КорректировкаРеализацииЗапасы.Сумма
	...
	|					Иначе
	|						Выбор Когда КорректировкаРеализацииЗапасы.Ссылка.СуммаВключаетНДС
	|							Тогда СуммыДокументовРегламентированныйУчет.Всего
	|							Иначе Выразить(СуммыДокументовРегламентированныйУчет.Всего / КорректировкаРеализацииЗапасы.Количество КАК ЧИСЛО(15, 2)) Конец
	|				Конец
	|			Иначе КорректировкаРеализацииЗапасы.Сумма Конец КАК Сумма
	|		,Выбор когда &ПечатнаяФормаТолькоВРублях 
	|			Тогда
	|				Выбор Когда КорректировкаРеализацииЗапасы.Ссылка.ВалютаДокумента = &НациональнаяВалюта
	|						Тогда КорректировкаРеализацииЗапасы.СуммаНДС
	...
	|				Конец
	|			Иначе КорректировкаРеализацииЗапасы.СуммаНДС Конец КАК СуммаНДС
	|		,Выбор когда &ПечатнаяФормаТолькоВРублях 
	|			Тогда
	|				Выбор Когда КорректировкаРеализацииЗапасы.Ссылка.ВалютаДокумента = &НациональнаяВалюта
	|						Тогда КорректировкаРеализацииЗапасы.Всего
	...
	|				Конец
	|			Иначе КорректировкаРеализацииЗапасы.Всего Конец КАК Всего,
	|		0 КАК МассаБрутто,
...
Функция ТекстЗапросаДанныеДляПечатиКорректировочныхСчетовФактур(НомераТаблиц) Экспорт
	...
	|	Реквизиты.Ссылка,
	|	Реквизиты.Дата,
	|	Выбор когда Реквизиты.Организация.ГоловнаяОрганизация <> Значение(Справочник.Организации.ПустаяСсылка)
	|		Тогда Реквизиты.Организация.ГоловнаяОрганизация
	|		Иначе Реквизиты.Организация Конец КАК Организация,
	...
	|	Выбор когда Реквизиты.Контрагент.ГоловнойКонтрагент <> Значение(Справочник.Контрагенты.ПустаяСсылка)
	...
	|		Иначе Реквизиты.Контрагент Конец КАК Контрагент,
