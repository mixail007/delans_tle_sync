...
Процедура ЗаполнитьПоПлатежноеПоручение(ДокументСсылкаПлатежноеПоручение) Экспорт
	...
	|		КОГДА ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПлатежноеПоручение.ПрочиеРасчеты)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.ПрочиеРасчеты)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.ПереводНаДругойСчет)
	|	КОНЕЦ,
	...
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ОплатаПоставщикам)
	|		КОГДА ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПлатежноеПоручение.ПеречислениеНалога)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.Прочее)
	|		КОГДА ТаблицаДокумента.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПлатежноеПоручение.ЛичныеСредстваПредпринимателя)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ЛичныеСредстваПредпринимателя)
	|		КОГДА ТаблицаДокумента.Статья <> ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ПустаяСсылка)
	|			ТОГДА ТаблицаДокумента.Статья
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.Прочее)
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.Статья = ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ПустаяСсылка)
	...
	Если ЭтотОбъект.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Налоги Тогда
		ЭтотОбъект.Контрагент = Неопределено;
	ИначеЕсли ЭтотОбъект.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Зарплата Тогда
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Налоги Тогда
		...
		//EFSOL_oks_kry_06.04.2015 15:22:32_BEGIN
	ИначеЕсли  ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ES_ВозвратНаложенныхПлатежей тогда
		УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "Подотчетник");
		УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "Корреспонденция");
		УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "ВидНалога");
		УправлениеНебольшойФирмойСервер.УдалитьПроверяемыйРеквизит(ПроверяемыеРеквизиты, "ДоговорКредитаЗайма");
		...
		//EFSOL_oks_kry_06.04.2015 15:22:32_END
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если НЕ Отказ И ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ES_ВозвратНаложенныхПлатежей Тогда
...
Процедура ПриЗаписи(Отказ)
	...
	Если НЕ ПометкаУдаления И ЗначениеЗаполнено(ДокументОснование) И СуммаДокумента > 0 Тогда
		Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ES_Возвраты") Тогда
			...
			Если ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ES_ВозвратНаложенныхПлатежей Тогда
...
// Процедура заполнения документа на основании возврата платежей.
//
// Параметры:
//	ДокументОснование - ДокументСсылка.ES_ВозвратуПлатежейИОбъектов
//	ДанныеЗаполнения - Структура - Данные заполнения документа.
//	
Процедура ЗаполнитьПоESВозвраты(ДокументОснование) Экспорт
	
	Если ДокументОснование.ВидОплаты = Перечисления.ТипыДенежныхСредств.Наличные Тогда
	...
	|	ES_ВозвратыПлатежи.Ссылка.Заказчик КАК Контрагент,
	|	ES_ВозвратыПлатежи.Ссылка.Счет
	|ИЗ
	...
	|ВЫБРАТЬ
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(&Ссылка) КАК Основание,
	//ЭР Несторук С.И. 11.08.2017 16:05:58 {
	//|	ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.ES_ВозвратНаложенныхПлатежей) КАК ВидОперации,
	...
	|	ТаблицаДокумента.СуммаДокумента КАК СуммаПлатежа,
	|	ТаблицаДокумента.СуммаДокумента КАК СуммаДокумента,
	|	ТаблицаДокумента.Счет КАК БанковскийСчет
	|ИЗ
	...
	Если НЕ РезультатПоЗаказам.Пустой() Тогда
		...
		//ЭР Несторук С.И. 11.08.2017 16:16:53 {
		ЭтотОбъект.Корреспонденция 	= ПланыСчетов.Управленческий.РасчетыСПрочимиПоставщикамиИТаможеннымиОрганами;
		ЭтотОбъект.Контрагент 		= ВыборкаЗаказ.Контрагент;
		ЭтотОбъект.БанковскийСчет  	= ВыборкаЗаказ.Счет;
		//}ЭР Несторук С.И.
	КонецЕсли; 
...
Процедура ДвиженияПоРегистрамДоставки()
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	РасходСоСчетаES_ВозвратыПлатежей.Заказ,
		|	РасходСоСчетаES_ВозвратыПлатежей.СуммаПлатежа
		|ИЗ
		|	Документ.РасходСоСчета.ES_ВозвратыПлатежей КАК РасходСоСчетаES_ВозвратыПлатежей
		|ГДЕ
		|	РасходСоСчетаES_ВозвратыПлатежей.Ссылка = &Ссылка";
