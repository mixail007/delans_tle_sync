...
Функция ДанныеДокументовРегУчет(МассивОбъектов, ИспользоватьФаксимиле, ПечатнаяФормаТолькоВРублях = Истина, Ошибки = Неопределено) Экспорт
	...
	|		Иначе ОтчетКомиссионера.Организация Конец КАК Организация
	...
	|		Иначе ОтчетКомиссионера.Организация Конец КАК ОбособленноеПодразделениеПоставщика
	|	,Выбор когда ОтчетКомиссионера.Организация.ЮридическоеФизическоеЛицо = Значение(Перечисление.ЮридическоеФизическоеЛицо.ФизическоеЛицо) 
	|		Тогда Истина
	...
	|		Иначе ОтчетКомиссионера.Контрагент Конец КАК Контрагент
	...
	|		,Выбор когда (ВЫРАЗИТЬ(ОтчетКомиссионера.Запасы.Номенклатура.НаименованиеПолное КАК СТРОКА(1000))) = """"
	|			Тогда ОтчетКомиссионера.Запасы.Номенклатура.Наименование
	|			Иначе ВЫРАЗИТЬ(ОтчетКомиссионера.Запасы.Номенклатура.НаименованиеПолное КАК СТРОКА(1000)) Конец КАК ПредставлениеНоменклатуры
	...
	|		,Выбор когда ОтчетКомиссионера.Запасы.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	...
	|			Тогда 
	|				Выбор когда ОтчетКомиссионера.Ссылка.Договор.РасчетыВУсловныхЕдиницах И ОтчетКомиссионера.Ссылка.ВалютаДокумента <> &НациональнаяВалюта
	|					Тогда Выразить(ОтчетКомиссионера.Запасы.Цена * ОтчетКомиссионера.Курс / ОтчетКомиссионера.Кратность КАК ЧИСЛО(15, 2))
	|					Иначе ОтчетКомиссионера.Запасы.Цена Конец
	|			Иначе ОтчетКомиссионера.Запасы.Цена Конец КАК Цена
	|		,Выбор когда &ПечатнаяФормаТолькоВРублях
	|			Тогда
	|				Выбор когда ОтчетКомиссионера.Ссылка.Договор.РасчетыВУсловныхЕдиницах И ОтчетКомиссионера.Ссылка.ВалютаДокумента <> &НациональнаяВалюта
	|					Тогда Выразить(ОтчетКомиссионера.Запасы.Сумма * ОтчетКомиссионера.Курс / ОтчетКомиссионера.Кратность КАК ЧИСЛО(15, 2))
	|					Иначе ОтчетКомиссионера.Запасы.Сумма Конец
	|			Иначе ОтчетКомиссионера.Запасы.Сумма Конец КАК Сумма
	|		,Выбор когда &ПечатнаяФормаТолькоВРублях
	|			Тогда
	|				Выбор когда ОтчетКомиссионера.Ссылка.Договор.РасчетыВУсловныхЕдиницах И ОтчетКомиссионера.Ссылка.ВалютаДокумента <> &НациональнаяВалюта
	|					Тогда Выразить(ОтчетКомиссионера.Запасы.СуммаНДС * ОтчетКомиссионера.Курс / ОтчетКомиссионера.Кратность КАК ЧИСЛО(15, 2))
	|					Иначе ОтчетКомиссионера.Запасы.СуммаНДС Конец
	|			Иначе ОтчетКомиссионера.Запасы.СуммаНДС Конец КАК СуммаНДС
	|		,Выбор когда &ПечатнаяФормаТолькоВРублях
	|			Тогда
	|				Выбор когда ОтчетКомиссионера.Ссылка.Договор.РасчетыВУсловныхЕдиницах И ОтчетКомиссионера.Ссылка.ВалютаДокумента <> &НациональнаяВалюта
	|					Тогда Выразить(ОтчетКомиссионера.Запасы.Всего * ОтчетКомиссионера.Курс / ОтчетКомиссионера.Кратность КАК ЧИСЛО(15, 2))
	|					Иначе ОтчетКомиссионера.Запасы.Всего Конец
	|			Иначе ОтчетКомиссионера.Запасы.Всего Конец КАК Всего
	...
	|Упорядочить по ОтчетКомиссионера.Запасы.НомерСтроки";
