...
Процедура СформироватьТаблицаЗакупкиДляКУДиР(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Если ЗначениеЗаполнено(ДокументСсылкаРасходнаяНакладная.ДокументОснование) Тогда
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ТаблицаЗапасы.Период,
...
Функция УниверсальныйЗапросПоДаннымДокумента(МассивОбъектов, ИспользоватьФаксимиле, ИспользоватьНаборы)
	...
	|	,РасходнаяНакладная.Ответственный.Физлицо КАК ФизическоеЛицоОтветсвенного
...
Функция ДанныеДокументовРегУчет(МассивОбъектов, ИспользоватьФаксимиле, ПечатнаяФормаТолькоВРублях = Истина, Ошибки = Неопределено) Экспорт
	...
	|						Иначе Выразить((СуммыДокументовРегламентированныйУчет.Всего - СуммыДокументовРегламентированныйУчет.НДС) / РасходнаяНакладнаяЗапасы.Количество КАК ЧИСЛО(15, 2)) Конец
	|			Конец
...
Процедура СформироватьТоварныйЧек(ТабличныйДокумент, ТекущийДокумент)
	...
	Для каждого Шапка Из ДанныеДокументов Цикл
		...
		СтруктураИтогов = Новый Структура("Сумма, СуммаНДС, Всего, Количество, НомерСтроки, ВсегоСкидок, ВсегоБезСкидок, ЕстьСкидки", 0, 0, 0, 0, 0, 0, 0, ЕстьСкидки);
		ПараметрыНоменклатуры = Новый Структура;
		...
		СтруктураЗаполненияСекции.Вставить("Сумма", УправлениеНебольшойФирмойСервер.ФорматСумм(СтруктураИтогов.Сумма));
		СтруктураЗаполненияСекции.Вставить("Всего", УправлениеНебольшойФирмойСервер.ФорматСумм(СтруктураИтогов.Всего));
		...
		Иначе
			...
			СтруктураЗаполненияСекции.Вставить("СуммаНДС", УправлениеНебольшойФирмойСервер.ФорматСумм(СтруктураИтогов.СуммаНДС));
		...
		СтруктураЗаполненияСекции.Вставить("ИтоговаяСтрока", 
			НСтр("ru ='Всего наименований '") + Строка(СтруктураИтогов.Количество) + НСтр("ru =', на сумму '") + УправлениеНебольшойФирмойСервер.ФорматСумм(СуммаКПрописи, Шапка.ВалютаДокумента));
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ИмяМакета, ПараметрыПечати)
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		//EFSOL_oks_kry_23.04.2015 17:04:06_BEGIN
		ИначеЕсли ИмяМакета = "ПФ_MXL_ES_АктОбОказанииУслуг" Тогда
			Если ТекущийДокумент.ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ПродажаПокупателю Тогда
				СформироватьESАктОбОказанииУслугДоставка(ТабличныйДокумент, ТекущийДокумент, Ошибки)
			Иначе
				Сообщить(Строка(ТекущийДокумент) + ": ""Акт об оказании услуг (Доставка)"" можно печатать только для документа с видом операции ""Продажа покупателю""");
				Продолжить;
			КонецЕсли; 
		//EFSOL_oks_kry_23.04.2015 17:04:06_END	
		
		//ЕФСОЛ Несторук 24.08.2016 +
		 ИначеЕсли ИмяМакета = "ПФ_MXL_ES_СчетНаОплату" Тогда
			Если ТекущийДокумент.ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ПродажаПокупателю Тогда
				ПечатьESСчетНаОплату(ТабличныйДокумент, ТекущийДокумент, Ошибки)
			Иначе
				Сообщить(Строка(ТекущийДокумент) + ": ""Счет на оплату (Доставка)"" можно печатать только для документа с видом операции ""Продажа покупателю""");
				Продолжить;
			КонецЕсли; 
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	//ЭР Несторук С.И. 06.09.2018 10:00:31 {
	
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_ES_СчетНаОплату") Тогда
		
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ПФ_MXL_ES_СчетНаОплату", "Счет на оплату (Доставка)", ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, "ПФ_MXL_ES_СчетНаОплату", ПараметрыПечати));
		
	КонецЕсли;
	
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_ES_АктОбОказанииУслуг") Тогда
		
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ПФ_MXL_ES_АктОбОказанииУслуг", "Акт об оказании услуг (Доставка)", ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, "ПФ_MXL_ES_АктОбОказанииУслуг", ПараметрыПечати));
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	//EFSOL_oks_kry_23.04.2015 17:01:10_BEGIN
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ПФ_MXL_ES_АктОбОказанииУслуг";
	КомандаПечати.Представление = НСтр("ru = 'Акт об оказании услуг (Доставка)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 5;
	//EFSOL_oks_kry_23.04.2015 17:01:10_END
	
	//ЕФСОЛ Несторук 24.08.2016 +
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ПФ_MXL_ES_СчетНаОплату";
	КомандаПечати.Представление = НСтр("ru = 'Счет на оплату (Доставка)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
...
КонецПроцедуры
// Процедура формирования печатной формы Акта об оказании услуг (Доставка)
//
Функция СформироватьESАктОбОказанииУслугДоставка(ТабличныйДокумент, ТекущийДокумент, Ошибки)
	...
	|	РасходнаяНакладная.Номер,
	|	РасходнаяНакладная.Организация.Префикс КАК Префикс,
	|	РасходнаяНакладная.Ответственный КАК ОтветственныйПредставление,
	|	РасходнаяНакладная.Запасы.(
	|		НомерСтроки КАК НомерСтроки,
	|		ВЫБОР
	|			КОГДА (ВЫРАЗИТЬ(РасходнаяНакладная.Запасы.Номенклатура.НаименованиеПолное КАК СТРОКА(100))) = """"
	...
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_РасходнаяНакладная_ES_АктОбОказанииУслуг";
	
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.РасходнаяНакладная.ПФ_MXL_ES_АктОбОказанииУслуг");
	СведенияОбОрганизации = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Шапка.Организация, Шапка.ДатаДокумента, ,);
	СведенияОбКонтрагенте = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Шапка.Контрагент, Шапка.ДатаДокумента, ,);
	...
	//EFSOL_oks_kry_23.04.2015 18:17:16_BEGIN
	ВыборкаЗапасы = Шапка.Запасы.Выгрузить();
	ВыборкаЗапасы.Свернуть("Артикул, Характеристика,Запас, ЕдиницаХранения, ЕстьСкидка, ПроцентСкидкиНаценки, ТипНоменклатуры, Цена, Содержание", "Сумма, СуммаНДС, Всего, Количество");
	
	Для каждого ВыборкаСтрокЗапасы Из ВыборкаЗапасы Цикл
	...
	//EFSOL_oks_kry_30.03.2015 19:31:07_BEGIN
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ВЫБОР
	               |		КОГДА ES_ДанныеПоЗаказамСрезПоследних.ОбщийВес > ES_ДанныеПоЗаказамСрезПоследних.ОбщийОбъемныйВес
	               |			ТОГДА ES_ДанныеПоЗаказамСрезПоследних.ОбщийВес
	               |		ИНАЧЕ ES_ДанныеПоЗаказамСрезПоследних.ОбщийОбъемныйВес
	               |	КОНЕЦ КАК ВесМакс,
	               |	ES_ДанныеПоЗаказамСрезПоследних.ДатаОтгрузки КАК ДатаОтгрузки,
	               ...
	               |	ПРЕДСТАВЛЕНИЕ(ВЫБОР
	               |			КОГДА ES_ДанныеПоЗаказамСрезПоследних.ЗаборКонтактноеЛицо = &СтрПуста
	               |				ТОГДА ES_ДанныеПоЗаказамСрезПоследних.ЗаказчикКонтактноеЛицо
	               |			ИНАЧЕ ES_ДанныеПоЗаказамСрезПоследних.ЗаборКонтактноеЛицо
	               |		КОНЕЦ) КАК ЗаборКонтактноеЛицо,
	               ...
	               |	ES_ДанныеПоЗаказамСрезПоследних.Заказ.ES_Расходы КАК Расходы,
	               |	ВложенныйЗапрос.КЛ КАК КЛ,
	               |	ВложенныйЗапрос.Фир КАК ЗаборФирма,
	               |	ES_ДанныеПоЗаказамСрезПоследних.Получатель КАК Получатель,
	               |	ES_ДанныеПоЗаказамСрезПоследних.ОбщийВес КАК Вес,
	               |	ES_ДанныеПоЗаказамСрезПоследних.ОбщийОбъемныйВес КАК ВесОбъемный
	               |ИЗ
	               ...
	               |					Расходная.Заказ
	               |				ИЗ
	               |					Расходная КАК Расходная)) КАК ES_ДанныеПоЗаказамСрезПоследних
	               ...
	               |						Расходная.Заказ
	               |					ИЗ
	               |						Расходная КАК Расходная)) КАК ES_СтатусыЗаказовСрезПоследних
	               ...
	               |			ПРЕДСТАВЛЕНИЕ(ВЫБОР
	               |					КОГДА ES_ДанныеПоЗаказамСрезПоследних.ЗаборКонтактноеЛицо = &СтрПуста
	               |						ТОГДА ПРЕДСТАВЛЕНИЕ(ES_ДанныеПоЗаказамСрезПоследних.Заказчик)
	               |					ИНАЧЕ ES_ДанныеПоЗаказамСрезПоследних.ЗаборОтправитель
	               |				КОНЕЦ) КАК Фир,
	               |			ES_ДанныеПоЗаказамСрезПоследних.Заказ КАК Заказ
	               |		ИЗ
	               |			РегистрСведений.ES_ДанныеПоЗаказам.СрезПоследних(
	               ...
	               |							Расходная.Заказ
	               |						ИЗ
	               |							Расходная КАК Расходная)) КАК ES_ДанныеПоЗаказамСрезПоследних) КАК ВложенныйЗапрос
	               |		ПО ES_ДанныеПоЗаказамСрезПоследних.Заказ = ВложенныйЗапрос.Заказ
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	ДатаДоставки";
			
	Запрос.УстановитьПараметр("Ссылка", ТекущийДокумент);
	Запрос.УстановитьПараметр("СтрПуста","");
	РезультатЗапроса = Запрос.Выполнить();
	Если НЕ РезультатЗапроса.Пустой() Тогда
		
		ОбластьШапкаПеречень = Макет.ПолучитьОбласть("ШапкаПеречень");
		ТабличныйДокумент.Вывести(ОбластьШапкаПеречень);
		
		ОбластьСтрокаПеречень = Макет.ПолучитьОбласть("СтрокаПеречень");
		ВыводитьИтоговуюСтоимость = Ложь;
		ИтоговаяСтоимость = 0;
		НомерСтроки = 0;
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			НомерСтроки = НомерСтроки + 1;
			ВыводитьИтоговуюСтоимость = Истина;
			ИтоговаяСтоимость = ИтоговаяСтоимость + ВыборкаДетальныеЗаписи.СтоимостьДоставки;
			
			ОбластьСтрокаПеречень.Параметры.Заполнить(ВыборкаДетальныеЗаписи);
			ОбластьСтрокаПеречень.Параметры.НомерСтроки = НомерСтроки;
			ТабличныйДокумент.Вывести(ОбластьСтрокаПеречень);
		КонецЦикла;
		
		Если ВыводитьИтоговуюСтоимость Тогда
			ОбластьИтоговаяСтоимость = Макет.ПолучитьОбласть("ИтогоПеречень");
			ОбластьИтоговаяСтоимость.Параметры.ИтогоСтоимость = ИтоговаяСтоимость;
			ТабличныйДокумент.Вывести(ОбластьИтоговаяСтоимость);
		КонецЕсли; 
	КонецЕсли; 
	
КонецФункции // СформироватьАктОбОказанииУслуг()

Функция ПечатьESСчетНаОплату(ТабличныйДокумент, ТекущийДокумент, Ошибки)
	
	
	Организация = ES_ОбщегоНазначения.ПолучитьСтартовуюНастройку(Перечисления.ES_ВидыСтартовыхНастроек.Организация);
	СведенияОбОрганизации = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Организация, ТекущаяДата(), ,);
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_РасходнаяНакладная_ES_АктОбОказанииУслуг";
	...
	//|	НомерСтроки";
		...
		|	РасходнаяНакладнаяЗапасы.ЕдиницаИзмерения КАК ЕдиницаХранения,
		|	СУММА(РасходнаяНакладнаяЗапасы.СуммаНДС) КАК СуммаНДС,
		|	СУММА(РасходнаяНакладнаяЗапасы.ПроцентСкидкиНаценки) КАК ПроцентСкидкиНаценки,
		|	СУММА(РасходнаяНакладнаяЗапасы.СуммаАвтоматическойСкидки) КАК Скидка,
		|	СУММА(РасходнаяНакладнаяЗапасы.Сумма) КАК Сумма,
		|	МИНИМУМ(РасходнаяНакладнаяЗапасы.Количество) КАК Количество,
		|	РасходнаяНакладнаяЗапасы.Ссылка,
		|	РасходнаяНакладнаяЗапасы.Номенклатура.Артикул КАК Артикул,
		|	РасходнаяНакладнаяЗапасы.Номенклатура.Код КАК Код,
		...
		|	ВТ_Запасы.Запас,
		|	ВТ_Запасы.Цена,
		|	ВТ_Запасы.ЕдиницаХранения,
		|	ВТ_Запасы.СуммаНДС,
		|	ВТ_Запасы.ПроцентСкидкиНаценки,
		|	ВТ_Запасы.Скидка КАК Скидка,
		|	ВТ_Запасы.Сумма,
		...
		|	РасходнаяНакладная.Организация.Префикс КАК Префикс,
		|	РасходнаяНакладная.ВалютаДокумента КАК ВалютаДокумента,
		|	РасходнаяНакладная.ПодписьРуководителя.РасшифровкаПодписи КАК ФИОРуководителя,
		|	РасходнаяНакладная.ПодписьГлавногоБухгалтера.РасшифровкаПодписи КАК ФИОБухгалтера,
		|	ВТ_Запасы.Артикул,
		|	ВТ_Запасы.Код,
		|	ВТ_Запасы.Всего,
		|	РасходнаяНакладная.СуммаВключаетНДС КАК СуммаВключаетНДС
		...
		|		ПО ВТ_Запасы.Ссылка = РасходнаяНакладная.Ссылка
		|ИТОГИ
		|	СУММА(Скидка),
		|	МАКСИМУМ(Номер),
	...
	//
	//ВыборкаСтрокЗапасы = Шапка.Запасы.Выбрать();
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_РасходнаяНакладная_ES_СчетНаОплату";
	
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.РасходнаяНакладная.ПФ_MXL_ES_СчетНаОплату");
	СведенияОбОрганизации = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Шапка.Организация, Шапка.ДатаДокумента, ,);
	СведенияОбКонтрагенте = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Шапка.Контрагент, Шапка.ДатаДокумента, ,);
	...
	//ОбластьМакета.Параметры.БанкПолучателяПредставление = "" + СведенияОбОрганизации.Банк+ " " +СведенияОбОрганизации.Банк.Город ;
	//ОбластьМакета.Параметры.ИНН							= СведенияОбОрганизации.ИНН;
	//ОбластьМакета.Параметры.КПП							= СведенияОбОрганизации.КПП;
	//ОбластьМакета.Параметры.БИКБанкаПолучателя			= СведенияОбОрганизации.Банк.Код;
	//ОбластьМакета.Параметры.СчетБанкаПолучателяПредставление	=	СведенияОбОрганизации.КоррСчет;
	//ОбластьМакета.Параметры.ПредставлениеПоставщика				=	СведенияОбОрганизации.Представление;
	...
	ОбластьМакета = Макет.ПолучитьОбласть("Заголовок");
	ОбластьМакета.Параметры.ТекстЗаголовка = "Счет на оплату № " + НомерДокумента + " от " + Формат(Шапка.ДатаДокумента, "ДЛФ=DD");
	ТабличныйДокумент.Вывести(ОбластьМакета);
	...
	//EFSOL_oks_kry_23.04.2015 18:17:16_END
	
	//EFSOL_Шаповал Олег_24.09.2015_BEGIN
		//Если ВыборкаСтрокЗапасы.ТипНоменклатуры <> Перечисления.ТипыНоменклатуры.Услуга Тогда
		//	
		//	Продолжить;
		//	
		//КонецЕсли;
	//EFSOL_Шаповал Олег_24.09.2015_END
	...
	//ОбластьМакета.Параметры.ФИОРуководителя = Шапка.ФИОРуководителя;
	//ОбластьМакета.Параметры.ФИОБухгалтера   = Шапка.ФИОГлавногоБухгалтера;
	...
	//ЭР Несторук С.И. 17.08.2017 10:22:17 {
	//Скрыл вывод таблицы с расшифровкой, закомментил код ниже 
	//}ЭР Несторук С.И.
	//EFSOL_oks_kry_30.03.2015 19:31:07_BEGIN
	//Запрос = Новый Запрос;
	//Запрос.Текст = 
	...
	//	КонецЕсли; 
	//КонецЕсли; 
	//EFSOL_oks_kry_30.03.2015 19:31:07_END
