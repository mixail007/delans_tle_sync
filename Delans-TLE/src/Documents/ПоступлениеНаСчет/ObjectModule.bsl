...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	//ЭР Несторук С.И. 15.09.2017 10:00:58 {
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ES_Приемка") Тогда
		ES_ОбщегоНазначения.ЗаполнитьПоступлениеНаСчетПоПриемке(ЭтотОбъект, ДанныеЗаполнения);
	КонецЕсли; 
	//}ЭР Несторук С.И.
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтКурьерскойКомпанииПочты Тогда
		...
		//EFSOL_oks_kry_23.03.2015 20:49:06_BEGIN
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ES_ПриемНаложенныхПлатежей Тогда
		...
		//EFSOL_oks_kry_23.03.2015 20:49:06_END
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//EFSOL_oks_kry_06.04.2015 18:19:04_BEGIN
	Если НЕ Отказ И СуммаДокумента > 0 Тогда
		ДвиженияПоРегистрамДоставки();
	КонецЕсли; 
	//EFSOL_oks_kry_06.04.2015 18:19:04_END
...
Процедура ПриЗаписи(Отказ)
	...
	Если НЕ ПометкаУдаления И ЗначениеЗаполнено(ДокументОснование) И СуммаДокумента > 0 Тогда
		...
		Тогда
			...
			Если ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ES_ПриемНаложенныхПлатежей Тогда
				...
				НоваяЗаписьПризнакУчета.Заказ 			= ES_Заказ;
				НоваяЗаписьПризнакУчета.УчтеноНП 		= Истина;
				//НоваяЗаписьПризнакУчета.УчтеноСтоимость = Ложь;
			ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ОтПокупателя Тогда 	
				Для каждого СтрЗаказ Из РасшифровкаПлатежа Цикл
					НоваяЗаписьПризнакУчета = НаборЗаписейПризнакиУчета.Добавить();
					НоваяЗаписьПризнакУчета.Период 			= Дата;
					НоваяЗаписьПризнакУчета.Регистратор 	= Ссылка;
					НоваяЗаписьПризнакУчета.Документ 		= ДокументОснование;
					НоваяЗаписьПризнакУчета.Заказ 			= СтрЗаказ.Заказ;
					//НоваяЗаписьПризнакУчета.УчтеноНП 		= Ложь;
					НоваяЗаписьПризнакУчета.УчтеноСтоимость = Истина;
				КонецЦикла; 
			КонецЕсли; 
...
//EFSOL_oks_kry_23.03.2015 20:50:28_BEGIN
...
Процедура ДвиженияПоРегистрамДоставки()
	...
	Тогда
		...
		Если ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ES_ПриемНаложенныхПлатежей Тогда
			...
			Движение.Заказ 				= ES_Заказ;
			Движение.УчтеноНП 			= Истина;
			//Движение.УчтеноСтоимость 	= Ложь;
		ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПокупателя Тогда 	
			Для каждого СтрЗаказ Из РасшифровкаПлатежа Цикл
				...
				Движение.Регистратор 		= Ссылка;
				Движение.Документ 			= ДокументОснование;
				Движение.Заказ 				= СтрЗаказ.Заказ;
				//Движение.УчтеноНП 			= Ложь;
				Движение.УчтеноСтоимость 	= Истина;
			КонецЦикла; 
		КонецЕсли;
