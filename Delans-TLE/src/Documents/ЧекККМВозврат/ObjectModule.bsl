...
Процедура ЗаполнитьПоЧекуККМ(ДанныеЗаполнения) Экспорт
	...
	//ЭР Несторук С.И. 11.05.2017 10:11:13 {
	|        ,
	|		ES_ЭтоГруз,
	|		ES_Содержание
	//}ЭР Несторук С.И.
	...
	ЭтотОбъект.ДобавленныеНаборы.Загрузить(Выборка.ДобавленныеНаборы.Выгрузить());
	ЭтотОбъект.БезналичнаяОплата.Загрузить(Выборка.БезналичнаяОплата.Выгрузить());
	ЭтотОбъект.БонусныеБаллыКНачислению.Загрузить(Выборка.БонусныеБаллыКНачислению.Выгрузить());
	ЭтотОбъект.Предоплата.Загрузить(Выборка.Предоплата.Выгрузить());
	
	УдалитьОдноразовыеСертификаты(ЭтотОбъект.БезналичнаяОплата);
	УдалитьОплатуБонусами(ЭтотОбъект.БезналичнаяОплата);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Запасы.Итог("Всего") < БезналичнаяОплата.Итог("Сумма") Тогда
		ТекстСообщения = НСтр("ru = 'Сумма безналичной оплаты больше суммы товаров и услуг.'");
		УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
			ЭтотОбъект,
			ТекстСообщения,
			,,,
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//ЭР Несторук С.И. 15.08.2017 14:22:50 {
	ДвижениеПоРегиструНомеклатураВЗаказах();
	//}ЭР Несторук С.И.
...
//ЭР Несторук С.И. 15.08.2017 14:23:21 {
Процедура ДвижениеПоРегиструНомеклатураВЗаказах()
	
	Если ЗначениеЗаполнено(Заказ) Тогда
		Движения.ES_РННоменклатураВЗаказах.Записывать = Истина;
		Для каждого Стр ИЗ Запасы Цикл 
			Если Стр.ES_ЭтоГруз = Истина И Стр.Количество > 0 Тогда
				...
				Движение.Содержание		= Стр.ES_Содержание;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
