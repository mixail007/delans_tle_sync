...
Процедура ОбработкаНавигационнойСсылки(НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ИначеЕсли СтрНачинаетсяС(НавигационнаяСсылкаФорматированнойСтроки, "РедактироватьПериод") Тогда
		...
		ИначеЕсли ИмяРедактируемогоПоля = "ДатаПроизводства" Тогда
			ИнтеграцияВЕТИСКлиент.ОткрытьФормуРедактированияДатыПроизводства(ЭтотОбъект, Элементы.Товары.ТекущиеДанные.ДанныеВСД, ОбработчикОповещения, Истина);
		ИначеЕсли ИмяРедактируемогоПоля = "СрокГодности" Тогда
			ИнтеграцияВЕТИСКлиент.ОткрытьФормуРедактированияСрокаГодности(ЭтотОбъект, Элементы.Товары.ТекущиеДанные, ОбработчикОповещения, ТолькоПросмотр Или РедактированиеФормыНедоступно);
		КонецЕсли;
	КонецЕсли;		
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, ИдентификаторСтроки = Неопределено, СписокРеквизитов = "")
	...
	Если СтруктураРеквизитов.Свойство("ТоварыНоменклатура")
		...
		Элементы.ТоварыБезРазбиенияНоменклатура.ТолькоПросмотр = ТекущиеДанные.ЕстьУточнения;
		Элементы.ТоварыСопоставлениеНоменклатура.Доступность = Не ТекущиеДанные.ЕстьУточнения;
...
Процедура ИнициализироватьСлужебныеРеквизитыТоваров()
	...
	Для каждого ТекущаяСтрока Из Объект.Товары Цикл
		...
		СтрокаСопоставленногоТовара = СопоставленныеТовары.Получить(НомерСтрокиСопоставления);
		Если СтрокаСопоставленногоТовара.НомерСтрокиТовара = ТекущаяСтрока.НомерСтроки Тогда
			
			ТекущаяСтрока.СопоставленоКоличество = СтрокаСопоставленногоТовара.Количество;
		
			Если СтрокаСопоставленногоТовара.Количество = 1 И Не ТекущаяСтрока.ЕстьУточнения
				И (ТекущаяСтрока.Номенклатура <> СтрокаСопоставленногоТовара.Номенклатура
				И ЗначениеЗаполнено(СтрокаСопоставленногоТовара.Номенклатура)
				Или ТекущаяСтрока.Характеристика <> СтрокаСопоставленногоТовара.Характеристика) Тогда
				ТекущаяСтрока.Номенклатура   = СтрокаСопоставленногоТовара.Номенклатура;
				ТекущаяСтрока.Характеристика = СтрокаСопоставленногоТовара.Характеристика;
				ПараметрыЗаполнения = ИнтеграцияВЕТИСКлиентСервер.ПараметрыЗаполненияТабличнойЧасти();
				ПараметрыЗаполнения.ЕдиницаИзмеренияВЕТИС              = ТекущаяСтрока.ЕдиницаИзмеренияВЕТИС;
				ПараметрыЗаполнения.ПересчитатьКоличествоЕдиницПоВЕТИС = Истина;
				ПараметрыЗаполнения.ПроверитьСериюРассчитатьСтатус     = ПараметрыУказанияСерий <> Неопределено;
			...
			НомерСтрокиСопоставления = НомерСтрокиСопоставления + 1;
