...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ES_Приемка") Тогда
		ES_ОбщегоНазначения.ЗаполнитьПоступлениеВКассуПоПриемке(ЭтотОбъект,ДанныеЗаполнения);   
	КонецЕсли; 
	//}ЭР Несторук С.И.
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ES_ПриемНаложенныхПлатежей Тогда
