...
Процедура ВалютаДенежныхСредствПриИзменении(Элемент)
	...
	Если Объект.ВидОперации <> ВидОперацииОтПокупателя ИЛИ ЗначениеЗаполнено(ВалютаДенежныхСредствПередИзменением) ИЛИ Объект.ВалютаДенежныхСредств.Пустая() Тогда
...
Процедура НастроитьЭлементыРаспределенияДолговНаСервере() Экспорт
	...
	РасчетыРаботаСФормамиВызовСервера.НастроитьЭлементыРаспределенияСуммыНаСервере(ЭтотОбъект, Перечисления.ВидыОперацийПоступлениеВКассу.ОтПокупателя);
