...
Процедура ЗаполнитьКолонкуРезервПоРезервам() Экспорт
	...
	|	ВЫБОР
	|		КОГДА ТабличнаяЧасть.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Подразделение)
	|			ТОГДА ТабличнаяЧасть.Номенклатура.СчетУчетаЗатрат
	|		ИНАЧЕ ТабличнаяЧасть.Номенклатура.СчетУчетаЗапасов
	|	КОНЕЦ КАК СчетУчета
...
Процедура ЗаполнитьПоЗаказуНаПроизводство(ДанныеЗаполнения) Экспорт
	...
	Если РучноеРаспределение Тогда
		ПроизводствоСервер.РаспределитьМатериалы(Продукция, Запасы, РаспределениеЗапасов, ВыполненныеЭтапы, ЗаказНаПроизводство);
	КонецЕсли; 
...
Процедура ЗаполнитьПоЗаказуПокупателя(ДанныеЗаполнения) Экспорт
	...
	Если ЗначениеЗаполнено(ДанныеЗаполнения) И ТипЗнч(ДанныеЗаполнения)=Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеЗаполнения, "ОжидаетсяВыборВариантаКП, ВидОперации");
	Иначе
		ЗначенияРеквизитов = Новый Структура("ОжидаетсяВыборВариантаКП, ВидОперации", Ложь, Перечисления.ВидыОперацийЗаказПокупателя.ПустаяСсылка());
	КонецЕсли; 
...
Процедура ЗаполнитьПоПроизводственнойОперации(ДанныеЗаполнения) Экспорт
	...
	Реквизиты = ИнтеграцияВЕТИСПереопределяемый.ДанныеПрикладныхДокументовИзПроизводственнойОперацииВЕТИС(ДанныеЗаполнения);
	Реквизиты.Следующий();
...
Процедура ЗаполнитьСкладыИРезервы()
	...
	Для каждого СтрокаТабличнойЧасти Из ЭтотОбъект[ИмяТЧ] Цикл
		...
		Для каждого СтрокаДанных Из Строки Цикл
			...
			Если СтрокаДанных.Порядок=1 И СтрокаДанных.Количество<Количество Тогда
			...
			Если СтрокаДанных.Порядок=1 Тогда
				...
				СтрокаДанных.Количество = СтрокаДанных.Количество - СтрокаТабличнойЧасти.Количество * Коэффициент;
			КонецЕсли;
			Прервать;
		КонецЦикла; 
	КонецЦикла; 
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ПоложениеСклада=Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти Тогда
		...
		СтруктурнаяЕдиницаЗапасов = СтруктурнаяЕдиницаШапки(ТЧ, СтруктурнаяЕдиницаЗапасов);
		ЯчейкаЗапасов = ЯчейкаШапки(ТЧ, СтруктурнаяЕдиницаЗапасов, ЯчейкаЗапасов);
...
Функция СтруктурнаяЕдиницаШапки(ТабличнаяЧасть, Знач ТекСтруктурнаяЕдиница)
...
Функция ЯчейкаШапки(ТабличнаяЧасть, Знач ТекСтруктурнаяЕдиница, Знач ТекЯчейка)
	
	Если ТабличнаяЧасть.Количество()=0 Тогда
		Возврат ТекЯчейка;
	КонецЕсли; 
	
	ТекЯчейка = ТабличнаяЧасть[0].Ячейка;
	
	Если ТабличнаяЧасть.Количество()=1 Тогда
		Возврат ТекЯчейка;
	КонецЕсли; 
	
	ТаблицаЯчейки = ТабличнаяЧасть.Выгрузить(Новый Структура ("СтруктурнаяЕдиница", ТекСтруктурнаяЕдиница), "Ячейка");
	ТаблицаЯчейки.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число"));
	Для Каждого ТекСтрока Из ТаблицаЯчейки Цикл
		ТекСтрока.Количество = 1;
	КонецЦикла;
	ТаблицаЯчейки.Свернуть("Ячейка", "Количество");
	
	Если ТаблицаЯчейки.Количество() < 2 Тогда
		Возврат ТекЯчейка;
	КонецЕсли;
	
	ТаблицаЯчейки.Сортировать("Количество Убыв");
	
	Если ТаблицаЯчейки[0].Количество = ТаблицаЯчейки[1].Количество Тогда
		Возврат ТекЯчейка;
	КонецЕсли;
	
	ТекЯчейка = ТаблицаЯчейки[0].Ячейка;
...
Процедура ДобавитьРаспределениеПоСпецификации(Запрос)
	...
	|	ТаблицаСостава.Количество КАК КоличествоПродукция,
...
Процедура ДобавитьОстаткиРаспределенияПоЗаказуНаПроизводство(Запрос)
	...
	Если РезультатЗапроса[2].Пустой() Тогда
		...
		ПроизводствоСервер.РаспределитьМатериалы(ТаблицаПродукции, ТаблицаЗапасы, ТаблицаРаспределение);
...
// При поэтапном производстве возможен сценарий выполнения этапа несколькими документами.
// При заполнении таблицы запасов исключаеются данные прочих документов производства
// по выполняемым этапам
...
Процедура ПроверитьЗаполнениеЭтаповПроизводства(Отказ, ОтменаПроведения = Ложь)
	...
	Для каждого СтрокаТабличнойЧасти Из ТаблицаПродукции Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТабличнойЧасти.Спецификация) Тогда
			Продолжить;
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(ЗаказНаПроизводство) И НЕ ЗначениеЗаполнено(СтрокаТабличнойЧасти.ЗаказПокупателя) Тогда
			Продолжить;
		КонецЕсли; 
		Если МассивСпецификаций.Найти(СтрокаТабличнойЧасти.Спецификация)=Неопределено Тогда
			МассивСпецификаций.Добавить(СтрокаТабличнойЧасти.Спецификация);
