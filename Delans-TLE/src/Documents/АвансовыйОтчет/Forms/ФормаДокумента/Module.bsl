...
Процедура ОбработатьИзмененияПоКнопкеРедактироватьВалютуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если СтруктураВозврата.ПересчитатьЦеныПоВалюте Тогда
		УправлениеНебольшойФирмойКлиент.ПересчитатьЦеныТабличнойЧастиПоВалюте(ЭтотОбъект, СтруктураПараметров.ВалютаДокумента, "Запасы", СтруктураВозврата.КурсПересчетаЦен);
		УправлениеНебольшойФирмойКлиент.ПересчитатьЦеныТабличнойЧастиПоВалюте(ЭтотОбъект, СтруктураПараметров.ВалютаДокумента, "Расходы", СтруктураВозврата.КурсПересчетаЦен);
	КонецЕсли;
...
Процедура ОплатыКурсПриИзменении(Элемент)
	...
	РасчетыРаботаСФормамиКлиент.ПриИзмененииКурсаВСтрокеОплаты(ЭтаФорма);
...
Функция ПолучитьСтавкуНоменклатуры(Номенклатура, ДатаДокумента)
	...
	Возврат Справочники.СтавкиНДС.СтавкаНДС(Номенклатура.ВидСтавкиНДС, ?(ЗначениеЗаполнено(ДатаДокумента), ДатаДокумента, ТекущаяДатаСеанса()));
