...
Процедура УстановитьВидимостьПриСозданииПоВидуКлиента()
	...
	Иначе
		...
		Если Объект.Предоплата.Количество() > 0
		 ...
		 ИЛИ НетЭквайринговыхТерминалов() Тогда
...
&НаСервере
Процедура ДобавитьОплатуКартойТолькоКарта()
...
Процедура ОК(Команда)
	...
	// Если оплата картой при неподключенном ЭТ, то добавим строку об оплате.
	Если НЕ Объект.ЭквайринговыйТерминал.Пустая()
		И ЭквайринговыйТерминал.Пустая()
		И Элементы.ГруппаОплата.ТекущаяСтраница = Элементы.ГруппаОплатаКартой Тогда
		ДобавитьОплатуКартойТолькоКарта();
	КонецЕсли;
		...
		Если ТекущаяСтрока.Сумма > 0 Или ТекущаяСтрока.СуммаБонусов > 0 Тогда
			Если ТекущаяСтрока.ВидОплаты = ПредопределенноеЗначение("Перечисление.ВидыБезналичныхОплат.БанковскаяКарта") Тогда
				...
				Если Не ЗначениеЗаполнено(ТекущаяСтрока.ВидПлатежнойКарты) Тогда
					Если ЗначениеЗаполнено(ВидКартыОплатаКартой) Тогда
						ТекущаяСтрока.ВидПлатежнойКарты = ВидКартыОплатаКартой;
					КонецЕсли;
				КонецЕсли;
	...
	ПараметрыЗакрытия = Новый Структура("Объект, Наличными, ПлатежнымиКартами, Предоплата, Сдача, Бонусы, СформироватьТоварныйЧек, БезналичнаяОплата, ЗаписатьДокумент, Кнопка", 
		Объект,
		ПолученоНаличными,
		Объект.Предоплата.Итог("СуммаРасчетов"),
		Объект.БезналичнаяОплата.Итог("Сумма"), 
		СуммаСдачи, 
		Объект.БонусныеБаллыКНачислению.Итог("КоличествоБонусныхБаллов"), 
		СформироватьТоварныйЧек, 
		Объект.БезналичнаяОплата, 
		ЗаписатьДокумент,
		"Пробить чек");
...
//
&НаКлиенте
Процедура ДобавитьОплатуКартой(ОднаКарта_МК = Ложь)
	...
	Если ИдентификаторУстройстваЭТ <> Неопределено 
		...
		ДополнительныеПараметры = Новый Структура(
			"ИдентификаторУстройстваФР, ИдентификаторУстройстваЭТ, НомерКарты, ОднаКарта_МК",
			ИдентификаторУстройстваФР, ИдентификаторУстройстваЭТ, НомерКарты, ОднаКарта_МК
		);
...
Процедура ПередатьНаТерминал(Команда)
	...
	ДобавитьОплатуКартой(ОднаКарта_МК);
