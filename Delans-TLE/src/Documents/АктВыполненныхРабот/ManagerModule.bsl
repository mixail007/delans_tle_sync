...
Процедура СформироватьТаблицаЗаказыНаПроизводство(ДокументСсылкаАктВыполненныхРабот, СтруктураДополнительныеСвойства)
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗаказыНаПроизводство", РезультатЗапроса.Выгрузить());
...
Функция ДанныеДокументовРегУчет(МассивОбъектов, ИспользоватьФаксимиле, ПечатнаяФормаТолькоВРублях = Истина, Ошибки = Неопределено) Экспорт
	...
	|	,Выбор когда АктВыполненныхРабот.Организация.ГоловнаяОрганизация <> Значение(Справочник.Организации.ПустаяСсылка)
	|		Тогда АктВыполненныхРабот.Организация.ГоловнаяОрганизация
	|		Иначе АктВыполненныхРабот.Организация Конец КАК Организация
	...
	|		Иначе АктВыполненныхРабот.Организация Конец КАК ОбособленноеПодразделениеПоставщика
	|	,Выбор когда АктВыполненныхРабот.Организация.ГоловнаяОрганизация <> Значение(Справочник.Организации.ПустаяСсылка)
	|		Тогда АктВыполненныхРабот.Организация.ГоловнаяОрганизация
	|		Иначе АктВыполненныхРабот.Организация Конец КАК Грузоотправитель
	|	,Выбор когда АктВыполненныхРабот.Организация.ЮридическоеФизическоеЛицо = Значение(Перечисление.ЮридическоеФизическоеЛицо.ФизическоеЛицо) 
	|		Тогда Истина
	|		Иначе Ложь Конец КАК ЭтоФизическоеЛицо
	|	,Выбор Когда АктВыполненныхРабот.НалогообложениеНДС = Значение(Перечисление.ТипыНалогообложенияНДС.НеОблагаетсяНДС)
	|		Тогда 2
	...
	|	,Выбор когда АктВыполненныхРабот.Контрагент.ГоловнойКонтрагент <> Значение(Справочник.Контрагенты.ПустаяСсылка)
	...
	|		Иначе АктВыполненныхРабот.Контрагент Конец КАК Контрагент
	|	,Выбор когда АктВыполненныхРабот.Контрагент.ГоловнойКонтрагент <> Значение(Справочник.Контрагенты.ПустаяСсылка)
	|			И АктВыполненныхРабот.Контрагент.ГоловнойКонтрагент.СтранаРегистрации = Значение(Справочник.СтраныМира.Россия)
	|		Тогда АктВыполненныхРабот.Контрагент.ГоловнойКонтрагент
	|		Иначе АктВыполненныхРабот.Контрагент Конец КАК Покупатель
	|	,Выбор когда АктВыполненныхРабот.Контрагент.ГоловнойКонтрагент <> Значение(Справочник.Контрагенты.ПустаяСсылка)
	|			И АктВыполненныхРабот.Контрагент.ГоловнойКонтрагент.СтранаРегистрации = Значение(Справочник.СтраныМира.Россия)
	|		Тогда АктВыполненныхРабот.Контрагент.ГоловнойКонтрагент
	|		Иначе АктВыполненныхРабот.Контрагент Конец КАК Грузополучатель
	...
	|		Иначе АктВыполненныхРабот.ВалютаДокумента Конец КАК ВалютаДокумента
	|	,Выбор когда &ПечатнаяФормаТолькоВРублях = Истина 
	|		Тогда НациональнаяВалюта.Значение.НаименованиеПолное
	|		Иначе АктВыполненныхРабот.ВалютаДокумента.НаименованиеПолное Конец КАК ВалютаНаименование
	|	,Выбор когда &ПечатнаяФормаТолькоВРублях = Истина 
	|		Тогда НациональнаяВалюта.Значение.Код
	|		Иначе АктВыполненныхРабот.ВалютаДокумента.Код Конец КАК ВалютаКод
	...
	|			Тогда ЕдиницаИзмерения
	|			Иначе Неопределено Конец КАК ЕдиницаИзмерения
	|		,Выбор когда ЕдиницаИзмерения Ссылка Справочник.КлассификаторЕдиницИзмерения
	...
	|		,Выбор когда ЕдиницаИзмерения Ссылка Справочник.КлассификаторЕдиницИзмерения
	|			Тогда Количество
	|			Иначе Неопределено Конец КАК Количество
	...
	|					Тогда
	|						Выбор когда АктВыполненныхРабот.Ссылка.Договор.РасчетыВУсловныхЕдиницах И АктВыполненныхРабот.Ссылка.ВалютаДокумента <> &НациональнаяВалюта
	|							Тогда Выразить(Цена * АктВыполненныхРабот.Курс / АктВыполненныхРабот.Кратность КАК Число(15,2))
	|							Иначе Цена Конец
	|					Иначе Цена Конец
	|			Иначе Неопределено Конец КАК Цена
	|		,Выбор когда &ПечатнаяФормаТолькоВРублях
	|			Тогда
	|				Выбор когда АктВыполненныхРабот.Ссылка.Договор.РасчетыВУсловныхЕдиницах И АктВыполненныхРабот.Ссылка.ВалютаДокумента <> &НациональнаяВалюта
	|					Тогда Выразить(Сумма * АктВыполненныхРабот.Курс / АктВыполненныхРабот.Кратность КАК Число(15,2))
	|					Иначе Сумма Конец
	|			Иначе Сумма Конец КАК Сумма
	|		,Выбор когда &ПечатнаяФормаТолькоВРублях
	|			Тогда
	|				Выбор когда АктВыполненныхРабот.Ссылка.Договор.РасчетыВУсловныхЕдиницах И АктВыполненныхРабот.Ссылка.ВалютаДокумента <> &НациональнаяВалюта
	|					Тогда Выразить(СуммаНДС * АктВыполненныхРабот.Курс / АктВыполненныхРабот.Кратность КАК Число(15,2))
	|					Иначе СуммаНДС Конец 
	|			Иначе СуммаНДС Конец КАК СуммаНДС
	|		,Выбор когда &ПечатнаяФормаТолькоВРублях
	|			Тогда
	|				Выбор когда АктВыполненныхРабот.Ссылка.Договор.РасчетыВУсловныхЕдиницах И АктВыполненныхРабот.Ссылка.ВалютаДокумента <> &НациональнаяВалюта
	|					Тогда Выразить(Всего * АктВыполненныхРабот.Курс / АктВыполненныхРабот.Кратность КАК Число(15,2))
	|					Иначе Всего Конец
	...
	|	АктВыполненныхРаботРаботыИУслуги.Номенклатура.ЕдиницаИзмерения.Наименование КАК ЕдиницаИзмеренияПоОКЕИ_Наименование,
	|	АктВыполненныхРаботРаботыИУслуги.Номенклатура.ЕдиницаИзмерения.Код КАК ЕдиницаИзмеренияПоОКЕИ_Код,
	...
	|	АктВыполненныхРаботРаботыИУслуги.ЕдиницаИзмерения КАК ВидУпаковки,
	...
	|	,АктВыполненныхРаботРаботыИУслуги.СтавкаНДС КАК СтавкаНДС
	|	,АктВыполненныхРаботРаботыИУслуги.НомерСтроки КАК НомерСтроки
	|	,Выбор Когда АктВыполненныхРаботРаботыИУслуги.Ссылка.ВалютаДокумента = &НациональнаяВалюта
	|			Тогда АктВыполненныхРаботРаботыИУслуги.СуммаСкидкиПоСтроке
	|			Иначе Выразить(АктВыполненныхРаботРаботыИУслуги.СуммаСкидкиПоСтроке * АктВыполненныхРаботРаботыИУслуги.Ссылка.Курс / АктВыполненныхРаботРаботыИУслуги.Ссылка.Кратность КАК ЧИСЛО(15, 2)) Конец КАК СуммаСкидкиПоСтроке
	|	,Выбор когда &ПечатнаяФормаТолькоВРублях 
	|		Тогда
	|			Выбор Когда АктВыполненныхРаботРаботыИУслуги.Ссылка.ВалютаДокумента = &НациональнаяВалюта
	...
	|				Иначе
	|					Выбор Когда АктВыполненныхРаботРаботыИУслуги.Количество = 0
	|						Тогда (СуммыДокументовРегламентированныйУчет.Всего - СуммыДокументовРегламентированныйУчет.НДС)
	|						Иначе Выразить((СуммыДокументовРегламентированныйУчет.Всего - СуммыДокументовРегламентированныйУчет.НДС) / АктВыполненныхРаботРаботыИУслуги.Количество КАК ЧИСЛО(15, 2)) Конец
	|			Конец
	|		Иначе АктВыполненныхРаботРаботыИУслуги.Цена Конец КАК Цена
	|	,Выбор когда &ПечатнаяФормаТолькоВРублях 
	|		Тогда
	|			Выбор Когда АктВыполненныхРаботРаботыИУслуги.Ссылка.ВалютаДокумента = &НациональнаяВалюта
	...
	|				Иначе
	|					Выбор Когда АктВыполненныхРаботРаботыИУслуги.Ссылка.СуммаВключаетНДС
	...
	|			Конец
	|		Иначе АктВыполненныхРаботРаботыИУслуги.Сумма Конец КАК Сумма
	|	,Выбор когда &ПечатнаяФормаТолькоВРублях 
	|		Тогда
	|			Выбор Когда АктВыполненныхРаботРаботыИУслуги.Ссылка.ВалютаДокумента = &НациональнаяВалюта
	|					Тогда АктВыполненныхРаботРаботыИУслуги.СуммаНДС
	...
	|			Конец
	|		Иначе АктВыполненныхРаботРаботыИУслуги.СуммаНДС Конец КАК СуммаНДС
	|	,Выбор когда &ПечатнаяФормаТолькоВРублях 
	|		Тогда
	|			Выбор Когда АктВыполненныхРаботРаботыИУслуги.Ссылка.ВалютаДокумента = &НациональнаяВалюта
	|					Тогда АктВыполненныхРаботРаботыИУслуги.Всего
	...
	|			Конец
	|		Иначе АктВыполненныхРаботРаботыИУслуги.Всего Конец КАК Всего
	|	,АктВыполненныхРаботРаботыИУслуги.Ссылка КАК Ссылка
	|	,АктВыполненныхРаботРаботыИУслуги.НоменклатураНабора КАК НоменклатураНабора
	|	,АктВыполненныхРаботРаботыИУслуги.ХарактеристикаНабора КАК ХарактеристикаНабора
	...
	|	,Выбор когда АктВыполненныхРаботРаботыИУслуги.Номенклатура.ТипНоменклатуры = Значение(Перечисление.ТипыНоменклатуры.Запас)
