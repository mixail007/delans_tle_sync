...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		РаботаСОтборами.НастроитьПанельОтборовМобильныйКлиент(ЭтотОбъект,,,
			"ОтборКонтрагент,ОтборОтветственный,ОтборТипСобытия,ОтборСостояние,ОтборВажность,ОтборПроект,ОтборУчетнаяЗапись");
	КонецЕсли;
...
Процедура СписокПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	ОбновитьСвойстваТелефонныхЗвонков(Строки);
...
Процедура УстановитьУсловноеОформление()
	...
	РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "Формат", "ДЛФ=T; ДП=0:00:00");
	...
	РаботаСФормой.ДобавитьОформляемоеПоле(НовоеУсловноеОформление, Элементы.СписокОкончаниеСобытия.Имя);
	РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "Формат", "ДФ='dd.MM.yy HH:mm'; ДЛФ=DT");
...
Процедура УправлениеФормой()
	...
	Элементы.ОтборПропущенные.Видимость = (ОтборВходящееИсходящее = ПредопределенноеЗначение("Перечисление.ВходящееИсходящееСобытие.Входящее"));
...
&НаСервереБезКонтекста
Процедура ОбновитьСвойстваТелефонныхЗвонков(СтрокиСписка)
	...
	|ГДЕ
	|	ДанныеЗвонков.Событие В(&События)");
	Запрос.УстановитьПараметр("События", СтрокиСписка.ПолучитьКлючи());
	...
	Пока Выборка.Следующий() Цикл
		СтрокаСписка = СтрокиСписка[Выборка.Событие];
		СтрокаСписка.Данные.ОкончаниеСобытия = Дата(1, 1, 1) + Выборка.ДлительностьРазговора;
		СтрокаСписка.Данные.Ответственный = Выборка.Сотрудник;
		Если Выборка.Неотвеченный Тогда
			Если СтрокаСписка.Данные.ПометкаУдаления Тогда
				СтрокаСписка.Данные.ИндексПиктограммы = 13;
			Иначе
				СтрокаСписка.Данные.ИндексПиктограммы = 4;
			КонецЕсли;
		КонецЕсли;
		СтрокаСписка.Данные.Неотвеченный = Выборка.Неотвеченный;
		СтрокаСписка.Данные.ЭтоЗвонок = Истина;
	КонецЦикла;
...
Процедура ОбработатьРезультатВыполненияЗаданияНаСервере(АдресРезультата)
	...
	ЗагруженоПрописью = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(";%1 письмо;;%1 письма;%1 писем;%1 письма", ЗагруженоПисем);
