...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаСоздатьКонтрагент", "Видимость", Объект.ТипСобытия = Перечисления.ТипыСобытий.ТелефонныйЗвонок);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаСоздатьКонтакт", "Видимость", Объект.ТипСобытия = Перечисления.ТипыСобытий.ТелефонныйЗвонок);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаСоздатьЛид", "Видимость", Объект.ТипСобытия = Перечисления.ТипыСобытий.ТелефонныйЗвонок И ДоступнаРаботаСЛидами);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаПривязатьКСуществующемуКонтрагенту", "Видимость", Объект.ТипСобытия = Перечисления.ТипыСобытий.ТелефонныйЗвонок);
	...
	// todo Объект.ТипСобытия = ПредопределенноеЗначение("Перечисление.ТипыСобытий.ТелефонныйЗвонок");
...
Процедура ПрослушатьЗапись(Команда)
	...
	Перем ОбновитьДанныеЗвонка;
	ТелефонияКлиент.ПрослушатьЗаписьРазговора(ИдентификаторЗвонкаВАТС, ИдентификаторЗаписиРазговора, ОбновитьДанныеЗвонка);
	
	Если ОбновитьДанныеЗвонка Тогда
		ЗаполнитьДанныеТелефонногоРазговора();
	КонецЕсли;
...
Процедура ОбновитьКакСвязаться()
	...
	Если ТипЗнч(Контрагент) = Тип("СправочникСсылка.Лиды") Тогда
		...
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ЛидыКонтакты.Наименование КАК Наименование,
		|	ЛидыКонтакты.ИдентификаторСтрокиТабличнойЧасти КАК ИдентификаторСтрокиТабличнойЧасти
		|ИЗ
		|	Справочник.Лиды.Контакты КАК ЛидыКонтакты
		|ГДЕ
		|	ЛидыКонтакты.Ссылка = &Лид";
		Запрос.УстановитьПараметр("Лид", Контрагент);
		
		ДанныеКИ = ПолучитьКакСвязаться(Контрагент, Объект.ТипСобытия);
		
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			НоваяСтрока = Контакты.Добавить();
			НоваяСтрока.Контакт = Выборка.Наименование;
			НоваяСтрока.КакСвязаться = ДанныеКИ.Получить(Выборка.ИдентификаторСтрокиТабличнойЧасти);
		КонецЦикла;
...
Процедура УправлениеФормой()
	...
	// todo
	Элементы.ПрослушатьЗапись.Видимость = ЗначениеЗаполнено(ИдентификаторЗаписиРазговора) ИЛИ ТрубетсяЗапроситьЗаписьРазговора;
	...
	Если ОбязательноЗаполнятьИсточникВоВстречах ИЛИ ОбязательноЗаполнятьИсточникВЗвонках Тогда
...
&НаСервере
Процедура ЗаполнитьДанныеТелефонногоРазговора()
	...
	|	ДанныеЗвонков.ТрубетсяЗапроситьЗаписьРазговора КАК ТрубетсяЗапроситьЗаписьРазговора,
	...
	|ГДЕ
	|	ДанныеЗвонков.Событие.Ссылка = &Событие";
	Запрос.УстановитьПараметр("Событие", Объект.Ссылка);
	
	ДанныеЗвонка = Запрос.Выполнить().Выбрать();
	
	Если НЕ ДанныеЗвонка.Следующий() Тогда
		Элементы.ДанныеЗвонка.Видимость = Ложь;
		Возврат;
	КонецЕсли;
	
	СобытиеПривязаноКЗвонку = Истина;
	Неотвеченный = ДанныеЗвонка.Неотвеченный;
	ИдентификаторЗвонкаВАТС          = ДанныеЗвонка.ИдентификаторЗвонкаВАТС;
	ИдентификаторЗаписиРазговора     = ДанныеЗвонка.СсылкаНаЗаписьРазговора;
	ТрубетсяЗапроситьЗаписьРазговора = ДанныеЗвонка.ТрубетсяЗапроситьЗаписьРазговора;
	ДлительностьРазговораСтрокой     = ПолучитьСтрокуВремени(ДанныеЗвонка.ДлительностьРазговора);
	Если ДанныеЗвонка.ВходящееИсходящее = Перечисления.ВходящееИсходящееСобытие.Входящее Тогда
		НомерАбонента = ДанныеЗвонка.НомерОтКого;
	Иначе
		НомерАбонента = ДанныеЗвонка.НомерКому;
	КонецЕсли;
	
	// todo почему в двух местах?
	Элементы.ПрослушатьЗапись.Видимость = ЗначениеЗаполнено(ИдентификаторЗаписиРазговора) ИЛИ ТрубетсяЗапроситьЗаписьРазговора;
	Элементы.ЗаписьРазговораОтсутствует.Видимость = НЕ Элементы.ПрослушатьЗапись.Видимость;
	
	Элементы.ОбновитьДанныеЗвонка.Видимость = НЕ ДанныеЗвонка.ВызовЗавершен;
	Элементы.ТелефонныйЗвонокВходящий.Видимость = НЕ Неотвеченный И ДанныеЗвонка.ВходящееИсходящее = Перечисления.ВходящееИсходящееСобытие.Входящее;
	Элементы.ТелефонныйЗвонокИсходящий.Видимость = НЕ Неотвеченный И ДанныеЗвонка.ВходящееИсходящее = Перечисления.ВходящееИсходящееСобытие.Исходящее;
	Элементы.ТелефонныйЗвонокНеотвеченный.Видимость = Неотвеченный;
	
	Ответственный = ДанныеЗвонка.Сотрудник;
