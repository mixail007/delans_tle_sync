...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	НомерОрганизации = "";
	Если ЭтотОбъект.ДополнительныеСвойства.Свойство("НомерОрганизации", НомерОрганизации) И НЕ ЗначениеЗаполнено(ИсточникПривлечения) Тогда
	...
	Если ТипСобытия = Перечисления.ТипыСобытий.Запись и Не Отказ Тогда
...
Процедура ПриЗаписи(Отказ)
	...
	Если ТипСобытия = Перечисления.ТипыСобытий.Запись и Не Отказ Тогда
		Если ПометкаУдаления 
			Или Состояние = Справочники.СостоянияСобытий.НайтиПоНаименованию("Отменено")
			Или ЕстьПодчиненныеЗаказНаряды() Тогда
			ПланированиеРесурсовУНФ.СформироватьЗаписиКолендаряСотрудникаПоРесурсам(Ссылка,, Отказ);
			ПланированиеРесурсовУНФ.ЗаписатьРасписаниеЗагрузкиРесурсов(Ссылка,, Отказ);
		Иначе
...
Процедура ЗаполнитьПоЛид(Лид)
	...
	ДанныеКИ = Документы.Событие.ПолучитьКакСвязатьсяКонтактов(Лид, ТипыКИ);
...
Процедура ЗаполнитьПоШаблону(ШаблонСообщения, ПредметСообщения = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ЭтотОбъект.ТипСобытия <> Перечисления.ТипыСобытий.ЭлектронноеПисьмо
		И ЭтотОбъект.ТипСобытия <> Перечисления.ТипыСобытий.СообщениеSMS Тогда
	...
	ЭтотОбъект.Тема           = СообщениеПоШаблону.Тема;
	Если ЭтотОбъект.ТипСобытия = Перечисления.ТипыСобытий.ЭлектронноеПисьмо Тогда
		ФорматированныйДокумент = Новый ФорматированныйДокумент;
		ФорматированныйДокумент.УстановитьHTML(СообщениеПоШаблону.Текст, Новый Структура);
		ЭтотОбъект.Содержание     = ФорматированныйДокумент.ПолучитьТекст();
		ЭтотОбъект.СодержаниеHTML = СообщениеПоШаблону.Текст;
	ИначеЕсли ЭтотОбъект.ТипСобытия = Перечисления.ТипыСобытий.СообщениеSMS Тогда
		ЭтотОбъект.Содержание     = СообщениеПоШаблону.Текст;
	КонецЕсли;
	
	ЭтотОбъект.Участники.Очистить();
	...
	Если СообщениеПоШаблону.Вложения.Количество() > 0 Тогда
		...
		ЭтотОбъект.КартинкиHTML = Новый ХранилищеЗначения(КартинкиHTML);
	КонецЕсли;
