...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Если Данные.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийЗаказПокупателя.ЗаказНаряд") Тогда
		...
		//EFSOL_oks_kry_18.03.2015 21:03:30_BEGIN
	ИначеЕсли Данные.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийЗаказПокупателя.ES_ЗаказНаДоставку") Тогда
		ЗаголовокПредставление = НСтр("ru='Заказ на доставку'");
	//EFSOL_oks_kry_18.03.2015 21:03:30_END
	Иначе
...
Функция ПолучитьСоответствиеВидовОперацийФормам(ФормыСписка = Ложь) Экспорт
	...
	Если ФормыСписка Тогда
		...
		//EFSOL_oks_kry_18.03.2015 19:32:17_BEGIN
		ФормыЗаказаПокупателя.Вставить(Перечисления.ВидыОперацийЗаказПокупателя.ES_ЗаказНаДоставку, "ES_ФормаСпискаЗаказаНаДоставку");
		//EFSOL_oks_kry_18.03.2015 19:32:17_END
	...
	Иначе
		...
		//EFSOL_oks_kry_18.03.2015 19:32:17_BEGIN
		ФормыЗаказаПокупателя.Вставить(Перечисления.ВидыОперацийЗаказПокупателя.ES_ЗаказНаДоставку, "ES_ФормаЗаказаНаДоставку");
		//EFSOL_oks_kry_18.03.2015 19:32:17_END
...
Процедура ИзменитьСостояниеЗаказНаряда(Заказ, Состояние, ВариантЗавершения = Неопределено, ПричинаОтмены = Неопределено, Заметки = Неопределено) Экспорт
	...
	РежимЗаписиЗаказНаряда = РежимЗаписиЗаказНаряда(ЗаказОбъект, Состояние);
	...
	ЗаказОбъект.Записать(РежимЗаписиЗаказНаряда);
	
	ЗаказОбъект.Разблокировать();
	
КонецПроцедуры

Функция РежимЗаписиЗаказНаряда(ЗаказОбъект, Состояние)
	
	Если НЕ ЗаказОбъект.Проведен Тогда
		Возврат РежимЗаписиДокумента.Запись;
	КонецЕсли;
	
	ТекущееСостояниеСостояниеВыполнения = СостоянияЗаказов.ЭтоСостояниеВыполненияЗаказНаряда(Состояние, ЗаказОбъект.ВидЗаказа);
	ВыбранноеСостояниеСостояниеВыполнения = СостоянияЗаказов.ЭтоСостояниеВыполненияЗаказНаряда(ЗаказОбъект.СостояниеЗаказа, ЗаказОбъект.ВидЗаказа);
	
	Если (ТекущееСостояниеСостояниеВыполнения ИЛИ ВыбранноеСостояниеСостояниеВыполнения) Тогда
...
Функция РежимЗаписиЗаказаПокупателя(Заказ, Состояние, ВариантЗавершения)
	
	ДанныеЗаказа = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Заказ, "Проведен,ВариантЗавершения,СостояниеЗаказа");
	
	Если НЕ ДанныеЗаказа.Проведен Тогда
		Возврат РежимЗаписиДокумента.Запись;
	КонецЕсли;
	
	ТекущееСостояниеСостояниеОтмены = ДанныеЗаказа.ВариантЗавершения = Перечисления.ВариантыЗавершенияЗаказа.Отменен 
		И ДанныеЗаказа.СостояниеЗаказа = Справочники.СостоянияЗаказовПокупателей.Завершен;
									
	ВыбранноеСостояниеСостояниеОтмены = ВариантЗавершения = Перечисления.ВариантыЗавершенияЗаказа.Отменен 
		И Состояние = Справочники.СостоянияЗаказовПокупателей.Завершен;
	
	Если (ТекущееСостояниеСостояниеОтмены ИЛИ ВыбранноеСостояниеСостояниеОтмены) Тогда
		Возврат РежимЗаписиДокумента.Проведение;
	КонецЕсли;
...
Процедура ИзменитьСостояниеЗаказаПокупателя(Заказ, Состояние, ВариантЗавершения = Неопределено, ПричинаОтмены = Неопределено, Заметки = Неопределено) Экспорт
	...
	РежимЗаписиЗаказаПокупателя = РежимЗаписиЗаказаПокупателя(Заказ, Состояние, ВариантЗавершения);
	...
	ЗаказОбъект.Записать(РежимЗаписиЗаказаПокупателя);
...
Процедура ПриОпределенииДействий(Действия) Экспорт
	...
	//НовоеПоле.ОписаниеТипов = Новый ОписаниеТипов("СправочникСсылка.СостоянияЗаказовПокупателей");;
	//НовоеПоле.ОбязательноеЗаполнение = Истина;
	//НовоеПоле.ВариантЗаполнения = "Указанный";
	//
	//АссистентУправления.ДобавитьОписаниеПолейДействия(НовоеДействие);
...
Функция ВыполненыУсловия(Предмет, Условия) Экспорт
	...
	ДанныеЗаказа = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Предмет, "ВидЗаказа,ВидОперации");

	Для каждого Условие Из Условия Цикл
		
		Если Условие.Параметр = "ВидОперации" Тогда
			Если НЕ АссистентУправления.ВыполненоУсловиеСравнения(ДанныеЗаказа.ВидОперации, Условие.ВидСравнения, Условие.Значение) Тогда
				Возврат Ложь;
			КонецЕсли;
		КонецЕсли;
		
		Если Условие.Параметр = "ВидЗаказа" Тогда
			Если НЕ АссистентУправления.ВыполненоУсловиеСравнения(ДанныеЗаказа.ВидЗаказа, Условие.ВидСравнения, Условие.Значение) Тогда
				Возврат Ложь;
			КонецЕсли;
		КонецЕсли;
...
Функция ПолучитьТекстЗапросаИнициализироватьДанныеДокументаЗаказПокупателя()
	...
	|	ЗаказПокупателяЗапасы.Партия КАК Партия,
	|	ЕСТЬNULL(ВидыПроизводстваЭтапы.Этап, ЗНАЧЕНИЕ(Справочник.ЭтапыПроизводства.ЗавершениеПроизводства)) КАК Этап,
	|	ЗаказПокупателяЗапасы.Количество - ЗаказПокупателяЗапасы.Резерв КАК КоличествоПлан,
	|	0 КАК КоличествоФакт,
	|	ЗНАЧЕНИЕ(Справочник.СтруктурныеЕдиницы.ПустаяСсылка) КАК СтруктурнаяЕдиница
	|ИЗ
	|	ВТЗаказПокупателяЗапасы КАК ЗаказПокупателяЗапасы
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства, ДокументОбъектЗаказПокупателя) Экспорт
	...
	//EFSOL_oks_kry_19.03.2015 19:58:08_BEGIN
	ИначеЕсли ДокументОбъектЗаказПокупателя.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ES_ЗаказНаДоставку Тогда
		ИнициализироватьДанныеДокументаЗаказНаДоставку(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства);
	//EFSOL_oks_kry_19.03.2015 19:58:08_END	
	Иначе
...
Процедура ПриВыводеТаблицыПараметровШаблона(ПараметрыПечати, ИмяТаблицы, ОписаниеПараметров, ПараметрыВывода = Неопределено) Экспорт
	...
	Пока НомерВарианта <= ПоследнийВариант Цикл
		...
		Строки = ДанныеЗаказа.ТаблицаЗапасы.НайтиСтроки(Отбор);
...
Процедура ДобавитьПараметрыКонтрагентаВШаблон(ПараметрыШаблона)
	...
	ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрШаблона(ПараметрыШаблона, "ЗаказПокупателя.Контрагент.КонтрагентРегистрационныйНомер", МетаданныеКонтрагента.Реквизиты.РегистрационныйНомер.Синоним);
...
Процедура ДобавитьПараметрыЗаказаПокупателяВШаблон(ПараметрыШаблона)
	...
	ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрШаблона(ПараметрыШаблона, "ЗаказПокупателя.НомерЗаказа", НСтр("ru='Номер'"));
	...
	НовыйПараметр = ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрШаблона(ПараметрыШаблона, "ЗаказПокупателя.СуммаДокумента", НСтр("ru='Сумма документа'"), ФорматЦены,, ТипЦены);
...
Процедура СформироватьСводныйОтчет(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки)
...
Функция ПечатьСводныйОтчетЗаказНаряд(МассивОбъектов, ОбъектыПечати, Ошибки)
...
Функция ДанныеДокументовРегУчет(МассивОбъектов, ИспользоватьФаксимиле, ПечатнаяФормаТолькоВРублях = Истина, Ошибки = Неопределено) Экспорт
	...
	|	,Выбор когда ЗаказПокупателя.Организация.ГоловнаяОрганизация <> Значение(Справочник.Организации.ПустаяСсылка)
	|		Тогда ЗаказПокупателя.Организация.ГоловнаяОрганизация
	|		Иначе ЗаказПокупателя.Организация Конец КАК Организация
	|	,ЗаказПокупателя.Организация.Префикс КАК Префикс
	|	,ЗаказПокупателя.Организация.ФайлФаксимильнаяПечать КАК ФаксимилеПечати
	|	,Выбор когда ЗаказПокупателя.Организация.ГоловнаяОрганизация <> Значение(Справочник.Организации.ПустаяСсылка)
	...
	|		Иначе ЗаказПокупателя.Организация Конец КАК ОбособленноеПодразделениеПоставщика
	|	,Выбор когда ЗаказПокупателя.Организация.ЮридическоеФизическоеЛицо = Значение(Перечисление.ЮридическоеФизическоеЛицо.ФизическоеЛицо) 
	|		Тогда Истина
	|		Иначе Ложь Конец КАК ЭтоФизическоеЛицо
	|	,Выбор Когда ЗаказПокупателя.НалогообложениеНДС = Значение(Перечисление.ТипыНалогообложенияНДС.НеОблагаетсяНДС)
	|		Тогда 2
	...
	|				И ЗаказПокупателя.СтруктурнаяЕдиницаПродажи.ГоловнаяОрганизация = ЗаказПокупателя.Организация
	|			Тогда ЗаказПокупателя.СтруктурнаяЕдиницаПродажи
	|		Иначе ЗаказПокупателя.Организация Конец КАК Грузоотправитель
	|	,Выбор когда ЗаказПокупателя.Контрагент.ГоловнойКонтрагент <> Значение(Справочник.Контрагенты.ПустаяСсылка)
	...
	|		Иначе ЗаказПокупателя.Контрагент Конец КАК Контрагент
	|	,Выбор когда ЗаказПокупателя.Контрагент.ГоловнойКонтрагент <> Значение(Справочник.Контрагенты.ПустаяСсылка)
	|			И ЗаказПокупателя.Контрагент.ГоловнойКонтрагент.СтранаРегистрации = Значение(Справочник.СтраныМира.Россия)
	|		Тогда ЗаказПокупателя.Контрагент.ГоловнойКонтрагент
	|		Иначе ЗаказПокупателя.Контрагент Конец КАК Покупатель
	|	,Выбор когда ЗаказПокупателя.Грузополучатель.ГоловнойКонтрагент <> Значение(Справочник.Контрагенты.ПустаяСсылка)
	...
	|		Иначе ЗаказПокупателя.Контрагент Конец КАК ОбособленноеПодразделениеПокупателя
	|	,ЗаказПокупателя.БанковскийСчетКонтрагента КАК БанковскийСчетКонтрагента
	|	,Выбор когда ЗаказПокупателя.Грузополучатель <> Значение(Справочник.Контрагенты.ПустаяСсылка)
	|		Тогда ЗаказПокупателя.Грузополучатель
	|		Иначе ЗаказПокупателя.Контрагент Конец КАК Грузополучатель
	|	,ЗаказПокупателя.АдресДоставки КАК АдресДоставки
	|	,ЗаказПокупателя.КонтактноеЛицоПодписант.Наименование КАК РасшифровкаПодписиКонтрагента
	...
	|		Иначе ЗаказПокупателя.ВалютаДокумента Конец КАК ВалютаДокумента
	|	,Выбор когда &ПечатнаяФормаТолькоВРублях = Истина 
	|		Тогда НациональнаяВалюта.Значение.НаименованиеПолное
	|		Иначе ЗаказПокупателя.ВалютаДокумента.НаименованиеПолное Конец КАК ВалютаНаименование
	|	,Выбор когда &ПечатнаяФормаТолькоВРублях = Истина 
	|		Тогда НациональнаяВалюта.Значение.Код
	|		Иначе ЗаказПокупателя.ВалютаДокумента.Код Конец КАК ВалютаКод
	...
	|			Тогда
	|				Выбор когда ЗаказПокупателя.Ссылка.Договор.РасчетыВУсловныхЕдиницах И ЗаказПокупателя.Ссылка.ВалютаДокумента <> &НациональнаяВалюта
	|					Тогда Выразить((СуммаАвтоматическойСкидки + СуммаСкидкиНаценки) * ЗаказПокупателя.Ссылка.Курс / ЗаказПокупателя.Ссылка.Кратность КАК Число(15,2))
	...
	|			Тогда
	|				Выбор когда ЗаказПокупателя.Ссылка.Договор.РасчетыВУсловныхЕдиницах И ЗаказПокупателя.Ссылка.ВалютаДокумента <> &НациональнаяВалюта
	|					Тогда Выразить(Цена * ЗаказПокупателя.Ссылка.Курс / ЗаказПокупателя.Ссылка.Кратность КАК Число(15,2))
	|					Иначе Цена Конец
	|			Иначе Цена Конец КАК Цена
	|		,Выбор когда &ПечатнаяФормаТолькоВРублях
	|			Тогда
	|				Выбор когда ЗаказПокупателя.Ссылка.Договор.РасчетыВУсловныхЕдиницах И ЗаказПокупателя.Ссылка.ВалютаДокумента <> &НациональнаяВалюта
	|					Тогда Выразить(Сумма * ЗаказПокупателя.Курс / ЗаказПокупателя.Кратность КАК Число(15,2))
	|					Иначе Сумма Конец
	|			Иначе Сумма Конец КАК Сумма
	|		,Выбор когда &ПечатнаяФормаТолькоВРублях
	|			Тогда
	|				Выбор когда ЗаказПокупателя.Ссылка.Договор.РасчетыВУсловныхЕдиницах И ЗаказПокупателя.Ссылка.ВалютаДокумента <> &НациональнаяВалюта
	|					Тогда Выразить(СуммаНДС * ЗаказПокупателя.Курс / ЗаказПокупателя.Кратность КАК Число(15,2))
	|					Иначе СуммаНДС Конец
	|			Иначе СуммаНДС Конец КАК СуммаНДС
	|		,Выбор когда &ПечатнаяФормаТолькоВРублях
	|			Тогда
	|				Выбор когда ЗаказПокупателя.Ссылка.Договор.РасчетыВУсловныхЕдиницах И ЗаказПокупателя.Ссылка.ВалютаДокумента <> &НациональнаяВалюта
	|					Тогда Выразить(Всего * ЗаказПокупателя.Курс / ЗаказПокупателя.Кратность КАК Число(15,2))
	|					Иначе Всего Конец
	...
	|	ЗаказПокупателяЗапасы.Номенклатура.ЕдиницаИзмерения.Наименование КАК ЕдиницаИзмеренияПоОКЕИ_Наименование,
	|	ЗаказПокупателяЗапасы.Номенклатура.ЕдиницаИзмерения.Код КАК ЕдиницаИзмеренияПоОКЕИ_Код,
	|	ЗаказПокупателяЗапасы.Характеристика КАК Характеристика,
	...
	|	,ЗаказПокупателяЗапасы.НомерСтроки КАК НомерСтроки
	|	,Выбор Когда ЗаказПокупателяЗапасы.Ссылка.ВалютаДокумента = &НациональнаяВалюта
	|			Тогда ЗаказПокупателяЗапасы.СуммаСкидкиПоСтроке
	|			Иначе Выразить(ЗаказПокупателяЗапасы.СуммаСкидкиПоСтроке * ЗаказПокупателяЗапасы.Ссылка.Курс / ЗаказПокупателяЗапасы.Ссылка.Кратность КАК ЧИСЛО(15, 2)) Конец КАК СуммаСкидкиПоСтроке
	|	,Выбор когда &ПечатнаяФормаТолькоВРублях 
	|		Тогда
	|			Выбор Когда ЗаказПокупателяЗапасы.Ссылка.ВалютаДокумента = &НациональнаяВалюта
	...
	|				Иначе
	|					Выбор Когда ЗаказПокупателяЗапасы.Количество = 0
	|						Тогда (СуммыДокументовРегламентированныйУчет.Всего - СуммыДокументовРегламентированныйУчет.НДС)
	|						Иначе Выразить((СуммыДокументовРегламентированныйУчет.Всего - СуммыДокументовРегламентированныйУчет.НДС) / ЗаказПокупателяЗапасы.Количество КАК ЧИСЛО(15, 2)) Конец
	|			Конец
	|		Иначе ЗаказПокупателяЗапасы.Цена Конец КАК Цена
	|	,Выбор когда &ПечатнаяФормаТолькоВРублях 
	|		Тогда
	|			Выбор Когда ЗаказПокупателяЗапасы.Ссылка.ВалютаДокумента = &НациональнаяВалюта
	...
	|				Иначе
	|					Выбор Когда ЗаказПокупателяЗапасы.Ссылка.СуммаВключаетНДС
	...
	|			Конец
	|		Иначе ЗаказПокупателяЗапасы.Сумма Конец КАК Сумма
	|	,Выбор когда &ПечатнаяФормаТолькоВРублях 
	|		Тогда
	|			Выбор Когда ЗаказПокупателяЗапасы.Ссылка.ВалютаДокумента = &НациональнаяВалюта
	...
	|			Конец
	|		Иначе ЗаказПокупателяЗапасы.СуммаНДС Конец КАК СуммаНДС
	|	,Выбор когда &ПечатнаяФормаТолькоВРублях 
	|		Тогда
	|			Выбор Когда ЗаказПокупателяЗапасы.Ссылка.ВалютаДокумента = &НациональнаяВалюта
	...
	|			Конец
	|		Иначе ЗаказПокупателяЗапасы.Всего Конец КАК Всего
	|	,ЗаказПокупателяЗапасы.Ссылка КАК Ссылка
	|	,ЗаказПокупателяЗапасы.НоменклатураНабора КАК НоменклатураНабора
	|	,ЗаказПокупателяЗапасы.ХарактеристикаНабора КАК ХарактеристикаНабора
	...
	|	,ЗаказПокупателяЗапасы.СтранаСсылка
	|	,ЗаказПокупателяЗапасы.СтранаПредставление
	|	,ЗаказПокупателяЗапасы.СтранаКод
	|	,ЗаказПокупателяЗапасы.ПредставлениеГТД
	|	,Выбор когда ЗаказПокупателяЗапасы.Номенклатура.ТипНоменклатуры = Значение(Перечисление.ТипыНоменклатуры.Запас)
	...
	|	ВЫБОР КОГДА &ПечатнаяФормаТолькоВРублях
	|			ТОГДА 
	|				Выбор когда ЗаказПокупателяРаботы.Ссылка.Договор.РасчетыВУсловныхЕдиницах И ЗаказПокупателяРаботы.Ссылка.ВалютаДокумента <> &НациональнаяВалюта
	...
	|					Иначе (ЗаказПокупателяРаботы.СуммаАвтоматическойСкидки + ЗаказПокупателяРаботы.СуммаСкидкиНаценки) Конец
	|			Иначе (ЗаказПокупателяРаботы.СуммаАвтоматическойСкидки + ЗаказПокупателяРаботы.СуммаСкидкиНаценки) Конец КАК СуммаСкидкиПоСтроке,
	|	Выбор когда &ПечатнаяФормаТолькоВРублях
	|			Тогда 
	|				Выбор когда ЗаказПокупателяРаботы.Ссылка.Договор.РасчетыВУсловныхЕдиницах И ЗаказПокупателяРаботы.Ссылка.ВалютаДокумента <> &НациональнаяВалюта
	...
	|					Иначе ЗаказПокупателяРаботы.Цена Конец
	|			Иначе ЗаказПокупателяРаботы.Цена Конец КАК Цена,
	|	Выбор когда &ПечатнаяФормаТолькоВРублях
	|			Тогда
	|				Выбор когда ЗаказПокупателяРаботы.Ссылка.Договор.РасчетыВУсловныхЕдиницах И ЗаказПокупателяРаботы.Ссылка.ВалютаДокумента <> &НациональнаяВалюта
	...
	|					Иначе ЗаказПокупателяРаботы.Сумма Конец
	|			Иначе ЗаказПокупателяРаботы.Сумма Конец КАК Сумма,
	|	Выбор когда &ПечатнаяФормаТолькоВРублях
	|			Тогда
	|				Выбор когда ЗаказПокупателяРаботы.Ссылка.Договор.РасчетыВУсловныхЕдиницах И ЗаказПокупателяРаботы.Ссылка.ВалютаДокумента <> &НациональнаяВалюта
	...
	|					Иначе ЗаказПокупателяРаботы.СуммаНДС Конец
	|			Иначе ЗаказПокупателяРаботы.СуммаНДС Конец КАК СуммаНДС,
	|	Выбор когда &ПечатнаяФормаТолькоВРублях
	|			Тогда
	|				Выбор когда ЗаказПокупателяРаботы.Ссылка.Договор.РасчетыВУсловныхЕдиницах И ЗаказПокупателяРаботы.Ссылка.ВалютаДокумента <> &НациональнаяВалюта
	...
	|					Иначе ЗаказПокупателяРаботы.Всего Конец
	|			Иначе ЗаказПокупателяРаботы.Всего Конец КАК Всего,
	...
	|	,ЗаказПокупателяРаботы.СтавкаНДС КАК СтавкаНДС
	|	,ЗаказПокупателяРаботы.НомерСтроки КАК НомерСтроки
	|	,Выбор Когда ЗаказПокупателяРаботы.Ссылка.ВалютаДокумента = &НациональнаяВалюта
	|			Тогда ЗаказПокупателяРаботы.СуммаСкидкиПоСтроке
	|			Иначе Выразить(ЗаказПокупателяРаботы.СуммаСкидкиПоСтроке * ЗаказПокупателяРаботы.Ссылка.Курс / ЗаказПокупателяРаботы.Ссылка.Кратность КАК ЧИСЛО(15, 2)) Конец КАК СуммаСкидкиПоСтроке
	|	,Выбор когда &ПечатнаяФормаТолькоВРублях 
	|		Тогда
	|			Выбор Когда ЗаказПокупателяРаботы.Ссылка.ВалютаДокумента = &НациональнаяВалюта
	...
	|					Тогда Выразить(ЗаказПокупателяРаботы.Цена * ЗаказПокупателяРаботы.Ссылка.Курс / ЗаказПокупателяРаботы.Ссылка.Кратность КАК ЧИСЛО(15, 2))
	|				Иначе
	|					Выбор Когда ЗаказПокупателяРаботы.Количество = 0
	|						Тогда (СуммыДокументовРегламентированныйУчет.Всего - СуммыДокументовРегламентированныйУчет.НДС)
	|						Иначе Выразить((СуммыДокументовРегламентированныйУчет.Всего - СуммыДокументовРегламентированныйУчет.НДС) / ЗаказПокупателяРаботы.Количество КАК ЧИСЛО(15, 2)) Конец
	|			Конец
	|		Иначе ЗаказПокупателяРаботы.Цена Конец КАК Цена
	|	,Выбор когда &ПечатнаяФормаТолькоВРублях 
	|		Тогда
	...
	|					Тогда Выразить(ЗаказПокупателяРаботы.Сумма * ЗаказПокупателяРаботы.Ссылка.Курс / ЗаказПокупателяРаботы.Ссылка.Кратность КАК ЧИСЛО(15, 2))
	|				Иначе
	|					Выбор Когда ЗаказПокупателяРаботы.Ссылка.СуммаВключаетНДС
	|						Тогда Выразить(СуммыДокументовРегламентированныйУчет.Всего КАК ЧИСЛО(15, 2))
	|						Иначе Выразить(СуммыДокументовРегламентированныйУчет.Всего - СуммыДокументовРегламентированныйУчет.НДС КАК ЧИСЛО(15, 2)) Конец
	|			Конец
	|		Иначе ЗаказПокупателяРаботы.Сумма Конец КАК Сумма
	...
	|	,Выбор когда ЗаказПокупателяРаботы.Номенклатура.ТипНоменклатуры = Значение(Перечисление.ТипыНоменклатуры.Запас)
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ПечатнаяФорма <> Неопределено Тогда
		...
		СформироватьСводныйОтчет(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки);
	...
	Если ПечатнаяФорма <> Неопределено Тогда
		...
		ПечатнаяФорма.ТабличныйДокумент = ПечатьСводныйОтчетЗаказНаряд(МассивОбъектов, ОбъектыПечати, Ошибки);
	...
	//EFSOL Лохоня Ю.А. 13.09.2017 11:56:32	
	...
	Если ПечатнаяФорма <> Неопределено Тогда
		...
		ПечатнаяФорма.ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЗаказНаДоставку_ES_Стикер";
		ПечатнаяФорма.ПолныйПутьКМакету = "Документ.ЗаказПокупателя.ПФ_MXL_ES_Стикер";
		ПечатнаяФорма.СинонимМакета = НСтр("ru ='Стикер'");
		
	КонецЕсли;
	
	Если МассивОбъектов[0].Контрагент.ES_ВидКонтрагента = Перечисления.ES_ВидКонтрагента.В2В  Тогда	
		ПечатнаяФорма = УправлениеПечатью.СведенияОПечатнойФорме(КоллекцияПечатныхФорм, "ПФ_MXL_ES_НакладнаяB2B");
		Если ПечатнаяФорма <> Неопределено Тогда
			
			ПечатнаяФорма.ТабличныйДокумент = ПечатьESНакладнаяB2B(МассивОбъектов, ОбъектыПечати, "ПФ_MXL_ES_НакладнаяB2B");
			ПечатнаяФорма.ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЗаказНаДоставку_ES_НакладнаяB2B";
			ПечатнаяФорма.ПолныйПутьКМакету = "Документ.ЗаказПокупателя.ПФ_MXL_ES_НакладнаяB2B";
			ПечатнаяФорма.СинонимМакета = НСтр("ru ='Накладная B2B'");
			
		КонецЕсли;
		
	ИначеЕсли МассивОбъектов[0].Контрагент.ES_ВидКонтрагента = Перечисления.ES_ВидКонтрагента.В2С  Тогда
		МакетВ2В = ?(ТипЗнч(КоллекцияПечатныхФорм) = Тип("ТаблицаЗначений") И КоллекцияПечатныхФорм.Количество() > 0,
		КоллекцияПечатныхФорм.Найти("ПФ_MXL_ES_НакладнаяB2B", "ИмяМакета"),
		Неопределено);
		Если ЗначениеЗаполнено(МакетВ2В) Тогда
			КоллекцияПечатныхФорм.Удалить(МакетВ2В);	
		КонецЕсли;
		...
		Если ПечатнаяФорма <> Неопределено Тогда
			...
			ПечатнаяФорма.ТабличныйДокумент = ПечатьESНакладнаяB2C(МассивОбъектов, ОбъектыПечати, "ПФ_MXL_ES_НакладнаяB2C");
			ПечатнаяФорма.ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЗаказНаДоставку_ES_НакладнаяB2C";
			ПечатнаяФорма.ПолныйПутьКМакету = "Документ.ЗаказПокупателя.ПФ_MXL_ES_НакладнаяB2C";
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	//EFSOL_oks_kry_26.03.2015 17:34:10_BEGIN
	...
	//КомандаПечати.СписокФорм = "ES_ФормаЗаказаНаДоставку,ES_ФормаСпискаЗаказаНаДоставку";
	//КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	//КомандаПечати.МестоРазмещения = "ГруппаВажныеКомандыЗаказПокупателя";
	//КомандаПечати.Порядок = 10;
	////
	//Лохоня Ю.А.
// Стикер
	...
	КомандаПечати.СписокФорм = "ES_ФормаЗаказаНаДоставку,ES_ФормаСпискаЗаказаНаДоставкуТЗ";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	//КомандаПечати.МестоРазмещения = "ГруппаВажныеКомандыЗаказПокупателя";
	КомандаПечати.Порядок = 10;	
	//Лохоня Ю.А.
	...
	//ЭР Несторук С.И. 16.03.2017 11:57:36 {
	//Временно скрываем эту печ. форму
	// Накладная на запасы
	//КомандаПечати = КомандыПечати.Добавить();
	...
	//ЭР Несторук С.И. 19.04.2017 10:52:40 {
	//// Расписка о материальной ответственности курьера
	//КомандаПечати = КомандыПечати.Добавить();
	//КомандаПечати.Идентификатор = "ПФ_MXL_ES_РаспискаОМатериальнойОтветственностиКурьера";
	//КомандаПечати.Представление = НСтр("ru = 'Расписка о материальной ответственности курьера'");
	//КомандаПечати.СписокФорм = "ES_ФормаЗаказаНаДоставку,ES_ФормаСпискаЗаказаНаДоставку";
	//КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	//КомандаПечати.МестоРазмещения = "ГруппаВажныеКомандыЗаказПокупателя";
	...
	//EFSOL_oks_kry_26.03.2015 17:34:10_END
КонецПроцедуры
...
//EFSOL_oks_kry_19.03.2015 19:59:05_BEGIN

// Инициализирует таблицы значений, содержащие данные табличных частей документа.
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура ИнициализироватьДанныеДокументаЗаказНаДоставку(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства) 
	...
	|	ЗаказПокупателяЗапасы.Ссылка = &Ссылка
	|	И НЕ ЗаказПокупателяЗапасы.Ссылка.ВариантЗавершения = ЗНАЧЕНИЕ(Перечисление.ВариантыЗавершенияЗаказа.Отменен)
	|	И ЗаказПокупателяЗапасы.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЗаказПокупателя.ES_ЗаказНаДоставку)
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки
	|;
	...
	|	И ЗаказПокупателяКалькуляция.Ссылка.КалькуляцияРассчитана
	|	И ЗаказПокупателяКалькуляция.Себестоимость <> 0
	|	И ЗаказПокупателяКалькуляция.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЗаказПокупателя.ES_ЗаказНаДоставку)
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗаказПокупателяКалькуляция.Ссылка.Дата,
	|	ЗаказПокупателяКалькуляция.Ссылка,
	...
	//ЭР Несторук С.И. 30.03.2017 21:29:56 {
	//ТаблицаЗаказыПокупателей =  МассивРезультатов[0].Выгрузить();
	//
	//Если ДокументСсылкаЗаказПокупателя.ES_НППлан > 0 Тогда
	//	НоваяСтрока = ТаблицаЗаказыПокупателей.Добавить();
	//	НоваяСтрока.НомерСтроки 	= ТаблицаЗаказыПокупателей.Количество()+1;
	//	НоваяСтрока.Период		    = ДокументСсылкаЗаказПокупателя.Дата;
	...
	//	НоваяСтрока.Номенклатура	= ES_ОбщегоНазначения.ПолучитьСтартовуюНастройку(Перечисления.ES_ВидыСтартовыхНастроек.НоменклатураДляЧека);
	//	НоваяСтрока.ДатаОтгрузки	= ДокументСсылкаЗаказПокупателя.ДатаОтгрузки;
	//КонецЕсли;
	//}ЭР Несторук С.И.
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаЗапасыТоварыЗаказаНаДоставку(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
	...
	"ВЫБРАТЬ
	|	ЗаказНаДоставкуТовары.НомерСтроки КАК НомерСтроки,
	|	ЗаказНаДоставкуТовары.Ссылка.Дата КАК Период,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
	//|	ВЫБОР
	//|		КОГДА ЗаказНаДоставкуТовары.Ссылка.СостояниеЗаказа.СтатусЗаказа = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказов.Выполнен)
	//|			ТОГДА ИСТИНА
	//|		ИНАЧЕ ЛОЖЬ
	//|	КОНЕЦ КАК Выполнен,
	|	&Организация КАК Организация,
	|	ЗНАЧЕНИЕ(Справочник.СценарииПланирования.Фактический) КАК СценарийПланирования,
	|	ЗаказНаДоставкуТовары.Ссылка КАК Документ,
	|	ЗаказНаДоставкуТовары.Ссылка КАК ДокументПродажи,
	|	ЗаказНаДоставкуТовары.Ссылка КАК ЗаказПродажи,
	|	ЗаказНаДоставкуТовары.Ссылка КАК Заказ,
	|	ЗаказНаДоставкуТовары.Ссылка.СтруктурнаяЕдиницаПродажи КАК Подразделение,
	|	ЗаказНаДоставкуТовары.Ссылка.Ответственный КАК Ответственный,
	|	ЗаказНаДоставкуТовары.Номенклатура.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ЗаказНаДоставкуТовары.Ссылка.СтруктурнаяЕдиницаПродажи КАК ПодразделениеПродажи,
	|	ЗаказНаДоставкуТовары.Номенклатура.НаправлениеДеятельности.СчетУчетаСебестоимостиПродаж КАК СчетУчетаСебестоимость,
	|	НЕОПРЕДЕЛЕНО КАК КоррОрганизация,
	|	ЕСТЬNULL(ЗаказНаДоставкуТовары.Ссылка.СтруктурнаяЕдиницаРезерв, ЗНАЧЕНИЕ(Справочник.СтруктурныеЕдиницы.ПустаяСсылка)) КАК СтруктурнаяЕдиница,
	|	ЕСТЬNULL(НЕОПРЕДЕЛЕНО, ЗНАЧЕНИЕ(Справочник.СтруктурныеЕдиницы.ПустаяСсылка)) КАК КоррСтруктурнаяЕдиница,
	|	ЗаказНаДоставкуТовары.Номенклатура.СчетУчетаЗапасов КАК СчетУчета,
	|	НЕОПРЕДЕЛЕНО КАК КоррСчетУчета,
	|	ВЫБОР
	|		КОГДА &ИспользоватьПартии
	|				И ЗаказНаДоставкуТовары.Партия.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПартий.ТоварыНаКомиссии)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ТоварыНаКомиссии,
	|	ЗаказНаДоставкуТовары.Номенклатура КАК Номенклатура,
	|	НЕОПРЕДЕЛЕНО КАК КоррНоменклатура,
	|	ВЫБОР
	|		КОГДА &ИспользоватьХарактеристики
	|			ТОГДА ЗаказНаДоставкуТовары.Характеристика
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
	|	КОНЕЦ КАК Характеристика,
	|	НЕОПРЕДЕЛЕНО КАК КоррХарактеристика,
	|	ВЫБОР
	|		КОГДА &ИспользоватьПартии
	|			ТОГДА ЗаказНаДоставкуТовары.Партия
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка)
	|	КОНЕЦ КАК Партия,
	...
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ЗаказНаДоставкуТовары.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
	|			ТОГДА ЗаказНаДоставкуТовары.Количество
	|		ИНАЧЕ ЗаказНаДоставкуТовары.Количество * ЗаказНаДоставкуТовары.ЕдиницаИзмерения.Коэффициент
	|	КОНЕЦ КАК Количество,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ЗаказНаДоставкуТовары.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
	|			ТОГДА ЗаказНаДоставкуТовары.Резерв
	|		ИНАЧЕ ЗаказНаДоставкуТовары.Резерв * ЗаказНаДоставкуТовары.ЕдиницаИзмерения.Коэффициент
	|	КОНЕЦ КАК Резерв,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ЗаказНаДоставкуТовары.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
	|			ТОГДА ЗаказНаДоставкуТовары.РезервОтгрузка
	|		ИНАЧЕ ЗаказНаДоставкуТовары.РезервОтгрузка * ЗаказНаДоставкуТовары.ЕдиницаИзмерения.Коэффициент
	|	КОНЕЦ КАК РезервОтгрузка,
	|	ЗаказНаДоставкуТовары.СтавкаНДС КАК СтавкаНДС,
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ЗаказНаДоставкуТовары.Ссылка.НДСВключатьВСтоимость
	|				ТОГДА 0
	|			ИНАЧЕ ВЫБОР
	|					КОГДА ЗаказНаДоставкуТовары.Ссылка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|						ТОГДА ЗаказНаДоставкуТовары.СуммаНДС * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|					ИНАЧЕ ЗаказНаДоставкуТовары.СуммаНДС * ЗаказНаДоставкуТовары.Ссылка.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * ЗаказНаДоставкуТовары.Ссылка.Кратность)
	|				КОНЕЦ
	|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаНДС,
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ЗаказНаДоставкуТовары.Ссылка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|				ТОГДА ЗаказНаДоставкуТовары.Всего * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|			ИНАЧЕ ЗаказНаДоставкуТовары.Всего * ЗаказНаДоставкуТовары.Ссылка.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * ЗаказНаДоставкуТовары.Ссылка.Кратность)
	|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК Сумма,
	|	0 КАК Себестоимость,
	|	ЛОЖЬ КАК ФиксированнаяСтоимость,
	|	ЗаказНаДоставкуТовары.Номенклатура.НаправлениеДеятельности.СчетУчетаСебестоимостиПродаж КАК СчетДт,
	|	ЗаказНаДоставкуТовары.Номенклатура.СчетУчетаЗапасов КАК СчетКт,
	|	ВЫРАЗИТЬ(&СписаниеЗапасов КАК СТРОКА(100)) КАК Содержание,
	|	ВЫРАЗИТЬ(&СписаниеЗапасов КАК СТРОКА(100)) КАК СодержаниеПроводки
	|ИЗ
	|	Документ.ЗаказПокупателя.Запасы КАК ЗаказНаДоставкуТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(
	|				&МоментВремени,
	|				Валюта В
	|					(ВЫБРАТЬ
	...
	|	Константа.НациональнаяВалюта КАК КонстантаНациональнаяВалюта
	|ГДЕ
	|	ЗаказНаДоставкуТовары.Ссылка = &Ссылка
	|	И ЗаказНаДоставкуТовары.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЗаказПокупателя.ES_ЗаказНаДоставку)
	//|	И НЕ ЗаказНаДоставкуТовары.Ссылка.СостояниеЗаказа.СтатусЗаказа = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказов.Открыт)
	//|	И НЕ(ЗаказНаДоставкуТовары.Ссылка.СостояниеЗаказа.СтатусЗаказа = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказов.ВРаботе)
	//|				И ЗаказНаДоставкуТовары.Ссылка.Закрыт)
	|	И ЗаказНаДоставкуТовары.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Запас)";
	...
	"ВЫБРАТЬ
	|	ЗаказНаДоставкуТовары.НомерСтроки КАК НомерСтроки,
	|	ЗаказНаДоставкуТовары.Ссылка.Финиш КАК Период,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
	//|	ВЫБОР
	//|		КОГДА ЗаказНаДоставкуТовары.Ссылка.СостояниеЗаказа.СтатусЗаказа = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказов.Выполнен)
	//|			ТОГДА ИСТИНА
	...
	|	ВЫРАЗИТЬ(&СписаниеЗапасов КАК СТРОКА(100)) КАК Содержание,
	|	ВЫРАЗИТЬ(&СписаниеЗапасов КАК СТРОКА(100)) КАК СодержаниеПроводки
	|ПОМЕСТИТЬ ВременнаяТаблицаТоварыНаДоставку
	|ИЗ
	|	Документ.ЗаказПокупателя.Запасы КАК ЗаказНаДоставкуТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(
	|				&МоментВремени,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВременнаяТаблицаТоварыНаДоставку.Организация КАК Организация,
	|	ВременнаяТаблицаТоварыНаДоставку.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
	|	ВременнаяТаблицаТоварыНаДоставку.СчетУчета КАК СчетУчета,
	|	ВременнаяТаблицаТоварыНаДоставку.Номенклатура КАК Номенклатура,
	|	ВременнаяТаблицаТоварыНаДоставку.Характеристика КАК Характеристика,
	|	ВременнаяТаблицаТоварыНаДоставку.Партия КАК Партия,
	|	ВременнаяТаблицаТоварыНаДоставку.ЗаказПокупателя КАК ЗаказПокупателя
	|ИЗ
	|	ВременнаяТаблицаТоварыНаДоставку КАК ВременнаяТаблицаТоварыНаДоставку
	|
	|СГРУППИРОВАТЬ ПО
	|	ВременнаяТаблицаТоварыНаДоставку.Организация,
	|	ВременнаяТаблицаТоварыНаДоставку.СтруктурнаяЕдиница,
	|	ВременнаяТаблицаТоварыНаДоставку.СчетУчета,
	|	ВременнаяТаблицаТоварыНаДоставку.Номенклатура,
	|	ВременнаяТаблицаТоварыНаДоставку.Характеристика,
	|	ВременнаяТаблицаТоварыНаДоставку.Партия,
	|	ВременнаяТаблицаТоварыНаДоставку.ЗаказПокупателя";
    ...
    |						КОНЕЦ
    |					ИЗ
    |						ВременнаяТаблицаТоварыНаДоставку КАК ТаблицаЗапасы
    |					ГДЕ
    ...
    |						ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
    |					ИЗ
    |						ВременнаяТаблицаТоварыНаДоставку КАК ТаблицаЗапасы)) КАК ЗапасыОстатки
    |	
    |	СГРУППИРОВАТЬ ПО
    |		ЗапасыОстатки.Организация,
    |		ЗапасыОстатки.СтруктурнаяЕдиница,
...
//EFSOL_oks_kry_26.03.2015 17:51:56_BEGIN

Функция ПечатьESНакладнаяНаДоставку(МассивОбъектов, ОбъектыПечати, ИмяМакета) Экспорт
	
	ТабДок = Новый ТабличныйДокумент;
	ТабДок.АвтоМасштаб = Истина;
	ТабДок.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЗаказНаДоставку_ES_НакладнаяНаДоставку";
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ЗаказПокупателя."+ ИмяМакета);
	...
	Запрос.Текст = 
		...
		|	ЗаказПокупателя.Ответственный,
		|	ЗаказПокупателя.ES_АдресДоставки КАК АдресДоставки,
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока ВыборкаСсылка.Следующий() Цикл
			...
			ОбластьШапка.Рисунки.Штрих.Картинка = ES_ПолучитьКартинкуШтрихкода(ВнешняяКомпонента,номерШК , 70, 50);

			ОбластьШапка.Параметры.Организация = ВыборкаСсылка.Организация;
			ОбластьШапка.Параметры.ВидДоставки = ВыборкаСсылка.ВидДоставки;
			ОбластьШапка.Параметры.Заказчик = ВыборкаСсылка.Заказчик;
			ОбластьШапка.Параметры.ЗаказчикКонтактноеЛицо = ВыборкаСсылка.ЗаказчикКонтактноеЛицо;
			ОбластьШапка.Параметры.ЗаказчикТелефон = ВыборкаСсылка.ЗаказчикТелефон;
			ОбластьШапка.Параметры.Получатель = ВыборкаСсылка.Получатель;
			ОбластьШапка.Параметры.ПолучательКонтактноеЛицо = ВыборкаСсылка.ПолучательКонтактноеЛицо;
			ОбластьШапка.Параметры.ПолучательТелефон = ВыборкаСсылка.ПолучательТелефон;
			ОбластьШапка.Параметры.Плательщик = ВыборкаСсылка.Плательщик;
			ОбластьШапка.Параметры.СтоимостьДоставки = ВыборкаСсылка.СтоимостьДоставки;
			ОбластьШапка.Параметры.НППлан = ВыборкаСсылка.НППлан;
			ОбластьШапка.Параметры.АдресДоставки = ВыборкаСсылка.АдресДоставки;
			ОбластьШапка.Параметры.АдресЗабора = ВыборкаСсылка.ЗаборАдрес;
			ОбластьШапка.Параметры.Отправитель = ВыборкаСсылка.Отправитель;
			ОбластьШапка.Параметры.ТелефонОтправителя = ВыборкаСсылка.ТелефонОтправителя;
			ОбластьШапка.Параметры.Описание = ВыборкаСсылка.Описание;
			ОбластьШапка.Параметры.Вес = ВыборкаСсылка.Вес;
...
Функция ПечатьESНаклейкаЗаказа(МассивОбъектов, ОбъектыПечати, ИмяМакета) Экспорт
	
	ТабДок = Новый ТабличныйДокумент;
	ТабДок.АвтоМасштаб = Истина;
	ТабДок.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЗаказНаДоставку_ES_НаклейкаЗаказа";
	...
	Для Каждого МассивОбъект Из МассивОбъектов Цикл 
		...
		Если НЕ МассивОбъект.ES_КоличествоМест = 0 Тогда 
			
			НачЗнач = 1;
			
			Для Счетчик = НачЗнач  по  МассивОбъект.ES_КоличествоМест Цикл   			
				
				Если МассивОбъект.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ES_ЗаказНаДоставку И МассивОбъект.ПометкаУдаления = Ложь тогда
					
					ВнешняяКомпонента = МенеджерОборудованияСерверПовтИсп.ПодключитьВнешнююКомпонентуПечатиШтрихкода(); 
					
					НомерСтрокиНачало = ТабДок.ВысотаТаблицы + 1; 					
					
					НомерШК = МассивОбъект.ES_НомерНакладной;
					//ЭР Несторук С.И. 14.07.2017 13:57:16 {
					НомерДокБезПрефикса = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(МассивОбъект.Номер,Истина,Истина);
					//ПолеАдрес = "Адрес доставки: "+ МассивОбъект.ES_АдресДоставки;
					//ПолеВидДоставки = "Вид доставки: " + Строка(МассивОбъект.ES_ВидДоставки);
					//ПолеНомер = "Номер заказа: " + НомерДокБезПрефикса;
					//ПолеВхНакладная = "Вх. накладная: "+ МассивОбъект.ES_ВхНакладная;
					//
					//ПолеАдрес = МассивОбъект.ES_АдресДоставки;
					//ПолеВидДоставки = Строка(МассивОбъект.ES_ВидДоставки);
					ПолеНомер = "№ "+ НомерДокБезПрефикса;
					//ПолеМетро = МассивОбъект.ES_ПолучательРегион.Наименование;
					//ПолеВхНакладная = "Вх. № "+ МассивОбъект.ES_ВхНакладная;

					//НомерШК = ПолеАдрес + "|" + ПолеВидДоставки+ "|" + ПолеНомер +"|"+ ПолеВхНакладная;
					//НомерШК = ПолеАдрес + Символы.ПС + ПолеМетро + Символы.ПС + ПолеВидДоставки+ Символы.ПС + ПолеНомер +Символы.ПС+ ПолеВхНакладная;
					//НомерШК = СтрЗаменить(ПолеВидДоставки+ "|" + ПолеНомер +"|"+ ПолеВхНакладная, " ", "");
					//}ЭР Несторук С.И.
					//ОбластьПеч.Рисунки.Штрих.Картинка = ПолучитьКартинкуШтрихкода(ВнешняяКомпонента,номерШК , 25, 40);
					ОбластьПеч.Рисунки.Штрих.Картинка = ES_ПолучитьКартинкуШтрихкода(ВнешняяКомпонента,номерШК , 70, 50);
					//ОбластьПеч.Рисунки.Штрих.Картинка =   ЭР_ПолучитьQRШтрихКод(НомерШК);
					...
					//ОбластьПеч.Параметры.Получатель 	= МассивОбъект.ES_Получатель + ?(ЗначениеЗаполнено(МассивОбъект.ES_ПолучательТелефон), ", тел. " + МассивОбъект.ES_ПолучательТелефон, "");
					ОбластьПеч.Параметры.ВхНакладная 	= МассивОбъект.ES_ВхНакладная;			
					ОбластьПеч.Параметры.ВидДоставки	= Строка(МассивОбъект.ES_ВидДоставки);			
					ОбластьПеч.Параметры.НомерЗаказа 	= ПолеНомер;			

					Если МассивОбъект.ES_ВидДоставки = Перечисления.ES_ВидыДоставки.СкладСклад Тогда 						
						ОбластьПеч.Параметры.АдресДоставки = МассивОбъект.ES_АдресСкладаДоставки;						
					Иначе						
						ОбластьПеч.Параметры.АдресДоставки = МассивОбъект.ES_АдресДоставки;
					КонецЕсли;
					
					//ES_Коваленко М. 12.07.2016 {
					Если СчетчикДокументов % 10 = 0 Тогда
						ТабДок.ВывестиГоризонтальныйРазделительСтраниц();
					КонецЕсли;
					
					//ТабДок.Вывести(ОбластьПеч);
					Если (СчетчикДокументов) % 2 = 1 Тогда
						ТабДок.Вывести(ОбластьПеч);
					Иначе
						ТабДок.Присоединить(ОбластьПеч);
					КонецЕсли;
					// } ES_Коваленко
						
					//УправлениеПечатью.ЗадатьОбластьПечатиДокумента(ТабДок, НомерСтрокиНачало, ОбъектыПечати, МассивОбъект);
		...
		Иначе 
			Если МассивОбъект.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ES_ЗаказНаДоставку И МассивОбъект.ПометкаУдаления = Ложь тогда
				...
				ПолеАдрес = МассивОбъект.ES_АдресДоставки;
				ПолеВидДоставки = ЭР_ПолучитьВидДоставки(МассивОбъект.ES_ВидДоставки);
				ПолеНомер = НомерДокБезПрефикса;
				ПолеВхНакладная = МассивОбъект.ES_ВхНакладная;
				НомерШК = СтрЗаменить(ПолеВидДоставки+ "|" + ПолеНомер +"|"+ ПолеВхНакладная, " ", "");
				//ОбластьПеч.Рисунки.Штрих.Картинка = ПолучитьКартинкуШтрихкода(ВнешняяКомпонента,номерШК , 25, 40);
				ОбластьПеч.Рисунки.Штрих.Картинка = ES_ПолучитьКартинкуШтрихкода(ВнешняяКомпонента,номерШК , 70, 50);
				ВнешняяКомпонента = Неопределено;
	...
	//				НомерСтрокиНачало = ТабДок.ВысотаТаблицы + 1;
	//				
	//				ОбластьПеч = Макет.ПолучитьОбласть("Наклейка"); 					
	//				
	//				НомерШК = МассивОбъект.ES_НомерНакладной;
	//				
	//				ОбластьПеч.Рисунки.Штрих.Картинка = ПолучитьКартинкуШтрихкода(ВнешняяКомпонента,номерШК , 25, 40);
	...
	//			
	//			НомерДокБезПрефикса = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(МассивОбъект.Номер,Истина,Истина);
	//			НомерШК = СформироватьШтрихкод(НомерДокБезПрефикса);
	//			
	//			ОбластьПеч.Рисунки.Штрих.Картинка = ПолучитьКартинкуШтрихкода(ВнешняяКомпонента,номерШК , 25, 40); 
	//			ОбластьПеч.Параметры.Организация = "" + МассивОбъект.Контрагент + ", тел. " + МассивОбъект.ES_ЗаказчикТелефон;
	//			ОбластьПеч.Параметры.Получатель = МассивОбъект.ES_Получатель.Наименование + ", тел. " + МассивОбъект.ES_ПолучательТелефон;
...
Функция ПечатьESСтикер(МассивОбъектов, ОбъектыПечати, ИмяМакета) Экспорт
	...
	ТабДок.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЗаказНаДоставку_ES_Стикер";
		...
		Для Каждого МассивОбъект Из МассивОбъектов Цикл
			...
			Если НЕ МассивОбъект.ES_КоличествоМест = 0 Тогда 
				...
				Для Счетчик = НачЗнач  по  МассивОбъект.ES_КоличествоМест Цикл
					...
					Если МассивОбъект.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ES_ЗаказНаДоставку И МассивОбъект.ПометкаУдаления = Ложь тогда
						...
						//ОбластьПеч.Параметры.Организация 	= "" + МассивОбъект.Контрагент + ?(ЗначениеЗаполнено(МассивОбъект.ES_ЗаказчикТелефон), ", тел. " + МассивОбъект.ES_ЗаказчикТелефон, "");
						//ОбластьПеч.Параметры.КоличествоМест = "(" + Строка(Счетчик) + " из " + Строка(МассивОбъект.ES_КоличествоМест) +")";
						ОбластьПеч.Параметры.Получатель 	= МассивОбъект.ES_Получатель;
						ОбластьПеч.Параметры.Адрес 	= МассивОбъект.ES_АдресДоставки;	
						ОбластьПеч.Параметры.Телефон 	= МассивОбъект.ES_ПолучательТелефон;
	
						Док = МассивОбъект.ПолучитьОбъект();
						...
						ОбластьПеч.Параметры.Вес 	= " " + МассивОбъект.ES_ОбщийВес + " кг.";
						//ОбластьПеч.Параметры.Мест = Строка(МассивОбъект.ES_КоличествоМест);
						ОбластьПеч.Параметры.Мест = Строка(Счетчик) + " из " + Строка(МассивОбъект.ES_КоличествоМест);
						//ОбластьПеч.Параметры.Вес 	= МассивОбъект.ES_Грузы[0].Вес;
						//ОбластьПеч.Параметры.Артикул 	= МассивОбъект.ES_Грузы[0].Артикул;
						ОбластьПеч.Параметры.Отправитель 	= МассивОбъект.Контрагент;
						
						//Пока РезультатЗапроса.Выбрать().Следующий() Цикл
						Для Каждого ЭЛ ИЗ ВыборкаНаЗаказПокупателя ЦИКЛ 
							...
							КонецЕсли;
						КонецЦикла;	
						ОбластьПеч.Параметры.Код			= Справочники.ES_Направления.НайтиПоНаименованию(МассивОбъект.ES_Направление).КодНаправления;
			...
			Иначе 
				Если МассивОбъект.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ES_ЗаказНаДоставку И МассивОбъект.ПометкаУдаления = Ложь тогда
					...
					//ОбластьПеч.Параметры.Вес 	= " " + Вес + " кг.";
					ОбластьПеч.Параметры.Вес 	= МассивОбъект.ES_ОбщийВес;
					ОбластьПеч.Параметры.Мест = Строка(МассивОбъект.ES_КоличествоМест);
					//ОбластьПеч.Параметры.Вес 	= МассивОбъект.ES_Грузы[0].Вес;
					//ОбластьПеч.Параметры.Артикул 	= МассивОбъект.ES_Грузы[0].Артикул;
					ОбластьПеч.Параметры.Отправитель 	= МассивОбъект.Контрагент;
...
Функция ПечатьESНакладнаяB2C(МассивОбъектов, ОбъектыПечати, ИмяМакета) Экспорт
	//Лохоня Ю.А.
	ТабДок = Новый ТабличныйДокумент;
	ТабДок.АвтоМасштаб = Истина;
	ТабДок.ОриентацияСтраницы = ОриентацияСтраницы.Портрет;
	ТабДок.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЗаказНаДоставку_ES_НакладнаяB2C";
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ЗаказПокупателя."+ ИмяМакета);
	Если ТипЗнч(Макет)<> Тип("ТабличныйДокумент") Тогда
	Макет = ПолучитьМакет(ИмяМакета);
	КонецЕсли;
...
Функция ПечатьESНакладнаяB2B(МассивОбъектов, ОбъектыПечати, ИмяМакета) Экспорт
//Лохоня Ю.А.
	ТабДок = Новый ТабличныйДокумент;
	ТабДок.АвтоМасштаб = Истина;
	ТабДок.ОриентацияСтраницы = ОриентацияСтраницы.Портрет;
	ТабДок.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЗаказНаДоставку_ES_НакладнаяB2B";
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ЗаказПокупателя."+ ИмяМакета);
	Если ТипЗнч(Макет)<> Тип("ТабличныйДокумент") Тогда
	Макет = ПолучитьМакет(ИмяМакета);
	КонецЕсли;
			...
			Пока ВыборкаСсылка.Следующий() Цикл
				...
				Для Каждого Строка Из Док.ES_Грузы Цикл
					...
					Если Номер = 1 Тогда
						...
						//ОбластьШапка.Параметры.Стоим0 = Строка.Количество * Строка.Цена;
						ОбластьШапка.Параметры.Габ0 = Строка.Объем;//Строка.Объем;
					...
					Если Номер = 2 Тогда
						...
						ОбщееКоличествоТоваров = ОбщееКоличествоТоваров + Строка.Количество;
						//ОбластьШапка.Параметры.Стоим1 = Строка.Количество * Строка.Цена;
						ОбластьШапка.Параметры.Габ1 = Строка.Объем;//Строка.Объем;
					...
					Если Номер = 3 Тогда
						...
						ОбщееКоличествоТоваров = ОбщееКоличествоТоваров + Строка.Количество;
						//ОбластьШапка.Параметры.Стоим2 = Строка.Количество * Строка.Цена;
						ОбластьШапка.Параметры.Габ2 = Строка.Объем;//Строка.Объем;
					...
					Если Номер = 4 Тогда
						...
						ОбщееКоличествоТоваров = ОбщееКоличествоТоваров + Строка.Количество;
						//ОбластьШапка.Параметры.Стоим3 = Строка.Количество * Строка.Цена;
						ОбластьШапка.Параметры.Габ3 = Строка.Объем;//Строка.Объем;
					...
					Если Номер = 6 Тогда
						...
						ОбщееКоличествоТоваров = ОбщееКоличествоТоваров + Строка.Количество;
						//ОбластьШапка.Параметры.Стоим5 = Строка.Количество * Строка.Цена;
						ОбластьШапка.Параметры.Габ5 = Строка.Объем;//Строка.Объем;
					...
					Если Номер = 7 Тогда
						...
						ОбщееКоличествоТоваров = ОбщееКоличествоТоваров + Строка.Количество;
						//ОбластьШапка.Параметры.Стоим6 = Строка.Количество * Строка.Цена;
						ОбластьШапка.Параметры.Габ6 = Строка.Объем;//Строка.Объем;
					...
					Если Номер = 8 Тогда
						...
						ОбщееКоличествоТоваров = ОбщееКоличествоТоваров + Строка.Количество;
						//ОбластьШапка.Параметры.Стоим7 = Строка.Количество * Строка.Цена;
						ОбластьШапка.Параметры.Габ7 = Строка.Объем;//Строка.Объем;
...
Функция ПечатнаяФормаES_НакладнаяНаЗапасы(МассивОбъектов, ОбъектыПечати, ИмяМакета)
	
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.АвтоМасштаб = Истина;
	ТабличныйДокумент.ОриентацияСтраницы = ОриентацияСтраницы.Портрет;
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЗаказНаДоставку_ES_НакладнаяНаЗапасы";
...
Функция ПечатьES_РаспискаОМатериальнойОтветственностиКурьера(МассивОбъектов, ОбъектыПечати, ИмяМакета) Экспорт
	
	ТабДок = Новый ТабличныйДокумент;
	ТабДок.АвтоМасштаб = Истина;
	ТабДок.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЗаказНаДоставку_ES_РаспискаОМатериальнойОтветственностиКурьера";
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока ВыборкаСсылка.Следующий() Цикл
			...
			ОбластьШапка = Макет.ПолучитьОбласть("Шапка");
			ТабДок.Вывести(ОбластьШапка);
			ОбластьОсновнойРаздел = Макет.ПолучитьОбласть("ОсновнойРаздел"); 
			ТабДок.Вывести(ОбластьОсновнойРаздел);
		КонецЦикла;
...
Функция ЭР_ПолучитьВидДоставки(ВидДоставки)
	
	Сокращение = "";
	Если ВидДоставки = Перечисления.ES_ВидыДоставки.СкладДвери Тогда
		Сокращение = "S-D";
	ИначеЕсли ВидДоставки = Перечисления.ES_ВидыДоставки.ДвериДвери Тогда
		Сокращение = "D-D";
	ИначеЕсли ВидДоставки = Перечисления.ES_ВидыДоставки.СкладСклад Тогда
		Сокращение = "S-S";
	ИначеЕсли ВидДоставки = Перечисления.ES_ВидыДоставки.Самовывоз Тогда
		Сокращение = "S";
	КонецЕсли;
