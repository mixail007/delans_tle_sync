...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	// Изменение типовой проверки: ДатыЗапретаИзменения.ОбъектПриЧтенииНаСервере(ЭтотОбъект, ТекущийОбъект);
	ДатыЗапретаИзмененияПереопределяемый.ПроверитьДатуЗапретаРедактированияОбъектаЗаказПокупателя(ЭтотОбъект, ТекущийОбъект);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УправлениеФормой();
	ОбновитьПараметрыВыбораСпецификаций();
	...
	Если ЗначениеЗаполнено(Номенклатура) Тогда
		...
		Если Элемент.ТекущийЭлемент = Элементы.ЗапасыОстатокОбщий 
			...
			Если ЗначениеЗаполнено(ЗначениеПоля) Тогда
				...
				Если Не ТекущиеДанные.ОстатокОбщий = 0 Тогда
...
Процедура ЗаполнитьПоСпецификации(Команда)
	...
	Если Объект.МатериалыЗаказчика.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоСпецификацииЗавершение", ЭтотОбъект), НСтр("ru = 'Табличная часть ""Материалы заказчика"" будет перезаполнена! Продолжить выполнение операции?'"), 
							РежимДиалогаВопрос.ДаНет, 0);
        Возврат;
...
Процедура ШапкаТабличнаяЧастьЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(СтруктураНастройкаДокумента) = Тип("Структура") И СтруктураНастройкаДокумента.БылиВнесеныИзменения Тогда
		...
		Если НЕ ЗначениеЗаполнено(Объект.ПоложениеДатыОтгрузки) Тогда
			Объект.ПоложениеДатыОтгрузки = ПредопределенноеЗначение("Перечисление.ПоложениеРеквизитаНаФорме.ВШапке");
		КонецЕсли;
		Если Объект.ПоложениеДатыОтгрузки<>СтруктураНастройкаДокумента.ПоложениеДатыОтгрузкиВЗаказеПокупателя Тогда
			Если СтруктураНастройкаДокумента.ПоложениеДатыОтгрузкиВЗаказеПокупателя=ПредопределенноеЗначение("Перечисление.ПоложениеРеквизитаНаФорме.ВШапке") Тогда
				// Было в ТЧ, стало в шапке.
				Для каждого Строка Из Объект.Запасы Цикл
					Если НЕ ЗначениеЗаполнено(Строка.ДатаОтгрузки) Тогда
						Продолжить;
					КонецЕсли;
					Объект.ДатаОтгрузки = Строка.ДатаОтгрузки;
					Прервать;
				КонецЦикла;
			ИначеЕсли СтруктураНастройкаДокумента.ПоложениеДатыОтгрузкиВЗаказеПокупателя=ПредопределенноеЗначение("Перечисление.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти") Тогда
				// Было в шапке, стало в ТЧ.
				Для каждого Строка Из Объект.Запасы Цикл
		...
		Если Объект.ВидОперации=ПредопределенноеЗначение("Перечисление.ВидыОперацийЗаказПокупателя.ЗаказНаПродажу") Тогда
			...
			Если Объект.ПоложениеСклада<>СтруктураНастройкаДокумента.ПоложениеСкладаВДокументахОтгрузки Тогда
				Если СтруктураНастройкаДокумента.ПоложениеСкладаВДокументахОтгрузки=ПредопределенноеЗначение("Перечисление.ПоложениеРеквизитаНаФорме.ВШапке") Тогда
					// Было в ТЧ, стало в шапке.
					Для каждого Строка Из Объект.Запасы Цикл
						Если НЕ ЗначениеЗаполнено(Строка.СтруктурнаяЕдиницаРезерв) Тогда
							Продолжить;
						КонецЕсли;
						Объект.СтруктурнаяЕдиницаРезерв = Строка.СтруктурнаяЕдиницаРезерв;
						Прервать;
					КонецЦикла;
					Для каждого Строка Из Объект.Запасы Цикл
						Строка.СтруктурнаяЕдиницаРезерв = Объект.СтруктурнаяЕдиницаРезерв;
					КонецЦикла;
				ИначеЕсли СтруктураНастройкаДокумента.ПоложениеСкладаВДокументахОтгрузки=ПредопределенноеЗначение("Перечисление.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти") Тогда
					// Было в шапке, стало в ТЧ.
					Для каждого Строка Из Объект.Запасы Цикл
						Строка.СтруктурнаяЕдиницаРезерв = Объект.СтруктурнаяЕдиницаРезерв;
					КонецЦикла;
...
Функция ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные)
	...
	Если ЗначениеЗаполнено(СтруктураДанные.Номенклатура) и СтруктураДанные.Номенклатура.ИспользоватьХарактеристики
		...
		Если ЗначениеЗаполнено(СтруктураДанные.Характеристика)
			Тогда
...
Функция ЗаполнитьПоДаннымШтрихкодов(ДанныеШтрикодов)
	...
	Для каждого ТекШтрихкод Из СтруктураДанные.МассивШтрихкодов Цикл
		...
		Иначе
			МассивСтрокТЧ = Объект.Запасы.НайтиСтроки(Новый Структура("Номенклатура,Характеристика,ЕдиницаИзмерения,НомерВариантаКП,НоменклатураНабора",ДанныеШтрихкода.Номенклатура,ДанныеШтрихкода.Характеристика,ДанныеШтрихкода.ЕдиницаИзмерения,ТекущийВариантКП,ПредопределенноеЗначение("Справочник.Номенклатура.ПустаяСсылка")));
...
Процедура ОбновитьОтображениеКолонокВРазрезеЗапасов()
	...
	Если РежимОстаткиИРезервы Тогда
		...
		Элементы.ЗапасыКонтекстноеМенюГруппаРаботаССтроками.Видимость = Истина;
	...
	Иначе
		...
		Элементы.ЗапасыКонтекстноеМенюГруппаРаботаССтроками.Видимость = Ложь;
...
&НаСервере
Функция ПолучитьДанныеПриИзмененииПоляСОбработкойОстатковИРезервов(СтруктураДанные, ИмяПоля = Неопределено)
	
	Если ИмяПоля = "Характеристика" Тогда
		СтруктураДанные = ПолучитьДанныеХарактеристикаПриИзменении(СтруктураДанные);
	КонецЕсли;
	
	НоменклатураВДокументахСервер.ЗаполнитьЗначенияРезервовИОстатокНоменклатурыВТабличнойЧасти(Объект, СтруктураДанные, ТекущийВариантКП,, РезервированиеЗапасов);
...
Процедура ПослеВыбораИзВыпадающегоСпискаТабличнойЧасти(Результат, Параметры) Экспорт
	...
	Если Параметры.ВыбранноеПоле = "ЗапасыОстатокОбщий" Тогда
		...
		Если Не РезервированиеЗапасов Тогда Возврат КонецЕсли;
...
&НаСервере
Процедура ОбновитьПараметрыВыбораСпецификаций()
	
	МассивПараметров = Новый Массив(Элементы.ЗапасыСпецификация.ПараметрыВыбора);
	Для ии = 1 По МассивПараметров.Количество() Цикл
		Индекс = МассивПараметров.Количество()-ии;
		Параметр = МассивПараметров[Индекс];
		Если Параметр.Имя="Отбор.ЗаказПокупателя" Тогда
			МассивПараметров.Удалить(Индекс);
		КонецЕсли; 
	КонецЦикла; 
	
	Значения = Новый Массив;
	Значения.Добавить(Документы.ЗаказПокупателя.ПустаяСсылка());
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Значения.Добавить(Объект.Ссылка);
	КонецЕсли;
	МассивПараметров.Добавить(Новый ПараметрВыбора("Отбор.ЗаказПокупателя", Новый ФиксированныйМассив(Значения)));
	
	Элементы.ЗапасыСпецификация.ПараметрыВыбора = Новый ФиксированныйМассив(МассивПараметров);
...
Процедура НастроитьЭлементыЯндексДоставки()
	...
	Если ЗначениеЗаполнено(НастройкаЯндексДоставки) Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПередатьВЯндексДоставку", "Доступность", Не ТолькоПросмотр);
...
Процедура НоменклатураДоставкиПриИзмененииНаСервере()
	...
	ЗаполнятьСтоимость = ЗаполнятьСтоимостьДоставкиПоЦенеНоменклатуры(ЗаданаФормулаРасчета, ВыбранаСлужбаДоставкиЯндекса, ВариантУчетаДоставки);
