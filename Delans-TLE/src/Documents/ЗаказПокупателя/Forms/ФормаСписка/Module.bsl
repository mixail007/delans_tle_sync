...
Процедура УстановитьСостояниеЗаказа(ИмяКоманды, Заказы, ВидЗаказа = Неопределено, ПричинаОтмены = Неопределено, Заметки = Неопределено)
	...
	ПоказатьОповещениеПользователя(СтрШаблон(НСтр("ru='Изменение (%1)'"),
		...
		НСтр("ru='Заказы покупателей'"),БиблиотекаКартинок.Информация32);
...
Процедура УстановитьСостояниеЗаказаСервер(ВидЗаказа, ИмяКоманды, Заказы, ПричинаОтмены, Заметки)
	...
	Для Каждого Заказ Из Заказы Цикл
		...
		Если Заказ.СостояниеЗаказа = СсылкаНаСостояние И СсылкаНаСостояние <> Справочники.СостоянияЗаказовПокупателей.Завершен Тогда
			НеизмененныеЗаказы.Добавить(Заказ);
			Продолжить;
		КонецЕсли;
		
		Если Заказ.СостояниеЗаказа = СсылкаНаСостояние И СсылкаНаСостояние = Справочники.СостоянияЗаказовПокупателей.Завершен
