...
Процедура ЗаполнитьНазначениеПлатежа(НазначениеВФорме = "", ИзменениеСуммы = Ложь) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(НазначениеВФорме) Тогда
	...
	Если ПустаяСтрока(НазначениеВФорме)
		...
		Если ТекНомер <> "" Тогда
			НазначениеВФорме = НСтр("ru='Оплата по счету N %НомерСчета%'");
			НазначениеВФорме = СтрЗаменить(НазначениеВФорме, "%НомерСчета%", СокрЛП(Строка(ТекНомер)));
			Если ЗначениеЗаполнено(ТекДата) Тогда
				НазначениеВФорме = НазначениеВФорме + " от " + СокрЛП(Строка(Формат(ТекДата, "ДФ='dd MMMM yyyy'"))) + " г.";
			КонецЕсли;
		КонецЕсли;
		...
		Иначе
			...
			Если ПустаяСтрока(НазначениеВФорме)
				...
				ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПриходнаяНакладная") Тогда
				    НазначениеВФорме = НСтр("ru='Оплата по накладной N %НомерСчета%'");
				КонецЕсли; 
				
				НазначениеВФорме = СтрЗаменить(НазначениеВФорме, "%НомерСчета%", СокрЛП(Строка(НомерВходящегоДокумента)));
				
				Если ЗначениеЗаполнено(ДатаВходящегоДокумента) Тогда
					НазначениеВФорме = НазначениеВФорме + " от " + СокрЛП(Строка(Формат(ДатаВходящегоДокумента, "ДФ='dd MMMM yyyy'"))) + " г.";
				КонецЕсли;
...
Процедура ЗаполнитьПоНачислениеНалогов(ДокументОснование) Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Выборка.Следующий();
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, Выборка);
		ЭтотОбъект.Налог = Выборка.ВидНалога;
