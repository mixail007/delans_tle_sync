...
Процедура ЗаполнитьПоНакладнойЕГАИС(ТТНВходящаяЕГАИС) Экспорт
	...
	ЭтотОбъект.ПолученИзЕГАИС = Истина;
...
Процедура ЗаполнитьТоварыПоступленияПоТТН(ТТНВходящаяЕГАИС) Экспорт
	...
	|	ТаблицаЕГАИС.Номенклатура.ЕдиницаИзмерения КАК ЕдиницаИзмерения
	|ИЗ
	...
	|	ТаблицаЕГАИС.Номенклатура.ЕдиницаИзмерения
	|
	|УПОРЯДОЧИТЬ ПО
	|	Порядок";
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаТаблицы = ЭтотОбъект.Запасы.Добавить();
		...
		Если ЭтотОбъект.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС Тогда
			...
			Иначе
				СтрокаТаблицы.СтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(ЭтотОбъект.Организация.ВидСтавкиНДСПоУмолчанию);
			КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	РаботаСПодарочнымиСертификатами.ПроверитьВозможностьИспользованияСертификатов(Отказ, ЭтотОбъект, "Запасы", "СтруктурнаяЕдиница",
...
Процедура ЗаполнитьПоВходящейТранспортнойОперации(ДанныеЗаполнения) Экспорт
	...
	Реквизиты = ИнтеграцияВЕТИСПереопределяемый.ДанныеПрикладныхДокументовИзВходящейТранспортнойОперацииВЕТИС(ДанныеЗаполнения);
	Реквизиты.Следующий();
	...
	Для Каждого СтрокаТовары Из ТоварыУточнение Цикл 
		...
		НоваяСтрокаЗапасы.СтавкаНДС = Справочники.СтавкиНДС.СтавкаНДС(НоваяСтрокаЗапасы.Номенклатура.ВидСтавкиНДС);
