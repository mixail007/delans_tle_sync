...
Функция ПодготовитьПараметрыОтчета(НастройкиОтчета)
	...
	ИначеЕсли ПараметрПериод <> Неопределено
		И НЕ ПараметрПериод.Использование Тогда
		ПараметрПериод.Использование = Истина;
		Период = КонецДня(ТекущаяДата());
		ПараметрПериод.Значение = Период;		
	КонецЕсли;
	
	ПараметрДоговор = НастройкиОтчета.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("Договор"));
	Если ПараметрДоговор <> Неопределено Тогда
		Если НЕ ПараметрДоговор.Использование Тогда
			ПараметрДоговор.Использование = Истина;
			Договор = Справочники.ДоговорыКонтрагентов.ПустаяСсылка();
			ПараметрДоговор.Значение =  Договор;
		КонецЕсли;
	КонецЕсли;
	...
	ПараметрыОтчета.Вставить("ИдентификаторОтчета" , "ПрайсЛистДоставки");
