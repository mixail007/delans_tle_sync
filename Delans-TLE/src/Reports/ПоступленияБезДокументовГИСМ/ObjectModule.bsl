...
Процедура ВывестиДанныеОПоступлениях(Макет,
	...
	ДополнитьТаблицу(Поступления, РезультатЗапроса[6].Выгрузить(), ИмяНабораДанных);
	ДополнитьТаблицу(ДокументыГИСМ, РезультатЗапроса[7].Выгрузить(), ИмяНабораДанных);
	
	НомерСтроки = 1;
	СтруктураПоиска = ПустаяСтруктураКлючевыеПоля();
	
	ВыборкаКонтрагенты = РезультатЗапроса[5].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
