...
Функция ВыгрузкаВозможна(ИмяФормы,
	...
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Приложение2, "П000020000101" + _НомСтр)
		...
		Если СтрокаЗаполнена Тогда
			...
			Если НЕ ЗначениеЗаполнено(ЗначГр1) Тогда
				...
				СтатусПроверки = Ложь;
			ИначеЕсли СтрДлина(ЗначГр1) < 5
				ИЛИ НЕ РегламентированнаяОтчетностьКлиентСервер.ОКВЭДСоответствуетТребованиям(ЗначГр1) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
				РезультатВыгрузки, НСтр("ru='В строке " + Формат(НомСтр, "ЧГ=")
				+ " распределения доходов и поступлений за предыдущий финансовый год задан код основного вида экономической деятельности, не соответствующий одному из возможных шаблонов: ""XX.XX"", ""XX.XX.X"", ""XX.XX.XX"" (X - любая цифра).'"),
				"Приложение2", "П000020000101" + _НомСтр);
				СтатусПроверки = Ложь;
			КонецЕсли;
	...
	Если НЕ ЗаполненаХотяБыОднастрока Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
		РезультатВыгрузки, НСтр("ru='В распределении доходов и поступлений за предыдущий финансовый год не заполнена ни одна строка.'"),
...
Функция СформироватьСтруктуруПараметров(ИмяФормы,
	...
	СтруктураРеквизитовФормы.мСохраненныйДок.Организация, Приложение1.ДатаПодписи, СтрокаСведений);
	...
	ПараметрыВыгрузки.Вставить("РегНомер",         СокрЛП(Приложение1.РегистрационныйНомерФСС));
	ПараметрыВыгрузки.Вставить("КодПодчиненности", СокрЛП(Приложение1.КодПодчиненности));
