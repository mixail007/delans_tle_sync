...
Функция ВременныеТаблицыВоронкиПродажПоЛидам(НастройкиКД) Экспорт 
	...
	|	МАКСИМУМ(КурсыВалютСрезПоследних.Период) КАК ПериодВалюта,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют КАК КурсыВалютСрезПоследних
	|		ПО ВТ_Заказы.ПоследнийПериод >= КурсыВалютСрезПоследних.Период
	|ГДЕ
	|	КурсыВалютСрезПоследних.Валюта В (&ВалютаУчета, &ВалютаНациональная)
	|
	|СГРУППИРОВАТЬ ПО
...
Функция ТаблицыДанныхВоронкиПоЗаказам()
	...
	|	ВТ_Заказы КАК ВТ_Заказы
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют КАК КурсыВалютСрезПоследних
	|		ПО ВТ_Заказы.ПериодСостояния >= КурсыВалютСрезПоследних.Период
	|ГДЕ
	|	КурсыВалютСрезПоследних.Валюта В (&ВалютаУчета, &ВалютаНациональная)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаКурсыВалютСрезПоследних КАК КурсыУпрВалюты
	|		ПО (КурсыУпрВалюты.Валюта = &ВалютаУчета)
	|			И (НАЧАЛОПЕРИОДА(ВТ_ПериодЗаказы.ПериодСостояния, ДЕНЬ) = КурсыУпрВалюты.ПериодСостояния)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаКурсыВалютСрезПоследних КАК КурсыРегВалюты
	|		ПО (КурсыРегВалюты.Валюта = &ВалютаНациональная)
	|			И (НАЧАЛОПЕРИОДА(ВТ_ПериодЗаказы.ПериодСостояния, ДЕНЬ) = КурсыРегВалюты.ПериодСостояния)";
