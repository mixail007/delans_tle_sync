...
Функция ДанныеПоПотерям(Знач ТаблицаПотерь)
	...
	Для Каждого ТекСтр Из ТаблицаПотерь Цикл
		...
		Если Результат[ТекСтр.ПолеГруппировки][ТекСтр.СостояниеОтмены] = Неопределено Тогда
			...
			Результат[ТекСтр.ПолеГруппировки][ТекСтр.СостояниеОтмены].Вставить("ВидЗаказа", ТекСтр.ВидЗаказа);
...
Процедура ЗаполнитьПотери(Знач ОбластьСостояние, Знач ВыборкаВоронкаПродажСостояние, Знач ДанныеПоПотерям, Знач КоличествоСостоянийПоВидуЗаказа, Знач ЦветаГрадиента)
	...
	Если ПотериПоВидуЗаказаИСостоянию.ВидЗаказа <> ВыборкаВоронкаПродажСостояние.ВидЗаказа Тогда
		Возврат;
	КонецЕсли;
...
Функция ФильтрыВоронки(Форма, НастройкиОтчета, НастройкиСКД, ВидВоронки, ГруппаФильтрВидОперацииВидЗаказа, ДобавляемыеРеквизиты, ИменаРеквизитов)
	...
	ДобавитьФильтрВидОперации(Форма, ВидВоронки, ГруппаФильтрВидОперацииВидЗаказа,ДобавляемыеРеквизиты, ИменаРеквизитов, ФильтрыВоронки);
...
Процедура ДобавитьФильтрВидОперации(Форма, ВидВоронки, ГруппаФильтр, ДобавляемыеРеквизиты,ИменаРеквизитов, ФильтрыВоронки)
...
Функция ТекстЗапросаИсточникиПоПокупателям(НастройкиКД)
	...
	Для Каждого ТекЭлементОтбора Из НастройкиКД.Отбор.Элементы Цикл
		...
		Если ТипЗнч(ТекЭлементОтбора) <> Тип("ЭлементОтбораКомпоновкиДанных") Тогда
			Продолжить;
		КонецЕсли;
